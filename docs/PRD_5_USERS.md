...
- 구독하기 버튼 → `/chat` 페이지로 이동 (구독 결제는 채팅 화면에서)
...
function getPaymentLabel(memo: string, metadata: Record<string, unknown>): string {
  ...
  if (type === 'CHAT_ROOM_CREATE') return '채팅방 생성'
...
```json
{
  "profile": {
    ...
    "subscription": {
      ...
      "subscribe": "구독하기",
      "benefits": {
        "roomQuota": "월 {count}개 무료 채팅방 생성",
        "roomUnlimited": "무제한 채팅방 생성",
        "aiQuota": "AI 월 {count}회 사용",
        "aiUnlimited": "AI 무제한 사용",
        "canTip": "Pi Tip 전송",
        "premiumTheme": "프리미엄 테마 사용",
        "eventRoom": "이벤트 채팅방 개설"
      }
    }
  }
}
