...
| 5 | **글로벌 동시통역 (PiTranslate™)** | 어떤 언어로 보내도 내가 선택한 언어로 즉시 통역 — 언어 장벽 제로 |
...
## 1-4. 글로벌 동시통역 (PiTranslate™) — "언어 없는 채팅"

> **Discord도 없다. Telegram도 없다. WhatsApp도 없다.**
> 전 세계 어디서든, 어떤 언어로 보내도, 내가 선택한 언어로 실시간 통역되는 채팅은 **PiChat이 처음이다.**

---

### 왜 이것이 글로벌 런칭의 킬러 피처인가

Pi Network는 전 세계 200개국 3,500만 명이 사용하는 글로벌 네트워크다. 그런데 지금까지 Pi 기반 서비스에서 **한국 사용자와 나이지리아 사용자가 같은 채팅방에서 자유롭게 대화한 적이 없다.** 이유는 하나 — 언어 장벽.

PiTranslate™는 이 장벽을 허문다:
...
하나의 대화가 **동시에 4개 언어로 흐른다.** 누군가 번역 버튼을 누를 필요도 없다. 채팅창을 나갈 필요도 없다.
...
> **전략적 판단**: 월 $30 미만의 비용으로 "언어 장벽 없는 채팅"이라는 글로벌 킬러 피처를 전 사용자에게 무료로 제공한다. Pi 생태계에서 전례 없는 경험이다.
...
사용자 프로필에서 "내가 보고 싶은 언어"를 설정한다. 이후 모든 채팅방에서 자동 적용.
...
| **다언어 채팅방** (3개국 이상 참가) | 전체 공개방의 20% |
| **번역 캐시 히트율** | 60% 이상 (중복 번역 최소화) |
| **PiTranslate™ DAU** | MAU의 30% |
...
CREATE TABLE msg_msg (
  ...
  src_lang_cd VARCHAR(20),              -- 원본 언어 코드 (PiTranslate™ — Claude Haiku 감지)
  del_yn      CHAR(1)      DEFAULT 'N',
  regr_id     TEXT  NOT NULL DEFAULT 'ADMIN',
  reg_dtm     TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  modr_id     TEXT         NOT NULL DEFAULT 'ADMIN',
...
| **AI 채팅 비서** (`@ai` 멘션) | **Claude Haiku** (`claude-haiku-4-5-20251001`) | 이미 통합된 SDK, 테마 맥락·슬랭·Pi 생태계 이해 우수 |
...
### Phase 10: 글로벌 동시통역 (PiTranslate™) ⭐ 글로벌 런칭 킬러 피처
...
src/components/chat/lang-setting-button.tsx 채팅방 내 표시 언어 변경 버튼
```
...
| v1.4 | 2026-06-10 | 글로벌 동시통역 (PiTranslate™) 전면 추가 — 섹션 1-4 신설·KPI 번역 지표·DB `msg_trans` 테이블·번역 API·기술 아키텍처(Claude Haiku + dedup + broadcast)·Phase 10 로드맵 TASK-080~089 |
