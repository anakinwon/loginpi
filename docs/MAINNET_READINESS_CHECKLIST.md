...
- **✅ 마스터 결정(2026-06-27)**: 수집항목은 **O2O 서비스용 선택 옵션·강제 아님** → 완료. 매핑표=부록2, 프로필 UI 필수/선택 표기 완료(common.optional).
- **✅ 개인정보처리방침 반영(2026-07-09)**: v1.1 개정 초안(kor/eng)에 **3-9절 「프로필 추가 정보(선택)」 신설** — 실명·연락처·주소·닉네임/자기소개의 수집 범위·용도(O2O 미픽업 연락/배송 대비)·선택여부·보유기간 명시. 입증 체인(UI 표기 → 방침 명시 → 부록2 매핑표) 완결. 시행은 v1.1 공지 절차(제11조)에 따름.
...
| 10 | 등재 신청 제출 (요건 A-1~A-7 충족 상태) | A 전체 | ✅ **완료**(2026-07-16) — 공식 문서 3소스 확인 결과 별도 제출 폼/버튼 부재. **포털 App Checklist 10/10 완료(마스터 확인)가 곧 신청 행위**, Ecosystem 노출은 Core Team 선별 큐레이션. 이후=승인 대기 모니터링(디렉토리 노출·A2U 개방 프로브) → 승인 감지 시 0순위 런북 |
...
**근거 정본**: 위 공식 출처 4개 · **최종 갱신**: 2026-07-16 (Part E 표 DB `mainnet_checklist` 실측 기준 현행화 — 등재팀 워크플로우 점검 반영)
...
## 권고 조치 (잔여)
- ✅ 프로필 폼 **선택 항목 라벨** — 구현 확인(2026-07-09): `profile-form.tsx`가 display_name만 `common.required`, 나머지 전 필드 `common.optional` 표시 (i18n 키, 189 locale 적용).
- ✅ 개인정보처리방침 **수집 항목·용도·선택여부** 반영 — v1.1 개정 초안 3-9절 신설(2026-07-09, kor/eng).
- 등재 신청 시 본 매핑표를 "수집 항목별 필수성 근거"로 제출. ← 유일 잔여(신청 시점 액션)
...
## 🚀 등재 실행 런북 (2026-07-10 — 메인넷 등록팀 하네스 산출)

> 4역할 병렬 감사 결과(레드라인·체크리스트·제출 패키지·P0 시나리오) 종합. 재실행: `.claude/workflows/mainnet-listing-team.js`

### 판정 요약
- **레드라인 4종 전부 OK** (도박 0건·통화 게이트 3곳 연결·Pi 외 로그인 강제 없음·Py 개명 완료). LISTING_MODE=true 운영 설정 확인됨(E-7).
- **잔여 = 마스터 포털/env 수작업 5건뿐** (코드·문서 작업 0건). 문서-DB 정합 100%.
- 절제 오버레이 보강(2026-07-10): 관리자 화면 '발행' 문구 5키 → '지급' 순화 추가.

### 마스터 실행 순서 (7단 — 병렬 금지, 순서 엄수)

| 단계 | 작업 | 소요 |
|---|---|---|
| ① B-5 | Dev Portal 메인넷 프로젝트 생성 + **App Network=Mainnet** 선택(비가역) | 5분 |
| ② E-3 | 도메인 검증 키 발급(포털 자동) → 키 복사 | 1분 |
| ③ E-4 | Vercel env `PI_DOMAIN_VALIDATION_KEY` 입력 + 재배포 → `cafepi.vercel.app/validation-key.txt` 200 확인 | 10분 |
| ④ E-5 | 메인넷 PI_API_KEY 발급 확인 | 정보수집 |
| ⑤ E-7 | env 스왑: `PI_API_KEY`(mainnet)·`PI_WALLET_PRIVATE_SEED`(mainnet) + 재배포 | 5분 |
| ⑥ D-1 | **P0 실기기 검증** (아래 12단계 시트, ~15분) | 15분 |
| ⑦ E-10 | Dev Portal 등재 신청 제출 | — |

### P0 실기기 검증 시트 (12단계 · ~15분)

1. Pi Browser 로그인 → 헤더 별명 표시 (pi_username 재바인딩 정상)
2. 카페 입장 → 메시지 송수신 (Realtime + 폴백)
3. 자동 번역 1건 (PyTranslate™)
4. 상점 상품 조회 → 결제 버튼 활성 (window.Pi 가드)
5. **Pi 결제 1건 U2A** (approve→complete, 소액)
6. 주문 상태 변경 → 텔레그램 알림 수신 (TXN_ST)
7. 관리자 /admin/monitor — Pi 결제 성공률 신호등 🟢
8. 텔레그램 딥링크 → Pi Browser 복귀 → 세션 유지
9. 일반 브라우저 Pi Sign-In (QR 또는 딥링크 여정 1종)
10. 레드라인 육안: 시세·환율·Bean Token·베팅 표현 0건, Pi Browser에서 Google 버튼 미노출
11. 프로모 게이트: 과금 경로 무료 표시 + /admin/open-promo 설정 확인
12. 전 단계 PASS → E-10 제출 / 1건이라도 FAIL → 중단·보고

### 제출 필드 초안 (submission-packager 산출)
- 앱명: **PyCafé** (pi 접두 금지 준수) · URL: `https://cafepi.vercel.app`
- 법무 URL(실존 확인): `/docs/legal/privacy` · `/docs/legal/terms` (7/16부터 v1.1)
- 기술부록: `PI_LISTING_TECH_APPENDIX_EN.md` (2026-07-10 현행화 — 189 locale·Pi Sign-In 반영)
- 잔여 보완: 영문 짧은 설명 확정·지원 이메일 결정(마스터)·스크린샷 촬영(홈·카페·상점·결제)
