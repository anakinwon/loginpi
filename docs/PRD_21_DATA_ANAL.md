...
**마지막 수정:** 2026-07-08  
**상태:** 구현 완료 — 4탭 통합 페이지(/admin/analytics) 운영 중. ✅내보내기(CSV) 완료(2026-07-10 — 현재 탭 데이터 섹션별 CSV, `src/lib/csv-export.ts`+허브 버튼, BOM 엑셀 호환). 잔여: 지리 지도(선결 대기)·채널별 전환 퍼널  
...
7. **브랜드 표기**: PyCafé™ / PyShop™ / PyTranslate™ (표시 텍스트만, 코드값은 원형)
...
| mps_order | PyShop™ 주문 | order_id, usr_id, order_status, reg_dtm |
...
- CLAUDE.md § 공식 브랜드 표기 — PyCafé™, PyShop™, Bean(카페빈)
