...
> **버전**: v2.1
> **최종 업데이트**: 2026-06-17
> **상태**: v1.x 구현·운영 중 (참여 7·완주 1) · v2.0 행위-이벤트 분리 아키텍처는 기획 · 10미션 완주 실 보상(1π 보증금 수동 지급) 구현 완료
...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| **v2.1** | 2026-06-17 | **10미션 완주 실 보상 구현 — A2U 폐기 → 1π 판매보증금 적립(§15-5 신설)** — ① Pi A2U 송금(시드 미설정·실패 시 PENDING)을 폐기하고 `mps_seller_bond` 보증금 1π 직접 적립으로 전환(`5f5e6b9`·`d540f68`). ② **자동 지급 → 관리자 수동 버튼**(`a277b80`): 이벤트 화면 '1Pi 판매보증금 지급' 버튼 + `POST /api/admin/event/bond-reward`(isAdmin 이중검증), `event.ts` 미션 평가의 자동 보상 호출 제거. ③ **원자적 중복방지** `fn_evt_grant_bond_reward`(`sql/061`) — 단일 트랜잭션 + `FOR UPDATE` 행잠금 + `reward_st_cd`('BONDED'/'PAID') 게이트로 TOCTOU race·A2U-보증금 교차 이중지급 차단, `grantBondReward`를 RPC 래퍼로 교체. ④ 추적 `evt_pi_reward_log`(`sql/048`, `UNIQUE(event_id,user_id)`). ⑤ M3 결함 수정: `premium_cafe_create` 기록을 유료 테마로 게이트(무료 FITNESS 우회 차단). §14-4 갱신. | asoká |
...
- 정기(배치): **매일 자정 1회** (`0 0 * * *`) 활성 이벤트의 모든 미션에 대해 사용자별 완료 여부 재평가
  - M6처럼 3가지 조건(voice+file+sticker)을 모두 감시할 때 필요
  - 개별 API 트리거가 복잡한 경우 배치 평가로 단순화
  - ⚠️ **Vercel Hobby 플랜 제약**: 하루 1회 초과 cron(`*/5`, `0 * * * *` 등)은 배포 자체가 차단됨. 고빈도 재평가는 Pro 플랜($20/월) 필요 → `docs/TROUBLESHOOT.md` 참조
...
### 15-5. ✅ 확정·구현: 10미션 완주 실 보상 = 1π 판매보증금 적립 (2026-06-16~17)

§14-4의 A2U 한계로, 10미션 완주자에 대한 실 보상을 **판매보증금(`mps_seller_bond`) 1π 직접 적립**으로 구현했다. 블록체인 송금이 아니라 플랫폼 보증금 장부에 잔액을 가산하는 방식이라 송금 실패가 구조적으로 없다.

#### 15-5-1. 지급 방식 = 관리자 수동 버튼 (자동 지급 폐기)

초기 구현(`5f5e6b9`·`d540f68`)은 미션 평가 시 자동으로 보증금을 적립(`grantBondReward`)했으나, **`a277b80`에서 자동 지급을 폐기하고 관리자 수동 버튼으로 전환**했다.

- **버튼**: 이벤트 화면(`ClientEventGate`)의 '🔄 미션 재평가' 버튼 옆에 **'1Pi 판매보증금 지급'** 버튼 신설 (관리자 전용)
- **지급 대상**: 10미션 완주(`count === 10`) + 미지급(`reward_st_cd` 미존재) + 어뷰저(`evt_exclude`) 제외자만
- **API**: `POST /api/admin/event/bond-reward` (`isAdmin` 이중 검증)
- **이유**: 보상 지급은 고객 신뢰·자금 직결 행위이므로, 자동 트리거가 아닌 **관리자 명시적 승인** 시점에만 발생하도록 게이트. 미션 평가(`event.ts`)에서 자동 보상 호출을 완전히 제거함.

#### 15-5-2. 원자적 중복방지 — DB 함수 `fn_evt_grant_bond_reward` (`sql/061`)

앱 레벨 check-then-act는 TOCTOU(Time-Of-Check-To-Time-Of-Use) race와 A2U-보증금 교차 이중지급에 취약하다. 이를 **단일 트랜잭션 DB 함수로 원자화**했다.

| 방어 장치 | 설명 |
|---|---|
| **단일 트랜잭션** | 잔액 확인 + 적립 + 로그 기록을 하나의 트랜잭션으로 처리 |
| **`FOR UPDATE` 행잠금** | 동일 사용자 보상 행을 잠가 동시 요청 직렬화 |
| **`reward_st_cd` 게이트** | `'BONDED'`/`'PAID'` 상태면 즉시 멱등 skip — 재호출해도 이중 지급 없음 |
| **교차 차단** | A2U 지급 이력과 보증금 적립 이력을 같은 게이트로 묶어 두 경로 동시 지급 차단 |

- `grantBondReward`(앱)는 이제 위 RPC를 **호출만** 하는 race-free 래퍼로 교체됨.
- **추적 테이블**: `evt_pi_reward_log` (`sql/048`) — `UNIQUE(event_id, user_id)` 멱등 보장, `reward_st_cd`로 지급 상태 추적.

#### 15-5-3. M3 미션 우회 결함 동시 수정 (`a277b80`)

보상 전환과 함께, **M3(PREMIUM 카페 생성) 미션을 무료 테마로 우회하던 결함**을 수정. `premium_cafe_create` 행위 기록을 **유료 테마 게이트**로 막아, 무료 FITNESS 테마로 카페를 만들어 M3를 부정 완료하던 경로를 차단(`src/app/api/chat/rooms/group/route.ts`).

> ⚠️ 부수: 병합 과정에서 `group/route.ts`에 LBS 위치 저장 블록(`validLat`/`validLng`)이 이중 삽입된 결함을 `6a648b5`에서 제거(로직 변경 없음).
...
- **정기 배치** (매일 자정 `0 0 * * *`): `evaluateUserMissions()` 엔진이 활성 이벤트의 모든 미션 재평가 → evt_user_mission 자동 갱신
  > ⚠️ Vercel Hobby 플랜은 하루 1회 초과 cron 배포 차단 — 고빈도 재평가 필요 시 Pro 업그레이드 필요 (`docs/TROUBLESHOOT.md`)
- **멱등성**: `(event_id, user_id, mission_cd)` UNIQUE 제약으로 중복 기록 방지
