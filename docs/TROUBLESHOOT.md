...
### [2026-07-15 정정·심화] 마스터 관찰로 2차 문제 확정 — 운영 시드가 테스트넷 앱 지갑 ⭐

마스터 관찰("결제 받은 주소 GA3L54ONC… ≠ 환불 시도 주소 GD3W3DGC…")을 온체인·이력으로 검증, **위 항목의 "시드 문제 아님" 판단을 정정**한다:

- **GD3W3DGC…(운영 시드가 파생하는 지갑) = 테스트넷 앱 지갑** — 근거 ① 7-02 테스트넷 A2U의 from_address와 동일 ② **메인넷에 계정 자체가 미존재**(활성화 이력 없음, 잔액 0 — 마스터 실측+Horizon 확인)
- **GA3L54ONC… = 메인넷 실결제 입금처** (Horizon 실측: 에스크로 0.8π 수신, 잔액 5,470π)
- 즉 7-11 "메인넷 키+시드 교체"에서 **API 키만 메인넷으로 바뀌고 시드는 테스트넷 것이 그대로** 남음(또는 테스트넷 시드를 재투입).

**두 층위의 구분 (중요)**:
1. 현재 400 feature_not_available은 createPayment(API 키) 단계 — **시드가 아예 사용되기 전**이라 시드가 이 에러의 원인은 아님(플랫폼 A2U 게이트).
2. 그러나 A2U가 열려도 현 시드로는 즉시 실패한다(서명 지갑이 메인넷에 미존재·잔액 0) — **시드 교체는 선결 필수**.

**조치 순서 (마스터 전용 — 시드는 절대 세션·저장소에 남기지 않음)**:
1. Pi Developer Portal(메인넷 앱) → App Wallet 주소 확인 (GA3L54ONC…인지)
2. 그 지갑의 시드로 운영 Vercel `PI_WALLET_PRIVATE_SEED` 교체 + 재배포
3. 반영 확인 = 에러 태그가 `serverWallet=GA3L…`로 바뀌는지 (관찰성 기구축)
4. 이후에도 feature_not_available이면 그때가 진짜 등재 승인 대기 — 개방 시 sweep 자동 환불
5. ⚠️ 보안: 입금처 지갑에 개인 자금(5,470π)이 함께 있으면 서버 시드 유출 시 전액 위험 — 운영 유동분만 남기고 분리 보관 권장

**[2026-07-15 15:30 결말]** 마스터가 포털 메인넷 앱 지갑 시드 확인·교체 + 재배포(ac6394c9) 후 환불 재시도 실측:
에러 태그 `serverWallet=GA3L54ON…`으로 전환 확인 → **시드 문제 해결**(서명 지갑=입금처, 잔액 5,470π).
잔여 차단은 `feature_not_available` 단 하나 = Pi 메인넷 A2U 미개방(등재 승인 대기, 앱 구성은 완결).
개방 시: 환불=sweep 자동 / 정산=admin/store/settle 백필. 검증 프로브=관리자 환불 버튼 1클릭 → pi_pymnt.metadata.refund 판독.

---

## [2026-07-17] "동의하고 시작" 버튼 무반응 — Pi 자동인증 레이스 401 오판 + 실패 무피드백 이중 결함 ⭐

**증상**: 마스터가 운영 실기기에서 지도(주변 탐색) 진입 시 LBS 동의 다이얼로그가 뜨고, "동의하고 시작" 버튼을 눌러도 아무 반응 없음. 재현 시점에 마스터 계정은 운영·스테이징 DB 모두 **이미 LBS 동의 완료(`lbs_consent_yn='Y'`)** 상태 — 즉 다이얼로그가 뜬 것 자체가 이상.

**원인 (2중 결함 — 플랫폼 제약 × 클라이언트 오판)**:
1. **401 오판** (`nearby-explorer.tsx`): Pi Browser 인증은 비동기(SDK `authenticate()` → `/api/auth/pi` → localStorage `pi_token`)라서, 마운트 직후의 동의 상태 조회 `GET /api/location/consent`는 **인증 완료 전 레이스로 401**이 날 수 있다. 기존 코드는 `r.ok ? r.json() : null` 후 null을 "미동의 확정(N)"으로 처리하고 **동의 localStorage 캐시(`lbs_consent_y`)까지 삭제** → 동의자에게 동의 게이트가 오뜸.
2. **silent failure** (`lbs-consent-dialog.tsx`): 동의 POST가 `if (res.ok)`만 처리하고 else 없음 — 같은 401로 실패해도 무반응이라 "죽은 버튼"으로 체감. 문제 보고가 원인 단서 없이 도착("버튼이 동작 안 해").

**진단 열쇠**: DB에서 해당 사용자의 `lbs_consent_yn` 실측 → 이미 Y인데 게이트가 떴다 = 동의 로직이 아니라 **세션/조회 계층** 문제로 확정. 최근 `sys_user_consent` LBS 행 존재(7/14) = POST 엔드포인트 자체는 정상.

**조치 (2c2a7748, 당일 운영 승격·반영 실측)**:
1. 동의 상태 조회: 401 등 비정상 응답은 **판단 보류**(기존 상태·낙관 캐시 유지) — 정상 200 응답만 동의 상태 정본으로 반영 (네트워크 catch 경로와 동일 대우).
2. 동의 POST 실패 시 **에러 토스트 노출** — `useApiErrorMessage` 카탈로그 해석(401→"로그인이 필요합니다") + 폴백 `lbs.consentFail`(ko+en json + 운영·스테이징 DB upsert).
