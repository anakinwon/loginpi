...
| R-02 | **오픈 프로모 자동 종료(2026-07-31 KST) 후 과금 복귀 미검증** — `fn_is_open_promo_active()` FALSE 전환 시 7개 청구경로 정상 과금·표시 전환 여부 | 중 | 🟠 높음 (과금 오류=신뢰 타격) | 종료 전 staging에서 `promo_end_dtm` 단축 시뮬레이션·종료 7일 전 사용자 공지(배너 D-day 기구현)·법무 갭 G-6 연계 | ⏳ 7/24까지 사전 검증 필요 |
