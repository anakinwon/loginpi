...
| R-02 | **오픈 프로모 자동 종료 후 과금 복귀** — `fn_is_open_promo_active()` FALSE 전환 시 7개 청구경로 정상 과금·표시 전환 여부 | 저 (게이트 검증 후) | 🟠 높음 (과금 오류=신뢰 타격) | ✅**2026-07-09 게이트 검증 완료**: staging 시뮬레이션(end_dtm 과거 설정→fn·`v_promo_fee_current` 모두 false→원복 확인)+staging은 6/30부로 이미 종료 상태 실증(자연 실측). `applyPromoGate`는 RPC 오류 시에도 정상 과금 폴백(fail-safe). ⚠️**운영 종료시각 실측=2026-12-31 23:59 KST**(홈 SSR `openPromoEndDtm` 실측 2026-12-31T14:59Z) — 문서상 "7/31"과 불일치(연장 경위 마스터 확인 필요). 잔여=종료 7일 전 사용자 공지(12월 하순) | ✅ 게이트 검증 완료·⏳ 12월 공지 |
