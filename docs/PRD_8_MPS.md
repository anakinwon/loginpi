...
- **설명**: 주문은 명확한 상태 머신을 따르며, 각 상태 전이는 권한 있는 당사자만 수행한다. **거래 완료는 판매자·구매자 양방향 확인이 필수**다.
- **우선순위**: P0
- **사용자 스토리**: As a Buyer or Seller, I want to track the order status in real-time, and both parties must confirm the deal is done before Pi is released.
- **수락 기준**:
  - [ ] 상태 전이 규칙 (에스크로 상세 설계 §5 참조):
    - `PENDING` → `ESCROW` (Pi 결제 완료 자동)
    - `ESCROW` → `TRADING` (판매자가 "거래 시작" 확인)
    - `TRADING` → `SELLER_DONE` (판매자가 "물건 전달 완료" 확인)
    - `SELLER_DONE` → `DONE` (구매자가 "물건 수령 완료" 확인 → Pi 판매자 정산)
    - `SELLER_DONE` → `DONE` (자동, 구매자 미확인 N일 경과 시 — 판매자 보호)
    - `PENDING/ESCROW/TRADING` → `CANCELLED` (구매자·판매자·관리자 취소)
    - `SELLER_DONE` → `CANCELLED` (구매자·관리자만 취소 가능 — 판매자는 불가)
  - [ ] 각 상태 변경 시 `mps_txn_hist`에 이력 기록
  - [ ] 주문 상세 화면에서 현재 상태·변경 이력 타임라인 표시
  - [ ] 비당사자(구매자·판매자 아닌 자)의 주문 상세 접근 시 403 반환
  - [ ] 자동 완료 타임아웃(N일) 값은 서버 환경변수로 관리 (기본 3일, → §12 미결 사항 #8)
...
- **수락 기준**:
  - [ ] `PENDING`, `ESCROW`, `TRADING`, `SELLER_DONE` 상태에서 취소 가능 (`DONE` 취소 불가)
  - [ ] `SELLER_DONE` 상태에서는 구매자·관리자만 취소 가능 (판매자 취소 불가 — 이미 물건 전달 선언)
  - [ ] 취소 시 취소 사유(텍스트) 필수 입력
  - [ ] 취소 요청자(`cancel_req_id`)가 수수료 부담 — 수수료율 TBD (→ §12 미결 사항)
  - [ ] `ESCROW` 이후 취소 시: 구매자에게 `(결제금액 - 수수료)` Pi 환불, 수수료는 운영자 계정 전송
  - [ ] 취소 완료 시 `ordered_qty -= 1`, `stock_qty += 1` 원자적 복원
  - [ ] 취소 이력 `mps_txn_hist`에 `REFUND`, `FEE` 타입으로 각각 기록
  - [ ] 관리자는 모든 주문 강제 취소 가능

---

### FR-11: 양방향 거래 완료 확인 후 판매자 Pi Coin 전송

- **설명**: **판매자와 구매자 양쪽이 모두 OK를 확인해야** 에스크로 Pi가 판매자에게 전송된다. 어느 한쪽만 확인해서는 `DONE`으로 전환되지 않는다.
- **우선순위**: P0
- **사용자 스토리**: As a Platform, I want to release escrowed Pi to the seller only after BOTH the seller marks delivery and the buyer confirms receipt, so that neither party can be cheated.
- **수락 기준**:
  - [ ] **1단계**: 판매자가 "물건 전달 완료" 확인 → `TRADING → SELLER_DONE` 전환
  - [ ] **2단계**: 구매자가 "물건 수령 완료" 확인 → `SELLER_DONE → DONE` 전환 → Pi 정산 트리거
  - [ ] 두 단계가 모두 완료된 시점에만 Pi 정산 수행 (단계 순서 강제 — 구매자가 먼저 확인 불가)
  - [ ] `DONE` 전환 시: 운영자 에스크로 Pi 계정 → 판매자 Pi 지갑으로 `(결제금액 - 플랫폼 수수료)` 전송
  - [ ] `release_txid` 컬럼에 전송 트랜잭션 ID 저장
  - [ ] 전송 완료 시 `mps_txn_hist`에 `RELEASE_OUT` 이력 기록
  - [ ] 판매자가 `SELLER_DONE`으로 전환한 시점부터 N일 내 구매자 미확인 시 자동 `DONE` 전환 (cron — 판매자 보호 장치)
  - [ ] 자동 전환 시 `mps_txn_hist`에 `AUTO_RELEASE` 타입으로 기록
  - [ ] 전송 실패 시 관리자 알림 + 수동 처리 플로우 안내
...
stateDiagram-v2
  ...
  PENDING --> CANCELLED : 결제 타임아웃 / 구매자 취소

  ESCROW --> TRADING : 판매자 "거래 시작" 확인
  ESCROW --> CANCELLED : 구매자·판매자·Admin 취소\n→ 구매자 Pi 전액 환불

  TRADING --> SELLER_DONE : ① 판매자 "물건 전달 완료" 확인
  TRADING --> CANCELLED : 구매자·판매자·Admin 취소\n→ 구매자 Pi 환불 (수수료 차감)

  SELLER_DONE --> DONE : ② 구매자 "물건 수령 완료" 확인\n→ 판매자 Pi 정산 실행
  SELLER_DONE --> DONE : ② (자동) N일 경과 시 구매자 미확인\n→ 판매자 Pi 자동 정산
...
> **핵심 원칙**: ① 판매자 전달 확인 → ② 구매자 수령 확인, **두 단계가 모두 완료된 시점에만** 에스크로 Pi가 판매자에게 전송된다.
...
| 상황 | 수수료율 | 부담자 |
|------|---------|--------|
| 정상 거래 완료 | TBD % | 판매자 (정산액에서 차감) |
| 구매자 취소 | TBD % | 구매자 (환불액에서 차감) |
| 판매자 취소 | TBD % | 판매자 (별도 청구 또는 향후 정산 차감) |
| 관리자 강제 취소 | 0% | 없음 (분쟁 처리) |

> ⚠️ 수수료율 미결 → §12 Open Issues #1 참조
...
| 화면ID | 화면명 | 경로 | 접근 권한 |
|--------|--------|------|-----------|
| SCR-01 | 상품 목록·검색 | `/[locale]/Shop` | 모든 사용자 (Guest 포함) |
| SCR-02 | 상품 상세 | `/[locale]/Shop/[itemId]` | 모든 사용자 (Guest 포함) |
| SCR-03 | 내 상품 관리 | `/[locale]/Shop/my/items` | 로그인 필요 (클라이언트 게이트) |
| SCR-04 | 상품 등록·수정 | `/[locale]/Shop/my/items/new`<br>`/[locale]/Shop/my/items/[itemId]/edit` | 로그인 필요 |
| SCR-05 | 주문 관리 (판매자) | `/[locale]/Shop/my/sales` | 로그인 필요 |
| SCR-06 | 주문 관리 (구매자) | `/[locale]/Shop/my/orders` | 로그인 필요 |
| SCR-07 | 거래 내역 | `/[locale]/Shop/my/history` | 로그인 필요 |
| SCR-08 | 매장 등록·관리 | `/[locale]/Shop/my/shops`<br>`/[locale]/Shop/my/shops/new` | 로그인 필요 |

> **클라이언트 게이트 패턴**: 로그인 필요 화면에서 `getSessionUser()` null 시 `redirect()` 금지,
> `<ClientShopGate />` 컴포넌트 렌더 → Pi Browser 대응.
...
| | `CLOSED` | 게시중단 |
| | `SOLD` | 판매완료 (재고 소진 자동) |
| `order_st_cd` | `PENDING` | 주문접수 |
| | `ESCROW` | 에스크로완료 |
| | `TRADING` | 거래중 |
| | `SELLER_DONE` | 판매자 전달완료 확인 (구매자 수령 확인 대기) |
| | `DONE` | 거래완료 (양방향 확인 → Pi 정산) |
| | `CANCELLED` | 취소 |
...
| Method | Path | 설명 | 인증 |
|--------|------|------|------|
| GET | `/api/Shop/items` | 상품 목록 조회 (검색·필터·페이지) | 불필요 |
| POST | `/api/Shop/items` | 상품 등록 | 필요 |
| GET | `/api/Shop/items/[itemId]` | 상품 상세 조회 | 불필요 |
| PATCH | `/api/Shop/items/[itemId]` | 상품 수정 (본인만) | 필요 |
| DELETE | `/api/Shop/items/[itemId]` | 상품 논리삭제 (본인만) | 필요 |
| GET | `/api/Shop/shops` | 내 매장 목록 조회 | 필요 |
| POST | `/api/Shop/shops` | 매장 등록 | 필요 |
| PATCH | `/api/Shop/shops/[shopId]` | 매장 수정 (본인만) | 필요 |
| DELETE | `/api/Shop/shops/[shopId]` | 매장 논리삭제 (본인만) | 필요 |
| POST | `/api/Shop/orders` | 주문 생성 (재고 원자적 차감) | 필요 |
| GET | `/api/Shop/orders/[orderId]` | 주문 상세 (당사자만) | 필요 |
| PATCH | `/api/Shop/orders/[orderId]/cancel` | 주문 취소 (당사자 + 관리자) | 필요 |
| POST | `/api/Shop/orders/[orderId]/seller-done` | ① 판매자 "물건 전달 완료" 확인 (TRADING→SELLER_DONE) | 필요 |
| POST | `/api/Shop/orders/[orderId]/buyer-done` | ② 구매자 "물건 수령 완료" 확인 (SELLER_DONE→DONE) | 필요 |
| GET | `/api/Shop/my/history` | 거래 내역 조회 | 필요 |
| POST | `/api/Shop/payments/approve` | Pi 결제 승인 콜백 (Pi SDK) | 필요 |
| POST | `/api/Shop/payments/complete` | Pi 결제 완료 콜백 (Pi SDK) | 필요 |
...
| # | 미결 사항 | 우선순위 | 담당 |
|---|-----------|---------|------|
| 1 | **수수료율 결정** — 정상 거래·구매자 취소·판매자 취소 각각의 수수료율 (TBD) | 높음 | 비즈니스 |
