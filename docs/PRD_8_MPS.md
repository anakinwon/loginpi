# PRD_8_MPS.md — PyShop™(MPS): Pi Coin P2P·O2O 마켓플레이스
...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| v2.1 | 2026-06-17 | **오프라인매장 카트(다중상품 주문) + 매장 스토어프론트 — FR-14·15 신규** — ① **FR-14 카트**: 오프라인매장 상품을 카트에 담아 단일 Pi 결제로 다중상품 1주문(매장 단위·라인별 원자 재고차감·전체 롤백, 게스트 담기→체크아웃 게이트). P2P 중고직거래는 단건 에스크로 유지. ② **FR-15 스토어프론트**: 매장별 상품 모아보기 `/store/shop/[shopId]`(카트의 무대). ③ 스키마 **`mps_order_item`**(주문 라인, §7-5-1) 신설 + `mps_order` header 확장(`shop_id`·`order_price_pi`=합계). ④ **SCR-09 장바구니·SCR-10 매장 스토어프론트**, §8 API(`shops/[shopId]/items`·`orders` 확장). 자국통화 합계는 등록시점 고정 참고가(레드라인 준수). | anakin |
| v2.0 | 2026-06-12 | **결제 즉시 거래중 + 2단계 확인으로 단순화** — ① 결제 완료 시 `PENDING → TRADING`(거래중) 직행, `ESCROW`·`SELLER_DONE`은 레거시(신규 전환 없음). "판매자 전달 완료" 단계·`/confirm` 라우트 제거 → **결제(거래중) → 구매자 수령 → 판매자 거래완료** 2단계 확인 ② 상품 목록·상세·내 상품 관리에 **거래 상태 배지**(판매중/거래중/판매완료, `deriveTradeStatus`) + 진행 중 주문 수(`trading_cnt`) 표기, 공개 목록에 `SOLD` 노출 ③ 구매 버튼은 `OPEN` && 재고(거래중 차감 후) > 0일 때만 활성화. FR-08·09·10·11, §5-1, §7-7, §8 현행화 | anakin |
| v1.9 | 2026-06-11 | **보증금 기능 베타 구현·적용** — `mps_seller_bond` 테이블 + `fn_mps_bond_deposit`(원자적 예치) + `fn_mps_order_cancel` 수수료 확장(보증금 활성 거래의 에스크로 이후 취소 시 0.1π). 예치 결제 `MPS_BOND` 분기, 보증금 카드 UI(내 상품 관리), 상품 상세 보증금 거래/자유 취소 배지·공시 문구. 반환 기능은 §12 #9 자문 전 미제공 | anakin |
| v1.8 | 2026-06-11 | **지급준비금 귀속 정의** — 보증금 1π 환불 요구 시 **0.9π만 환불**(수수료 미사용분 한도), 지급준비금 0.1π는 **플랫폼 운영유지 비용 귀속** (v1.7 전액 반환 원칙 대체). 예치 전 공시·명시적 동의 의무, §5-6 0π 원칙과 항목 분리(거래 수수료 아닌 1회성 운영비용 분담금). **공시 적법성 법적 자문 §12 #9 신설 — 자문 완료 전 시행 금지** | anakin |
...
**PyShop™ (MPS)** — Pi Coin 전용 P2P·O2O 마켓플레이스
...
Pi Network는 수천만 명의 KYC 기반 사용자를 보유하지만, Pi Coin을 실질적으로 소비할 수 있는 생태계가 부족하다. PyShop™는 Pi 사용자 간 P2P 중고·신품·수제품 거래를 가능하게 하여 **Pi Coin 소비 생태계의 핵심 인프라**를 구축한다.
...
- **수락 기준**:
  ...
  - [x] `/api/payments/complete` 콜백(`MPS_ESCROW` 분기)에서 주문 상태 `PENDING` → `TRADING`(거래중) 전환 _(v2.0)_
  - [x] `/api/payments/complete` 콜백에서 `pi_pymnt` 테이블에 기록 + `escrow_txid` 저장
...
### FR-09: 주문 상태 관리 _(v2.0 개정 — 결제 즉시 거래중, 2단계 확인)_

- **설명**: 주문은 명확한 상태 머신을 따르며, 각 상태 전이는 권한 있는 당사자만 수행한다. 결제 완료 즉시 **거래중(`TRADING`)** 으로 진입하고, **① 구매자 수령 확인 → ② 판매자 최종 거래완료 확인**의 2단계를 거쳐 종결된다. 정산은 ② 시점에 트리거된다.
- **우선순위**: P0
- **판매전략 (전체 흐름)**:
  ...
  | 1 | 임시등록 | 판매자 | (상품 `DRAFT`) |
  | 2 | 판매중 | 판매자 | (상품 `OPEN`) |
  | 3 | 거래중 — 결제 (구매자 → 에스크로) | 구매자 | `PENDING` → `TRADING` |
  | 4 | 주문자 취소 (보증금 거래 시 0.1π 수수료) | 구매자 | `TRADING` → `CANCELLED` (상품 재 `OPEN`) |
  | 5 | 판매자 취소 (보증금 거래 시 0.1π 페널티) | 판매자 | `TRADING` → `CANCELLED` (상품 재 `OPEN`) |
  | 6 | 상품 수취 | 구매자 | `TRADING` → `BUYER_DONE` |
  | 7 | 거래 완료 (에스크로 → 판매자, 정산 트리거) | 판매자 | `BUYER_DONE` → `DONE` |

- **사용자 스토리**: As a Buyer or Seller, I want to track the order status in real-time, and the deal closes only after receipt and the seller's final confirmation.
- **수락 기준**:
  - [x] 상태 전이 규칙 (에스크로 상세 설계 §5 참조):
    - `PENDING` → `TRADING` (Pi 결제 완료 자동 — `/api/payments/complete`의 `MPS_ESCROW` 분기)
    - `TRADING` → `BUYER_DONE` (① 구매자 "물건 수령 완료" 확인)
    - `BUYER_DONE` → `DONE` (② 판매자 "거래 완료" 확인 → Pi 판매자 정산 트리거)
    - `PENDING/TRADING` → `CANCELLED` (구매자·판매자·관리자 취소)
    - `BUYER_DONE` → `CANCELLED` (**관리자만** — 수령 확인 후 취소는 분쟁 처리)
    - _(레거시)_ `ESCROW`·`SELLER_DONE`은 v1.x 주문 호환용 — 신규 전환 없음, 화면 표시·수령 확인·취소는 `TRADING`과 동일하게 처리 (`SELLER_DONE` 취소는 구매자·관리자만)
  ...
  - [x] 상품 목록·상세·내 상품 관리에 **거래 상태 배지** 표기 _(v2.0)_: 판매중(`OPEN`+재고>0) / 거래중(재고 소진+진행 주문 존재) / 판매완료(재고 소진+전 거래 종결) — `deriveTradeStatus()` 단일 헬퍼, 진행 중 주문 수(`trading_cnt`) 병기
  - [x] 구매 버튼 활성화 조건 _(v2.0)_: 상품 `OPEN` && 거래중 수량을 차감한 재고(`stock_qty`) > 0 — `CLOSED`·`SOLD`·재고 0은 비활성
  - [ ] 주문 상세 화면에서 현재 상태·변경 이력 타임라인 표시 (후속)
  - [ ] 자동 전환 타임아웃: `TRADING`·`BUYER_DONE`에서 상대방 미확인 N일 경과 시 자동 다음 단계 전환 cron (기본 3일, → §12 미결 사항 #8, 후속)
...
#### 취소수수료 정책 (2026-06-11 확정 · v1.9 베타 적용)

> **베타 적용 (v1.9)**: 보증금 예치 기능을 베타에서 즉시 구현·적용한다 (`sql/031_mps_seller_bond.sql`).
> - **보증금 활성 판매자의 거래**: 에스크로 이후 취소 시 취소 요청자가 0.1π 수수료 부담 (FR-10 단서 그대로)
> - **무보증금 거래**: 양측 모두 취소수수료 0π — 취소 이력(`cancel_req_id`·`cancel_reason`)만 기록
> - 보증금 반환(환불) 기능은 §12 #9 법적 자문 완료 전까지 미제공 (UI 안내 표기)
...
#### 지급준비금의 귀속 (v1.8 수립 — 시행 전 법적 자문 필수)

> **환불 원칙: 보증금 1π 입금 후 환불을 요구하더라도 환불 한도는 0.9π(수수료 미사용분)이다.**
> **지급준비금 0.1π는 반환되지 않으며, 플랫폼 운영유지 비용으로 귀속된다.**

- 환불액 산식: `환불액 = 0.9π - (누적 취소 횟수 × 0.1π)` — 지급준비금 0.1π는 환불 대상에서 제외
- 귀속 시점: 보증금 예치 시점에 확정 (환불 여부와 무관)
- **사전 공시 의무**: 보증금 예치 화면에 "1π 중 0.1π는 지급준비금으로 플랫폼 운영유지 비용에
  귀속되며 환불되지 않습니다"를 예치 **전에** 명시하고 판매자의 명시적 동의를 받는다.
- **§5-6 "플랫폼 수령 수수료 = 0π" 원칙과의 관계**: 0π 원칙은 **거래·취소 수수료**에 적용된다.
  지급준비금은 거래 수수료가 아닌 **보증금 옵션 가입 시 1회성 운영비용 분담금**으로 항목을 분리해
  고지한다 — 두 정책이 충돌하지 않도록 UI·약관에서 명확히 구분할 것.
- ⚠️ **법적 자문 (시행 전 필수, → §12 미결 #9)**: "지급준비금의 플랫폼 운영유지 비용 귀속"을
  공시하는 방식이 문제의 소지가 없는지 법적 자문을 구한다.
  - 검토 관점: 전자상거래법·약관규제법상 환불 제한 조항의 유효성, 선불식 예치금 관련 규제,
    "지급준비금" 용어 사용의 적절성 (은행법상 용어와의 혼동 가능성)
  - 자문 결과에 따라 귀속 정책·공시 문구·용어를 조정할 수 있다

- **수락 기준**:
  - [x] `PENDING`, `TRADING`(레거시 `ESCROW` 포함) 상태: 구매자·판매자·관리자 취소 가능 (`DONE` 취소 불가)
  - [x] _(레거시)_ `SELLER_DONE` 상태에서는 구매자·관리자만 취소 가능 (판매자 취소 불가 — 이미 물건 전달 선언)
...
### FR-11: 2단계 거래 완료 확인 후 판매자 Pi Coin 전송 _(v2.0 개정)_

- **설명**: 에스크로 Pi는 **②단계(판매자 거래완료 확인)가 끝나야만** 판매자에게 정산된다. 구매자 수령 확인(①)만으로는 정산되지 않는다 — 정산 전 마지막 점검권을 판매자가 가진다. _(v1.x의 "판매자 전달 완료" 선행 단계는 v2.0에서 제거 — 결제 즉시 거래중)_
- **우선순위**: P0
- **사용자 스토리**: As a Platform, I want to release escrowed Pi only after the buyer confirms receipt AND the seller finally closes the deal, so that settlement never fires by a single party's mistake.
- **수락 기준**:
  - [x] **①단계**: 구매자가 "물건 수령 완료" 확인 → `TRADING → BUYER_DONE` 전환 (`POST /api/store/orders/[orderId]/release`)
  - [x] **②단계**: 판매자가 "거래 완료" 확인 → `BUYER_DONE → DONE` 전환 → Pi 정산 트리거 (`POST .../complete`)
  - [x] 두 단계가 모두 완료된 시점에만 Pi 정산 수행 (단계 순서 강제 — 건너뛰기 불가, 상태 가드 UPDATE)
  - [x] _(v2.0)_ `/api/store/orders/[orderId]/confirm`("판매자 전달 완료") 라우트·`markSellerDone` 제거
  ...
  - [ ] `TRADING`·`BUYER_DONE`에서 상대방 미확인 N일 경과 시 자동 다음 단계 전환 (cron — 양측 보호 장치, 후속)
...
### FR-14: 오프라인매장 장바구니(카트) 다중 상품 주문 _(v2.1 신규)_

- **설명**: 오프라인매장 상품은 카트에 여러 개를 담아 한 번에 주문·결제한다. 카페에서 음료+디저트를 함께 주문하는 실물 매장 UX를 온라인에 재현.
- **우선순위**: P1
- **사용자 스토리**: As a Buyer at an offline shop, I want to add multiple items to a cart and check out together, so that I pay once for a combined order.
- **적용 범위**:
  - ✅ 오프라인매장(`shop_id` 보유) 상품만 카트 대상
  - ❌ 중고직거래(P2P, `shop_id` 없음)는 **단건 에스크로 유지** — 중고품은 1점 단위라 카트 부적합
- **수락 기준**:
  - [ ] 카트는 **매장(shop_id) 단위** — 한 카트엔 동일 매장 상품만. 다른 매장 상품은 매장별 카트로 분리(또는 분리 안내)
  - [ ] 라인별 수량 조절(재고 범위 내), 라인 삭제, 카트 비우기
  - [ ] 카트 합계 = Σ(라인 단가 `price_pi` × 수량). **등록시점 자국통화 합계 참고가 병기**(고정값, 실시간 틱커 아님 — 레드라인 준수)
  - [ ] 체크아웃: 카트 → **1개 주문(`mps_order` header) + N개 주문라인(`mps_order_item`)**, **단일 Pi 결제**로 합계 에스크로 송금(`metadata.type='MPS_ESCROW'`)
  - [ ] 재고: 라인별 **원자적 검증·차감** — 한 라인이라도 재고 부족 시 **전체 주문 롤백**(부분 성공 금지)
  - [ ] 주문 상태머신(오프라인 O2O: 주문중→준비중→상품대기중→10분 자동 판매완료 / 또는 FR-09 에스크로 흐름)은 **주문 단위 1개 라이프사이클**
  - [ ] 취소/환불은 **주문 단위(전체)** — 라인 단위 부분취소는 v2 후속
  - [ ] 게스트 카트 담기 허용, **체크아웃 시 로그인 게이트**(Pi Browser `redirect` 금지·클라이언트 게이트)
- **데이터/구현 메모**: `mps_order`를 주문 header로 확장(`shop_id` 보유, `order_price_pi`=카트 합계), 라인은 신규 `mps_order_item`(§7-5-1). P2P 단건 주문은 `item_id` 직접(라인 미사용) 또는 라인 1개로 호환. 카트 자체는 초기 **클라이언트(localStorage) 보관** — 서버 영속 카트(`mps_cart`)는 후속 옵션.

---

### FR-15: 오프라인매장 스토어프론트 (매장별 상품 모아보기) _(v2.1 신규)_

- **설명**: 특정 오프라인매장의 상품을 한 화면에 모아 보여주는 **매장 전용 storefront**. 카트(매장 단위)의 자연스러운 무대 — 한 매장에서 여러 상품을 담아 결제하는 동선을 제공한다.
- **우선순위**: P1
- **사용자 스토리**: As a Buyer, I want a dedicated page per offline shop listing all its items, so that I can browse and add several products from that shop into one cart.
- **수락 기준**:
  - [ ] 경로 `/[locale]/store/shop/[shopId]` — 매장 정보(이름·인증배지·영업시간·위치) + 해당 매장 `OPEN` 상품 그리드
  - [ ] 게스트 포함 공개 접근. 상품별 "카트 담기" 버튼(FR-14)
  - [ ] 매장 카트 요약(담긴 수량·Pi 합계·자국통화 합계 참고가) + "주문하기"(체크아웃)
  - [ ] 진입 동선: 매장 관리(SCR-08)·지도 InfoWindow·상품 상세 → 해당 storefront
  - [ ] 자국통화 표시는 등록시점 고정 참고가(레드라인 준수 — 실시간 환율 노출 금지)
...
stateDiagram-v2
  ...
  PENDING --> TRADING : Pi SDK 결제 완료 = 거래중\n(complete 콜백 MPS_ESCROW 분기)
  PENDING --> CANCELLED : 결제 취소/실패 → 재고 복원

  TRADING --> BUYER_DONE : ① 구매자 "물건 수령 완료" 확인
  TRADING --> CANCELLED : 구매자·판매자·Admin 취소\n→ 재고 복원 (보증금 거래 시 0.1π 수수료)

  BUYER_DONE --> DONE : ② 판매자 "거래 완료" 확인\n→ 판매자 Pi 정산 트리거 (에스크로 → 판매자)
...
> **핵심 원칙 (v2.0)**: 결제 완료 즉시 **거래중(`TRADING`)**. ① 구매자 수령 확인 → ② 판매자 거래완료 확인, **두 단계가 모두 완료된 시점에만** 에스크로 Pi가 판매자에게 전송된다. 정산 직전 마지막 점검권은 판매자에게 있다.
>
> _(레거시)_ v1.x에서 생성된 `ESCROW`·`SELLER_DONE` 주문은 신규 전환 없이 화면 표시·수령 확인·취소 모두 `TRADING`과 동일하게 처리된다.
...
| SCR-09 | 장바구니(카트) — 오프라인매장 (FR-14) | `/[locale]/store/cart` | 모든 사용자 (체크아웃 시 로그인) | 🔜 v2.1 신규 |
| SCR-10 | 매장 스토어프론트 — 매장별 상품 모아보기 (FR-15) | `/[locale]/store/shop/[shopId]` | 모든 사용자 (Guest 포함) | 🔜 v2.1 신규 |
...
> **주제영역 접두사**: `mps_` (MPS = PyShop™ 마켓플레이스 신규 등록)
...
### 7-5-1. mps_order_item (주문 라인 — 카트 다중상품 / FR-14) _(v2.1 신규)_

```sql
-- mps_order(header) : mps_order_item(line) = 1 : N
-- 오프라인매장 카트 주문의 라인 항목. P2P 단건은 header item_id 직접(호환).
CREATE TABLE mps_order_item (
  order_item_id  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id       UUID NOT NULL REFERENCES mps_order(order_id),
  item_id        UUID NOT NULL REFERENCES mps_item(item_id),
  qty            INT  NOT NULL DEFAULT 1 CHECK (qty >= 1),   -- 주문 수량(재고 범위 내)
  unit_price_pi  NUMERIC(18,7) NOT NULL,                     -- 주문 시점 단가 스냅샷(정본)
  ccy_cd         VARCHAR(3),                                 -- 등록시점 통화 스냅샷(ISO 4217)
  ccy_amt        NUMERIC(18,2),                              -- 등록시점 자국통화 단가 스냅샷
  del_yn         CHAR(1) NOT NULL DEFAULT 'N',
  del_dtm        TIMESTAMPTZ,
  regr_id        TEXT NOT NULL DEFAULT 'ADMIN',
  reg_dtm        TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_mps_order_item_order ON mps_order_item(order_id);
CREATE INDEX idx_mps_order_item_item  ON mps_order_item(item_id);
```

> **mps_order header 확장(FR-14)**: `shop_id UUID REFERENCES mps_shop(shop_id)` 추가(오프라인매장 주문 식별), `order_price_pi`는 **카트 합계**. 단일상품(P2P) 주문은 라인 없이 `item_id` 직접 사용 — 호환 유지. 라인 단위 자국통화 스냅샷으로 §062 정책(등록시점 고정 참고가) 일관.
...
| | `TRADING` | 거래중 (Pi 결제 완료 = 에스크로 입금 — v2.0 기본 진입 상태) |
| | `BUYER_DONE` | ① 구매자 수령완료 확인 (판매자 거래종결 대기) |
| | `DONE` | ② 거래완료 (판매자 최종 확인 → Pi 정산) |
| | `CANCELLED` | 취소 |
| | `ESCROW` | _(레거시)_ v1.x 에스크로완료 — 신규 전환 없음, `TRADING`과 동일 처리 |
| | `SELLER_DONE` | _(레거시)_ v1.x 판매자 전달완료 — 신규 전환 없음, `TRADING`과 동일 처리 |
...
|--------|------|------|------|------|
| GET | `/api/store/items` | 상품 목록 조회 (검색·필터·정렬·페이지, `OPEN`·`SOLD` 노출 + `trading_cnt`) / `?mine=1` 내 상품 | 불필요 / mine은 필요 | ✅ |
...
| POST | `/api/store/orders/[orderId]/release` | ① 구매자 "물건 수령 완료" (TRADING→BUYER_DONE) | 필요 | ✅ |
| POST | `/api/store/orders/[orderId]/complete` | ② 판매자 "거래 완료" (BUYER_DONE→DONE, 정산 트리거) | 필요 | ✅ |
| POST | ~~`/api/store/orders/[orderId]/confirm`~~ | _(v2.0 제거)_ 판매자 "물건 전달 완료" 단계 폐지 | — | ❌ |
| POST | `/api/payments/approve` | Pi 결제 승인 콜백 (공용) | Pi SDK | ✅ |
| POST | `/api/payments/complete` | Pi 결제 완료 콜백 (공용 — `MPS_ESCROW` 분기: PENDING→TRADING + ESCROW_IN 이력 + 금액 재검증) | Pi SDK | ✅ |
| GET | `/api/store/shops` 외 매장 CRUD | 매장 등록·수정·삭제 (lib 구현 완료, 라우트는 Phase 2) | 필요 | 🔜 |
| GET | `/api/store/shops/[shopId]/items` | 매장 스토어프론트 상품 목록 (공개, FR-15) | 불필요 | 🔜 v2.1 |
| POST | `/api/store/orders` _(확장)_ | 카트 다중라인 주문 — `items[]`(item_id·qty) 수신, 라인별 원자 재고차감(전체 롤백) + 합계 단일 결제 (FR-14) | 필요 | 🔜 v2.1 |
...
| 6 | **PiRC3 마이그레이션 전략** — 기존 OPEN 주문·에스크로 상태의 마이그레이션 방안. **🔒 보류(2026-06-13 공식 확인)**: PiNetwork/PiRC 저장소에 PiRC1·PiRC2만 존재(PiRC3·에스크로 컨트랙트 없음), Pi SDK는 `authenticate`+`createPayment`만 제공(`invokeContract` 미지원), 스마트 컨트랙트 RPC는 Testnet 한정. → **현재의 플랫폼(운영자 Pi 계정) 가상 에스크로를 공식 지원 확인 시점까지 유지(정식 방식으로 간주)**. 트리거: ⓐ PiRC 저장소 에스크로 컨트랙트 공개 AND ⓑ Pi SDK 컨트랙트 호출 메서드 제공 — 둘 다 충족 시 재검토 | 중간 | 아키텍처 |
| 7 | **조회수 중복 방지** — `view_cnt` 세션 단위 중복 방지 방안 (Redis TTL vs. DB 세션 테이블) | 낮음 | 개발 |
| 8 | **상대방 미확인 자동 완료 타임아웃** — `TRADING` 진입 후 N일 내 구매자 수령 미확인, 또는 `BUYER_DONE` 진입 후 N일 내 판매자 거래완료 미확인 시 자동 다음 단계 전환 (기본값 3일 제안, cron 주기·알림 방식 결정 필요) | 높음 | 개발·기획 |
| 9 | **지급준비금 귀속 공시의 법적 자문** — "보증금 1π 중 0.1π는 환불되지 않고 플랫폼 운영유지 비용 귀속" 공시의 적법성 검토 (전자상거래법·약관규제법상 환불 제한 유효성, 선불식 예치금 규제, '지급준비금' 용어 적절성). **자문 완료 전 시행 금지** (FR-10 v1.8) | **높음** | 법무·비즈니스 |
