...
**구현 6종**: ① `sys_site_usr_map`에 `site_nick_nm` 등 페르소나 컬럼 — 새 사이트 첫 진입 시 전역 닉네임 자동 노출 금지, 사용자가 "프로필 가져오기" 선택 시에만 공유 ② 활동 비공유 원칙 방침·화면 명문화 ③ **첫 로그인 1회 투명 고지**("계정·Bean 공유, 활동·프로필은 이 사이트에만") + 푸터 운영 주체 동일성 표시 ④ 마이페이지 "내 사이트" 대시보드 + 사이트 단위 연결 해제(논리삭제, 전체 탈퇴와 분리) ⑤ 제재 공유는 선량한 사용자 보호 명분으로 약관 고지 ⑥ 개인정보처리방침 v1.2 예약 — "동일 운영자 서비스군 내 통합 계정" 고지(제3자 제공 아닌 동일 처리자 내 이용).
