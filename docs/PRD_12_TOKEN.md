...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| **v1.9** | 2026-06-17 | **사용처 확장 동기화(§1-5)** — 백서 v0.2의 확장 사용처를 PRD §1-5 유틸리티 목록에 반영: 사용권에 동시통역(PyTranslate™)·마켓 구매/판매 수수료, 보상에 이용후기(리뷰)·매출 쿠폰/스탬프, 후원에 도네이션 사회 기부 추가. 전체 9종 표는 백서 §4 참조(DRY). 분배 비율 영향 없음(리저브 지출 대상만 확대). | asoká |
...
- **1차 목적 = 유틸리티 토큰(Utility Token)**: BEAN은 cafe.pi 생태계 내 **사용권·보상·후원·결제 수단**으로 규정. (전체 사용처 9종 표: `PRD_12_TOKEN_백서.md` §4)
  - **사용권**: 카페/MPS/보이스챗 기능 이용·할인 · 글로벌 동시통역(PyTranslate™) 이용 · 마켓 구매/판매 수수료 결제
...
- **카페(PyChat)**: 사용자들이 실시간으로 소통·콘텐츠 공유
- **마켓플레이스(PyShop™/MPS)**: 판매자와 구매자가 Pi를 사용하여 거래
- **보이스챗(PyVoice)**: 음성 채팅을 통한 깊이 있는 커뮤니티 형성
...
| 사용처 | 통화 유형 | 플로우 |
|--------|---------|-------|
| **플랫폼 구독료** (PyCafé™/PyShop™) | Bean 정본 | 사용자 → 플랫폼 (Pi 충전 → Bean 결제) |
...
VALUES
  ('PISHOP_SELLER_MONTHLY', 'PyShop™ Seller 월간', 'PyShop™ Seller 월간 — 전문가 판매자용', 'PISHOP', '0.5000', 1, 'Y', 'N', 'ADMIN', CURRENT_TIMESTAMP),
  ('PISHOP_SELLER_ANNUAL', 'PyShop™ Seller 연간', 'PyShop™ Seller 연간 — 2개월 무료', 'PISHOP', '5.0000', 12, 'Y', 'N', 'ADMIN', CURRENT_TIMESTAMP);
...
- `PRD_4_CHAT.md` — 카페(PyChat) 요구사항
- `PRD_8_MPS.md` — 마켓플레이스(PyShop™) 요구사항
