...
### v0.4 추가 결정 (2026-06-29) — 후기 보상 "보증금 선행 게이트"

**규칙**: 후기 **작성 및 보상 지급**은 **매장이 보상 보증금을 예치(활성)한 경우에만** 허용. 보증금 없으면 후기 작성 자체 차단. **Bean/Pi 모드 무관 동일 규칙**(보증금·보상 통화만 모드별 1:100).

- **게이트 강화**: 기존 `fbck_consent_yn`(매장 동의)만으로 부족 → **동의 + 보증금 활성** = 후기 가능. `mps_seller_bond`의 FR-10("보증금 활성자만 거래") 패턴을 후기로 확장.
- **목적**: 보증금 = 후기 보상 재원의 **사전 담보** → ⓐ 재원 없는 과발행 구조적 차단 ⓑ 가짜 매장 어뷰징 방어(`fbck_system_design` 5층 방어를 재원 레벨로 강화).
