...
| 항목 | 결정 | 근거 |
|---|---|---|
| 참여자 모델 | **N:N 다:다 (1~4명)** — 1명도 가능(혼자 대기) | 1:1 MVP → N:N 확장 로드맵 단순화. 최소 1명부터 입장 가능(다른 참여자 join 대기). |
| 최대 동시 마이크 | **4명 제한** — 5명 이상은 청취 전용(mute 강제) | P2P 업스트림 대역 관리, 통화 품질 유지. |
| 방장 마이크 제어 | **OWNER/ADMIN만 원격 mute/unmute 가능** | 채팅방 거버넌스 일관성. 클라이언트 재연결/낙오 방지. |
