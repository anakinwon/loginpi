# PRD_9_VOICE_CHAT.md — PyVoice™: WebRTC 실시간 음성 통화
...
PyCafé 카페 참여자 간 **브라우저 기반 실시간 음성 통화**(1~4명 P2P 메시)를 추가한다. 별도 앱 설치 없이 Pi Browser·일반 브라우저에서 즉시 통화한다. 최소 1명부터 입장 가능 — 혼자 대기 중에도 다른 참여자 join 시 자동 연결된다.
