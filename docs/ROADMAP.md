...
> **등재 승인 = Pi가 이 앱의 A2U(앱→사용자 송금)를 개방하는 시점.** ⭐**감지 자동화 완료(2026-07-16)·하트비트 전환(2026-07-18)**: cron `a2u-probe`(매일 09:30 KST)가 A2U 생성 프로브(즉시 취소·송금 0)로 판정해 **매일 마스터 텔레그램으로 발송**한다 — 대기=📡 "승인 대기 D+N"·개방=🚨 긴급(0순위 런북)·이상=⚠️ 에러 원문. **알람이 하루라도 안 오면 그 자체가 감시 체계 이상 신호.** 알림(또는 승인 소식) 확인 즉시 다른 모든 작업보다 먼저 아래를 실행하고, **마스터에게 반드시 결과를 보고(알림)한다.** 완료 전까지 이 블록을 삭제하지 말 것.
...
| 버전 | 날짜 | 내용 |
|---|---|---|
| v13.7 | 2026-07-18 | **등재 알람 하트비트 전환**(a4378bda, 운영 검증 notified:true) — 미승인(CLOSED)도 매일 09:30 📡 "승인 대기 D+N" 알람·ERROR ⚠️ 원문 알람 승격(무통지 폐기)·OPEN 🚨 불변. "무소식=미승인 vs 알람 고장" 모호함 제거 — **알람 부재 자체가 이상 신호**. 마스터 수신 확인 완료 |
| v13.6 | 2026-07-17 | **오후 4트랙 — 홈 커스터마이징·근본수정 3건·종합 리뷰** ① **홈 개편**(마스터 지시, 운영 승격 4회): 글로벌 커뮤니티 히어로(3.2MB→395KB WebP·Ken Burns·글로우) + 골드 태그라인 "글로벌 Pi 커뮤니티 & Pi 마켓플레이스" + 그랜드오픈·기술백서·사용설명서 전 카드 접이식(기본 접힘, 세로폭 2/3) ② **i18n API lazy init 전수**(c0825fde): 모듈 스코프 createClient 5곳 → getSupabaseAdmin() — 승격마다 반복되던 loginpi preview 빌드 실패 노이즈 근본 소멸(전 컨텍스트 success 실측) ③ **매출 집계 catch-all 복원**(sql/183): "구매왕은 있는데 매출 0" — sql/043이 026 세분화 유실 회귀+신종 type(MPS_ESCROW·FBCK_BOND) 누락·배치 '성공' 은폐 → MPS_ESCROW=PRODUCT_ORDER 분류·미분류 총수용·ADMIN_REFUND 매출/구매왕 제외·백필(운영 157.8π/37건 복원 실측) ④ **문서**: 9영역 종합 리뷰(REVIEW_TOTAL)+차기 프로젝트 플레이북(PLAYBOOK_NEXT_PROJECT — 사고 회피 4질문·Day-1 체크리스트) + 병행 트랙(홈 대시보드 심사 절제 이원화 be392d62) |
