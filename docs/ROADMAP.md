...
> **등재 승인 = Pi가 이 앱의 A2U(앱→사용자 송금)를 개방하는 시점.** 승인 소식 확인 즉시 다른 모든 작업보다 먼저 아래를 실행하고, **마스터에게 반드시 결과를 보고(알림)한다.** 완료 전까지 이 블록을 삭제하지 말 것.
...
| 24 | 메인넷 전환 & 2단계 배포 인프라 | 🔶 **제출 직전 단계** | 기술 준비 완결 + **등재 실행 런북 확정(2026-07-10 등록팀 하네스 — 7단 순서 B-5→E-10·P0 실기기 12단계 시트·제출 필드 초안)**·기술부록 현행화(189 locale·Pi Sign-In)·절제 오버레이 보강(관리자 발행→지급 5키). 잔여=**마스터 포털 수작업 5건뿐**(코드 0건) (MAINNET_READINESS_CHECKLIST §런북) |
...
| 법무 | **법무 문서 4종 v1.1 동시 개정** (방침·환불·약관·커뮤니티) | ✅ 공지 완료 (2026-07-09) | 텔레그램/Gemini 제3자·kakao_id·건당 서비스 환불·후기 보상·판매보증금 조항 신설. 운영 공지 3건 게시(상단 고정)·**2026-07-16 promote로 동시 시행**. 잔여: T05 외부 대기·G-8/9 경미 (갭점검 정본) |
