...
| **24** | 메인넷 전환 & 2단계 배포 인프라 | 🚧 진행 중 (파이프라인 분리 완료·DB/메인넷 컷오버 대기) | **Pi Testnet→Mainnet 전환 물밑작업 + 배포 환경 분리.** ✅완료: ⓐ **Py 개명**(PyCafé™/PyShop™/PyTranslate™ — Pi 접두 상표 회피, C-A 공식답변 반영) ⓑ **2단계 배포 분리**(staging=loginpi/master/Testnet·🧪 / 운영=cafepi/`production` 브랜치 게이팅, 같은 repo 다중연결) + `scripts/promote-to-prod.mjs`(ff-only 승격) ⓒ **3-tier DB 라우터**(`src/lib/db-env.ts`·`getDbTierInfo`·🧪 StagingBanner, VERCEL_ENV/APP_TIER 판정·하위호환 폴백) ⓓ **운영 WAS 신설**(Vercel 운영 프로젝트, Google·Pi 로그인 정상화 — NEXT_PUBLIC_APP_URL 도메인별·Google Console 콜백 등록·GOOGLE_CLIENT_SECRET 재설정). 🔶진행: **운영DB 신설**=pg_dump 스키마 복제 확정(git replay는 genesis 누락으로 불가, `docs/PROD_DB_SETUP.md`). ⏳잔여(컷오버): 메인넷 Dev Portal 프로젝트·API Key·지갑·도메인검증·P0 실기기 검증·U2A 1건·등재 신청. 정본: `docs/MAINNET_READINESS_CHECKLIST.md`·`docs/DEPLOY_STRATEGY.md`·`docs/DEPLOY_NOTICE.md` · 체크리스트 `mainnet_checklist`(sql/129·132)·`ops_checklist`(sql/130·131·133) |
...
- **진행 중**: 4개 (Phase 20 화면 성능 최적화 · Phase 21 보안 강화 · Phase 23 실시간 모니터링 · **Phase 24 메인넷 전환 & 2단계 배포 — 파이프라인 분리·운영 WAS 완료, DB/메인넷 컷오버 대기 2026-06-28**)
