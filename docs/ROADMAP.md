...
- ✅ `src/components/admin/admin-sidebar.tsx` — `/admin`, `/admin/users` 네비게이션
...
- ✅ `/admin` 404 오류 수정 — route group 내 `admin/` 서브디렉토리 구조로 이동
  - 수정 전: `(admin)/page.tsx` → `/` URL (홈과 충돌)
  - 수정 후: `(admin)/admin/page.tsx` → `/admin` URL

### 미완성 항목

- [ ] **TASK-010**: 결제 내역 관리 (`/admin/payments`)
  - 전체 결제 내역 조회 (paymentId, txid, 금액, 상태, 사용자)
  - 결제 상태 필터 (completed, pending, cancelled)
- [ ] **TASK-011**: 계정 연동 현황 (`/admin/links`)
  - 연동 완료/미완료 사용자 현황
  - 미연동 사용자 수동 연동 처리
...
- [ ] **TASK-020**: DB 스키마 + 기반 구조 (brd_ctgr, brd_post, brd_cmnt, brd_attch)
- [ ] **TASK-021**: 게시글 CRUD API
- [ ] **TASK-022**: 댓글 + QNA 채택 API
- [ ] **TASK-023**: 첨부파일 API (Supabase Storage)
- [ ] **TASK-024**: UI 구현 (목록, 상세, 작성, 수정)
- [ ] **TASK-025**: 관리자 게시판 관리 (핀 고정, 강제 삭제)
...
| M7: 관리자 결제·연동 현황 | Phase 3 | — | /admin/payments, /admin/links | 🔄 진행 중 |
| M8: 통합 게시판 | Phase 4 | — | 4종 게시판 CRUD | ⏳ |
