...
> **기준일**: 2026-07-08
> **현재 버전**: Phase 7~12 완료 (PyCafé MVP · Pi 수익화 · 생태계 확장 · 사용자 프로필 · 통계 대시보드 · PyTranslateâ¢™ TASK-090~099 ✅) · **Phase 13 PyShopâ¢(MPS) Phase 1+2+3 완료 — P1+P2(TASK-100~111 ✅) + 후속 개선(A2U 환불·이미지 업로드) + Phase 3 O2O 오프라인 매장 커머스 완료 (TASK-113~120 ✅ 2026-06-16: 구글 카페 half-인증 등록·오프라인 주문 상태머신(주문중→준비중→상품대기중→10분 자동완료)·주문방법 3종·취소수수료·보이스 주문알림·지도 상품판매·IDOR 차단, `sql/050~060`) · PiRC3만 보류** · **Phase 14 PyVoice™ v3.0 권한 시스템 구현 완료 (TASK-120~125 ✅, `docs/PRD_9_VOICE_CHAT.md` v3.0 — 방장 보장 슬롯 + 멤버 자동 2/승인 2, S0 실기기 검증·TURN env 잔여)** · **Phase 15 LBS P0+P1 완료 (TASK-130~140 ✅, 상품 개별 위치 등록 포함)** · 횡단 개선: 무한 스크롤·지연 로딩 + SWR 캐싱·병렬 호출 성능 튜닝, Pi Tip→Bean 리브랜딩, 스티커 노출 개선 (2026-06-12) · **횡단 3차 (2026-06-13): Pi Browser admin 다국어 전환 무반응 수정(_pit 티켓 선발급) + 헤더 다국어 콤보 3계층 캐시 성능 개선** · **횡단 4차 (2026-06-14): CafePi 헤더 로고 교체·Pi Bet UI 아코디언·다국어 선택기 기억·PyShopâ¢ 브랜드 통일·admin open redirect 방어 + 어드민 대시보드 고도화(coin360 트리맵·사용자 관리 통합·KST 집계 교정·결제내역 개선)** · **Phase 16 평가 엔진 정밀화 (2026-06-15): M2 상태형 양방향 멱등·평가엔진 select후분기 복구·CHAR→VARCHAR(sql/046)·CRON_SECRET 프로덕션 필수·재평가 안전망 cron·관리자 재평가 버튼** · **횡단 5차 (2026-06-15): i18n 전체 자동번역 서버 after() 백그라운드화 + 번역률 반올림 버그·콤보 캐시 키 v2 무효화** · **GTM 문서화 (2026-06-16): 제품소개서(단기 4목표 13장) + 공개·라이선스 정책(오픈코어 3계층) + 성능 리스크 레지스터(7종 병목 분석) + 운영 이슈 기록(Vercel Hobby cron·GitHub Webhook)** · **이벤트 보상 전환 (2026-06-17, TASK-156): 10미션 완주 보상을 Pi A2U 송금 → mps_seller_bond 1π 직접 적립으로 전환, 자동→관리자 수동 지급 버튼, 원자적 RPC `fn_evt_grant_bond_reward`(FOR UPDATE+reward_st_cd 게이트)로 이중지급 차단, M3 유료테마 게이트 결함 수정** · **PyShopâ¢ 카트 다건 일괄 판매 (2026-06-17, 13-P4): 등록 페이지 분리(중고직거래/오프라인매장)+자국통화 등록표시(등록시점 고정 참고가 sql/062)+오프라인매장 카트 다중상품 단일 Pi 결제(`mps_order_item`·원자 RPC `fn_mps_cart_order_create`/`_cancel` sql/063)+주문관리 라인/호명/픽업매장명/색상. ⚠️ sql 062·063 적용+실기기 결제 검증 잔여** · **이후 Phase 16~27은 아래 진행률 표 참조 — 최신: Phase 25 이중 요금제(BEAN/PI) 런타임 스위칭·운영 fee_mode=PI 전환(메인넷 PI모드 운영 중) + Phase 26 오픈기념 무료요금 OneKey(2026-06-30~07-31 활성) + Phase 24 운영DB 컷오버 완료 + Phase 27 P2P 채팅 릴레이·직거래 문의방(당근형 O2O) (2026-07-01) + 운영 Pi 인증 메인넷(sandbox=false) 전환 확정·결제 전 흐름 실기기 검증·홈 설명서/다국어 정비 (2026-07-02) + **글로벌 i18n 대확장: 활성 locale 189개(187개국)·66개 언어 완역·카페 테마명 번역키 전환·UI 테마 12종 (2026-07-07~08, v12.7) + **Pi Sign-In(OAuth) 일반 브라우저 Pi 로그인·콤보 키인 검색·콤보 정렬 재배치 (2026-07-08, v12.8) + **Pi Sign-In 실사용 검증 완료·안정화 3건 (2026-07-08, v12.9 — 일반 브라우저 Pi 로그인 공식 개통) + **헤더 세션 표시 소유권 정리 (v12.10, 운영 확인 완료)**
> **배포 URL**: **staging** https://loginpi.vercel.app(Testnet·🧪) · **운영** https://cafepi.vercel.app(production 브랜치 게이팅) — 2단계 배포 분리(2026-06-28, docs/DEPLOY_STRATEGY.md)
...
| 항목 | 제약 | 현재 설정 | 해결책 |
|---|---|---|---|
| **Vercel Pro — Cron 주기** | 분 단위 cron 지원 (Hobby 제약 해소 — 2026-06-18 Pro 전환) | 고빈도 가능( 등) | Pro 유지 필수 (다운그레이드 시 분단위 cron 배포 차단) |
...
| **10** | 사용자 프로필 | ✅ 완료 | 100% |
| **11** | 통계 대시보드 | ✅ 완료 | 100% |
| **12** | PyTranslateâ¢™ 동시통역 | ✅ 완료 | 100% |
| **13** | PyShopâ¢(MPS) | ✅ 완료 (P1+P2+P3 O2O) | 100% (P3 O2O 카페 커머스 완료 2026-06-16: 구글 매장 인증등록·오프라인 주문 상태머신·주문방법 3종·보이스 알림·지도 상품판매 · PiRC3만 보류) |
| **14** | PyVoice™ v3.0 | ✅ 완료 (v3.0 권한 시스템) | 100% (S0 실기기 검증 잔여) |
...
| **13-P3** | PyShopâ¢ O2O 오프라인 매장 커머스 (2026-06-16) | ✅ 완료 | 100% (구글 카페 half-인증 등록·오프라인 주문 상태머신·주문방법 3종·취소수수료·보이스 알림·지도 상품판매·IDOR 차단 · TASK-113~120 · `sql/050~060`) |
| **13-P4** | PyShopâ¢ 카트 다건 일괄 판매 + 자국통화 (2026-06-17) | ✅ 완료 | 등록 페이지 분리(중고직거래/오프라인매장)·자국통화 등록표시(등록시점 고정 참고가 sql/062)·오프라인매장 카트(매장단위·담기 팝업·장바구니·라인 소계)→다중상품 단일 Pi 결제(`mps_order_item`+원자 RPC `fn_mps_cart_order_create`/`_cancel` sql/063)·주문관리 라인/호명/매장명/색상. ⚠️ sql 062·063 적용+실기기 결제 검증 잔여 |
...
| **24** | 메인넷 전환 & 2단계 배포 인프라 | 🔶 운영DB 컷오버 완료·배포 파이프라인 완성·메인넷 등재 잔여 | **Pi Testnet→Mainnet 전환 물밑작업 + 배포 환경 분리.** ✅완료: ⓐ **Py 개명**(PyCafé™/PyShop™/PyTranslate™ — Pi 접두 상표 회피, C-A 공식답변 반영) ⓑ **2단계 배포 분리**(staging=loginpi/master/Testnet·🧪 / 운영=cafepi/`production` 브랜치 게이팅, 같은 repo 다중연결) + `scripts/promote-to-prod.mjs`(ff-only 승격) ⓒ **3-tier DB 라우터**(`src/lib/db-env.ts`·`getDbTierInfo`·🧪 StagingBanner, VERCEL_ENV/APP_TIER 판정·하위호환 폴백) ⓓ **운영 WAS 신설**(Vercel 운영 프로젝트, Google·Pi 로그인 정상화 — NEXT_PUBLIC_APP_URL 도메인별·Google Console 콜백 등록·GOOGLE_CLIENT_SECRET 재설정). ✅추가완료(2026-06-28~29): ⓔ **운영DB 컷오버**(ajdwlcqoljkjamostutc·96테이블·59,280행·pg_dump+session_replication_role=replica·센티넬 검증) ⓕ **운영DB 읽기전용 스위치**+쓰기 가드(SUPABASE_READONLY_MODE env·JWT 전용 발급 스크립트·read-only 세션 유지) ⓖ **배포 컨트롤 2단 분리**(Stage/운영)+Staging DB 스위치 관리자 화면+진행상태 폴링 ⓗ **DB 정리**: 외부 테이블 12종 DROP(sql/134)+i18n 레거시 테이블 2종+데드 함수 제거. 🔶잔여(메인넷 등재): Dev Portal·API Key·지갑·도메인검증·P0 실기기 검증·U2A 1건·등재 신청. 정본: `docs/MAINNET_READINESS_CHECKLIST.md`·`docs/DEPLOY_STRATEGY.md`·`docs/DEPLOY_NOTICE.md` · 체크리스트 `mainnet_checklist`(sql/129·132)·`ops_checklist`(sql/130·131·133). ✅**추가완료(2026-07-02)**: ⓘ 운영 Pi 인증 **메인넷 전환 확정**(`NEXT_PUBLIC_PI_SANDBOX=false` 운영 고정)+운영DB 클린 초기화(`334e52d` 단일 TRUNCATE)+**결제→텔레그램 알림→딥링크 Pi Browser 복귀 전 흐름 실기기 성공 검증**. ⚠️sandbox 플래그 전환이 전 사용자 pi_uid 재발급→재동의·계정 무한 재생성 systemic 사고 유발 → 원인규명 + **SANDBOX_FLAG 환경별 고정값 정의**(staging=`true`·운영=`false`, `docs/TROUBLESHOOT.md` [2026-07-02]) |
| **25** | 이중 요금제(BEAN/PI) 런타임 스위칭 | ✅ 구현 완료·운영 PI모드 전환(2026-06-30) | `docs/PRD_24_FEES_STRATAGE.md` — 메인넷 A-5 대응 `fee_mode_config(BEAN\|PI)` 런타임 전환(`/admin/fee-mode`·캐시 60s→원자 전환). **PI=진짜 Pi 직결제**(window.Pi→pi_pymnt). 카페생성·이벤트방·스티커팩·카페선물(P2P)·구독을 Bean 차감→Pi 직결제 전환(sql/140~148), 마이크로요금(입장·번역·AI·배지·부스팅) PI 무료화, 후기보상 매장주 보증금 선행 게이트 + PI 후기 실 A2U 송금(sql/144 멱등·cron `fbck-pi-payout`), 대시보드·통합분석 매출 Pi 통계(sql/147 `_pi` 컬럼), 클라 요금표시 PI 반영. **운영 fee_mode=PI 전환 완료 — 운영=메인넷 PI모드 운영 중**('메인넷 전 BEAN 유지' 정책 종료). 잔여: 보상 A2U(이벤트미션·캠페인) 미전환 |
| **26** | 오픈기념 무료요금 OneKey | ✅ 구현 완료·운영+staging 활성 | `docs/PRD_26_OPEN_PROMO_FEE.md` — `promo_fee_config.promo_active_yn` 단일 토글로 9개 요금 전부 0 오버라이드(정상요금 `bean_fee_plan` 보존), `promo_end_dtm` 도달 시 `fn_is_open_promo_active()` FALSE 자동 종료. `applyPromoGate` 7개 청구경로 통합(번역 포함)·`/api/admin/open-promo`·그랜드오픈 Welcome 배너(KST·D-day)·번역 일일 무료한도(비구독자 10건). **활성 2026-06-30~2026-07-31 KST 자동 복귀**(sql/149). 잔여: 미구현 품목(기간연장·PyShop·노출) 추가 |
| **27** | P2P 채팅 알림·릴레이 + 직거래 문의방 (당근형 O2O 채팅) | ✅ 구현 완료·운영 배포 (2026-07-01) | `docs/PRD_13_MSG.md` §18 — 당근마켓 앱푸시를 Pi Browser(WebView·푸시 부재)에서 텔레그램으로 대체(**하이브리드**: 앱 내 DM `msg_room` D + 텔레그램 미러 45초지연·미읽음게이트·인용답장 릴레이, sql/152 `noti_tp_cd`·`msg_tlgm_out`·`cur_relay_room_id`). **"판매자에게 문의" 진입 UI**(상품상세·주문관리 양방향·`/api/chat/rooms` 재사용)·번역 21 locale. **직거래 문의방** 정밀 정의: 당사자2명 전용·🤝 DIRECT 전용테마(sql/158·`use_yn='N'`)·12h `expr_dtm` 자동만기+🗑️수동만기(expire API)·상품별 분리(`msg_room.item_id` sql/160·같은판매자 다른상품=별도방·`room_desc` 상품명)·"내 카페" 최상단 별도분류(`theme_tp_cd='DIRECT'` sql/159·직거래>구독>일반)·선물(Bean/Pi) 금지(3계층: `/api/tips` 403·`canTip`·complete PI_TIP 변조방어). +관리자 빠른메뉴(`sys_quick_menu` sql/157). staging+운영 DB(sql/152·157~160)+코드 전량 배포(promote). 잔여: 통합알림(TXN_ST/FBCK) 트리거·P0 실기기 검증 |
| **횡단8** | 홈 설명서 정비·다국어 sync (2026-07-02) | ✅ 완료 | 홈 "쉬운 사용 설명서" 8주제 **PiRC1/2/3 체계 표기**(💬카페&통역 PiRC1 · 🎫Pi구독 PiRC2 · 📦P2P중고거래 PiRC3 · 🏪O2O 매장/상품등록 PiRC3)+💬↔🎫 순서교환 · 🎁이벤트·🪙테스트넷보상 **숨김**(icon 필터·데이터 보존) · 🎫Bean&구독→**Pi구독(PiRC2)** 개편(Bean 내용 제거). **21 locale Gemini 재번역**(topics 42키만 정밀 갱신·나머지 무변경, icon 이모지·PiRC·PyCafé™/PyShop™/PyTranslate™·O2O·P2P 토큰 보존). 커밋 9129741·384bb66·10a352b, staging+운영 promote |

### 통계
- **총 Phase**: 27개 + 횡단7(DA) (0~18 + 횡단 개선·문서화 + Phase 20 성능 + Phase 21 보안 + 횡단7 DA 거버넌스 + Phase 22 분석 + Phase 23 모니터링 + Phase 24 메인넷 전환·배포 인프라 + Phase 25 이중 요금제 + Phase 26 오픈기념 무료요금 + **Phase 27 P2P 채팅 릴레이·직거래 문의방**)
- **완료**: 22개 구현 완료 (Phase 18 판매자 주문 알림 + Phase 25 이중 요금제 + Phase 26 오픈기념 무료요금 + **Phase 27 P2P 채팅 텔레그램 알림·릴레이·직거래 문의방(운영 배포)** 포함) + 횡단7 DA 중간점검 ✅
- **진행 중**: 4개 (Phase 20 화면 성능 최적화 · Phase 21 보안 강화 · Phase 23 실시간 모니터링 · **Phase 24 메인넷 전환 & 2단계 배포 — 운영DB 컷오버 완료·운영 fee_mode=PI 전환 완료(메인넷 PI모드 운영 중), 메인넷 등재(Dev Portal·API Key·실기기 검증·등재 신청) 대기 2026-07-01**)
- **기획·문서**: 1개 (Phase 17 BEAN 토큰 발행 — 문서 전용·앱 코드 0, T01/T02/T05 외부 회신 대기)
- **예정**: 확장 Phase (PiRC3 실 에스크로 보류 해제 대기, LBS 지도 UI 추가 확장, PyVoice TURN 운영, 이벤트 행위훅 전수 점검, StarterKit 패키지 제품화) · **알림 Phase 2**: 카카오 알림톡(한국 판매자)·Telegram webhook 양방향(버튼으로 접수/거절)
...
- PyVoice 잔여 (S0~S3): Pi Browser 실기기 마이크 검증 · **TURN 운영 설정**(`TURN_HOST`/`TURN_SECRET`) · 품질 데이터 기반 coturn 전환 판단 · 5인+ LiveKit SFU/결제 게이팅 검토
...
## Phase 8: PyCafé 수익화 기능 ✅ (완료, 2026-06-11)
...
## Phase 9: PyCafé 생태계 확장 ✅ (완료, 2026-06-11)
...
## Phase 12: PyTranslateâ¢™ 글로벌 동시통역 ✅ 완료 (2026-06-12 — TASK-090~099 전체)
...
> **P0 완료 = MVP**: TASK-090 → 091 → 092 → 093 → 094 → 095 ✅ — PyTranslateâ¢™ 기본 동작
...
- ✅ 어드민 통계 대시보드에 "번역 (PyTranslateâ¢™)" 섹션 추가 (`translate-stats-section.tsx` — LazySection 지연 로드)
...
## Phase 13: PyShopâ¢(MPS) ✅ (Phase 1 MVP + Phase 2 확장 완료 — 2026-06-13, Phase 3 고도화 예정)
...
> PRD §15.8 상세. 직거래(중고)와 별개로 **구글 지도의 실제 카페를 인증 등록 → Pi 메뉴 판매 → 픽업/배달**하는 O2O 커머스. PyChat(온라인 커뮤니티)에서 시작한 cafe.pi가 오프라인 실물 카페로 흐르는 교두보.
...
## Phase 14: PyVoice™ — WebRTC N:N 음성채널 ✅ (v3.0 권한 시스템 구현 완료 — 2026-06-12)
...
| M14: PyCafé DB + 테마 마스터 | Phase 7 | 2026-06-08 | msg_* 13개 테이블, 테마 20개, 플랜 5개, 스티커팩 60개 | ✅ 완료 |
| M15: PyCafé MVP | Phase 7 | 2026-06-08 | 그룹 카페 생성 4단계 마법사 (테마 선택 + Pi 결제), 카페 홈, 카페 페이지 | ✅ 완료 |
...
| M18: PyCafé 생태계 | Phase 9 | 2026-06-11 | 마켓플레이스, Pi Bet, Webhook, 분석 대시보드, 커스텀 스티커 (sql/022 + API 12종 + UI 4종) | ✅ 완료 |
...
| M21: PyTranslateâ¢™ 완성 | Phase 12 | 2026-06-12 | sql/020 + chat-translate.ts + dedup + translate API + broadcast 확장 + 표시언어 설정 + 원문 토글 + 어드민 번역 통계 + 품질 피드백 (TASK-090~099) | ✅ 완료 |
| M22: PyShopâ¢(MPS) Phase 1 MVP | Phase 13 | 2026-06-11 | sql/029_mps.sql (mps_ 6개 테이블) + lib 헬퍼 3종 + 상품·주문·에스크로 API 12종 + 화면 6종 + 판매자 보증금 (TASK-100~107) | ✅ 완료 (Phase 2·3 확장 예정) |
| M23: PyVoice™ v3.0 N:N 음성채널 + 권한 시스템 | Phase 14 | 2026-06-12 | sql/032 (msg_call_participant 신규·call_log room 레벨 전환) + sql/035 (mic_st_cd 3상태) + TURN 발급 + 음성채널 API 6종(join/leave/signal/mic-control/participants/request) + use-voice-channel 훅(Full Mesh) + 방장 보장 슬롯·자동 2/승인 2 권한 UI + S0 진단 메시지 (TASK-120~125) | ✅ 구현 완료 (S0 실기기 검증·TURN env 잔여) |
...
| M31: 횡단 4차 — Pi Browser 안정화 | 횡단 | 2026-06-14 | CafePi 헤더 로고 교체(12118e7)·Pi Bet UI 아코디언+스포트라이트(c490fb7)·다국어 선택기 기억(d52d7ef)·PyShopâ¢ 브랜드 전면 치환(04c9350)·admin 게이트 open redirect 방어(43ab342·8f419ee) | ✅ 완료 |
...
- ✅ `docs/제품설명서_202060615.pptx` — cafe.pi 직영 광장 + 단기 4목표 기준 13장 슬라이드
  ...
  - 단기 4목표: ① PyChat 무료→소액수수료 ② PyShopâ¢ 오프라인 교두보·매집 ③ StarterKit $100~500 ④ 외주연계
...
- ✅ `docs/공개_라이선스_정책.md` — 오픈코어 3계층 전략 정의
  ...
  - **상품층(BSL/시간제한)**: 카페/마켓 기능, i18n 엔진, LBS, PyVoice
  - **금고층(비매, 직영 전용)**: PyChat 실시간 엔진 + PyShopâ¢ 에스크로 코어 + 코어 5종
  - 왕관보석(PyChat·PyShopâ¢ 직영) 비매 원칙 — 목적=선점 창·단기 시장점유, 영구 비밀유지 아님
...
> ⚠️ **발행 전 앱 코드 0.** 레드라인 정책(자체 토큰=Pi 외 자산)상 BEAN은 공식 Pi Launchpad 경유로만 발행하며, 발행 전까지 토큰 관련 코드는 앱에 미포함. (문서는 git 커밋 — 파일럿 인큐베이팅, 2026-06-17 정책)
...
### 후속 UX·보안 개선 (2026-06-18)

- **알림 메시지 메뉴별 표기**: 주문 알림에 메뉴 라인별 명칭·수량·단가·소계 + 합계 표기(`enqueueOrderNoti` 라인 스냅샷 + `mps-noti.ts` 빌더). 직거래 단건은 `item_nm` 폴백.
- **알림 카드 UX**: Telegram 연동 카드를 개인정보 탭→스토어 탭(상품목록 위)으로 이동(판매자에게만 필요), 앰버 하이라이트+"추천" 배지, 문구 "Pi Browser 자체 알림이 없으므로, Telegram 알림을 추천합니다."
- **프로필 정리**: Google 계정 정보 섹션(이메일·사용자 ID·인증 제공자·이름) 제거.
- **매장별 주인 분리 발송 검증**: 알림 수신자=주문 `seller_id`(=매장 주인), 매장(주인)별 격리 확인. 미연동 주인은 Realtime+Pull 폴백.
...
- **지도 유입 결제 세션 복구**: 지도 등으로 진입해 세션이 유실/지연된 경우 결제 직전 `signIn({ silent })` 재인증 후 진행(서버는 X-Pi-Token으로 최종 판정) — `pi-auth-provider`·`store-item-detail`·`client-cart`
...
### §19.9 Bean 수동 조정 감사 로그 조회 ✅ 완료 (P0-4 마무리, 2026-06-22)

- ✅ `GET /api/admin/token/audit` — `bean_audit_log` 조회 API (usr_id 필터·최신 100건·총 건수 반환)
- ✅ `/admin/token/audit` 페이지 — 조정 전후 잔액·조정량·사유·증빙 URL 테이블 + usr_id 필터
- ✅ 어드민 사이드바 "감사 로그" 링크 추가 (`beanAudit` i18n 키)
- **PRD_16 P0-4 완전 완료**: 테이블(sql/069) + insert(adjust API) + 조회(audit API + 화면) 모두 연결
...
- ✅ **EVENT**: `getEventRanking` 6개 순차 Supabase 쿼리 → **2단계 병렬** — ① 메타 2쿼리(제외목록·missions) → ② 나머지 4쿼리 `Promise.all` 동시 실행. 순차 왕복 6회 → 병렬 2단계, 네트워크 왕복 ~40% 감소 (`src/lib/event.ts`)
...
> **정본**: `docs/PRD_2_SECURITY.md` (v2.0) · `docs/SECURITY_DDOS_POLICY.md`  
> **점검결과표**: `docs/보안취약점점검결과표.pptx` (11슬라이드, 23항목, python-pptx 1.0.2)  
> **투입 에이전트**: kisa-web-security-auditor (PRD_2_SECURITY 수립) · 아소카 (DDoS 코드 구현·v2.0 갱신·Pi Browser 보안 조치)

### 배경

Pi Browser WebView + Vercel 서버리스 + Supabase 환경에 KISA 행정안전부 21개 웹 취약점 기준을 적용하고, DDoS 공격에 대한 5계층 방어 체계를 코드로 구현한다.

### TASK-200 — KISA 21개 항목 기준 수립 + v2.0 갱신 ✅ 2026-06-23

- `docs/PRD_2_SECURITY.md` v1.0 작성 → **v2.0 완전 갱신** — 23개 항목 전체 재분석
- 결과: ✅ 양호 **18**개 / 🔍 추가확인필요 **3**개 (MC·IE·IL) / ➖ 해당없음 2개 (XP·WR)
- cafe.pi 특수 환경 반영: Pi Browser 이중 인증 경로·Supabase RLS 비활성화 근거·클라이언트 게이트 패턴
- `docs/보안취약점점검결과표.pptx` 신규 생성 — KISA 23항목 공식 점검결과표 (11슬라이드, 63KB)

### TASK-200a — Pi Browser 보안 제약 대응 ✅ 2026-06-23

- **CSP(Content-Security-Policy) 제거**: `vercel.json` → Pi Browser WebView SDK 브릿지(`minepi.com/static/native-ui.js`)가 CSP에 의해 차단되어 접속 불가했던 문제 근본 해결 (commit `01f1a58`)
- **X-Frame-Options 제거**: Pi Browser가 Pi 도메인을 iframe으로 임베딩할 때 차단되는 문제 해결
- **Google Maps "could not load" 오류 수정**: CSP가 Maps 스크립트 로드를 막던 동일 원인 수정 — 지도 탭 정상 작동
...
| `vercel.json` | 전역 보안 헤더 (HSTS·X-Content-Type) — **CSP·X-Frame-Options는 Pi Browser 제약으로 제거됨** (TASK-200a) |
...
## 횡단 7차: DA 거버넌스 중간점검 ✅ 완료 (2026-06-23)

> **담당 에이전트**: da-governance-expert (KDATA 한국데이터산업진흥원 표준 기반 DA 감사 전담)
> **감사 범위**: `sql/001`~`sql/101` 전체 (70+ 테이블)
> **보고서**: `docs/da/reports/2026-06-23_DA중간점검보고서.md` (21KB, 10개 섹션)

### 감사 결과

| 점검 영역 | 준수율 | 비고 |
|---|---|---|
| 시스템 컬럼 4종 (`regr_id`·`reg_dtm`·`modr_id`·`mod_dtm`) | 97% | i18n 3테이블 누락 (P1) |
| 논리삭제 패턴 (`del_yn`·`del_dtm`) | 95% | i18n 3테이블 누락 (P1) |
| 명명 규칙 (도메인 약어·복합어) | 100% | 이상 없음 |
| 표준단어 등록 | 100% | BET·OPTN·WEBHOOK 장기 등재 예정 |
| **전체 준수율** | **95% → 100%** | P1·P2 조치 후 완전 달성 |

### P1 조치 — i18n 테이블 시스템 컬럼 추가 ✅ Supabase 적용 완료

**`sql/102_i18n_metadata_fix.sql`** — i18n 3개 테이블 시스템 컬럼 4종 + 논리삭제 컬럼 일괄 추가

| 테이블 | 추가 컬럼 |
|---|---|
| `i18n_locale` | `regr_id`, `modr_id`, `del_yn CHAR(1) DEFAULT 'N'`, `del_dtm` |
| `i18n_message` | `regr_id`, `modr_id`, `del_yn`, `del_dtm` |
| `i18n_country` | `mod_dtm`(누락), `regr_id`, `modr_id`, `del_yn`, `del_dtm` |

### P2 조치 — bean_fee_plan 가격 역전 정정 ✅ Supabase 적용 완료

**`sql/103_bean_fee_plan_price_fix.sql`** — SSGDM(GENERAL)↔SSPDM(PREMIUM) Bean 금액 swap

| fee_plan_cd | 수정 전 amt_bean | 수정 후 amt_bean | 설명 |
|---|---|---|---|
| SSGDM (GENERAL) | 10 | **5** | 구독자 노출 1개월 일반 |
| SSPDM (PREMIUM) | 5 | **10** | 구독자 노출 1개월 프리미엄 |

원칙: PREMIUM amt_bean > GENERAL amt_bean (프리미엄이 더 비싼 요금, 역전 상태였음)
...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|---------|-------|
| v12.10 | 2026-07-08 | **헤더 세션 표시 소유권 정리 — Pi Sign-In 세션 시 Google 버튼 숨김·계정명 표시** (운영 확인 완료) — 일반 브라우저의 Pi Sign-In 세션(HMAC)은 NextAuth와 별개라 Google 버튼이 '비로그인'으로 오판 → 로그인 후에도 병행 노출 + 계정명 표시 주인 부재. 정리: GoogleLoginButton은 NextAuth 세션 없이 Pi 세션만 있으면 숨김(통합계정 Google 로그인 케이스는 session.user 존재로 무영향), PiOAuthLoginButton이 일반 브라우저 Pi 세션의 계정명(프로필 링크·nick_nm→@username) 표시 담당·Google 세션 존재 시 양보. 4상태 매트릭스(비로그인/Pi로그인/Google로그인/Pi Browser) 확정. **모바일 일반 브라우저 경로 실기기 검증 완료**: 인가 페이지가 모바일 UA에선 QR 없이 [Open Pi Browser] 딥링크(pi://redirect.pi/pi-net/<swp>)로 렌더링 → 같은 폰의 Pi Browser에서 승인 → 원래 브라우저 상태 폴링 자동 진행(기기 1대 완결) — **3종 여정(PC QR·모바일 딥링크·Pi Browser SDK) 전부 실사용 검증 통과, Pi Sign-In 최종 완결**. | asoká |
| v12.9 | 2026-07-08 | **Pi Sign-In 실사용 검증 완료 — 일반 브라우저 Pi 로그인 공식 개통** — 마스터 실기기로 전 구간 완주(운영 크롬 버튼 → 인가 → 파이지갑 QR 스캔·승인 → 콜백 → 세션 발급 → 헤더 계정명 표시 ✅). 안정화 3건: ① **isLoading 잠복 버그**(provider 마운트 세션 복원이 비로그인 시 로딩 미해제 → OAuth 버튼 disabled 굳음 — 수개월 잠복, 신규 버튼에서 발현. 복원 완료 시 user 유무 무관 해제) ② **state 재마운트 경합**(locale 자동 전환 네비게이션이 콜백 재마운트 → 1회용 state 선소거 → 실승인 후에도 "보안 검증 실패" 오탐. peek/clear 분리=성공 시에만 소거·모듈 레벨 1회 가드·콜백 경로 locale 자동 전환 제외) ③ **QR 스캔 사용자 경로 확정**(마스터 발견): 채굴 앱·폰 카메라가 아니라 **파이지갑 → Pay 버튼 → "QR 코드 스캔 및 표시" → 스캔 탭** — QR 내용은 accounts.pinet.com/swp-<세션ID> 일반 URL(실측 디코드). Playwright 자동화로 운영 직접 검증(버그 재현→수정 확인→위조 토큰 거부·재진입 내성 실측). Redirect URI는 앱 도메인+루프백만 허용(staging 등록 불가 → staging은 env 미설정으로 버튼 미노출 운영). 커밋 5362560 이후 ~5fa5869 계열. TROUBLESHOOT 2026-07-08 Pi Sign-In편 정본. | asoká |
| v12.8 | 2026-07-08 | **Pi Sign-In(OAuth) 일반 브라우저 Pi 로그인 + 콤보 UX 2종** — ① **Pi Sign-In**(커밋 4dd756f·5ae09f5·5362560, 운영 실기기 검증 완료): Developer Portal의 OAuth implicit flow 연동 — `accounts.pinet.com/oauth/authorize`(response_type=token·scope=username·state) → 콜백 `/auth/pi/callback`(프래그먼트 토큰 파싱·CSRF state 검증) → **기존 `/api/auth/pi` 재사용**(/v2/me 검증·HMAC 세션·username 재바인딩 그대로). "일반 브라우저=Google 로그인만" 제약 해소 → Pi 계정 단일 체계(등재 레드라인 'Pi 외 로그인' 완화). 함정 2종 실기기 해결: state 저장을 sessionStorage(탭 단위 오탐)→**localStorage+10분 만료**, Pi의 인가 페이지는 **Pi Browser 내 미지원** → 버튼 클릭 시 SDK signIn() 선시도(3초 상한, authenticate=유일 신뢰 신호) 후 실패 시에만 OAuth로 진행하는 자기교정. env `NEXT_PUBLIC_PI_OAUTH_CLIENT_ID`(환경별 Vercel 설정)·Redirect URI 3종 등록(cafepi·loginpi·localhost). ② **대형 콤보 키인 검색**(0ddf2f8): 언어 스위처(189)·통화 콤보(187)·번역 언어 선택에 sticky 검색 입력 — 국가 영문/현지명·코드·통화 다중 필드 부분일치(`comboMatch`), 섹션 카운트 실시간·결과없음 표시. ③ **콤보 정렬 재배치**(65cde5b·sql/168): 우선 12개국(한·미·중·일·베트남·인도·나이지리아·인니·필리핀·독일·태국·말련) 고정 + 나머지 국가 영문명 알파벳순(i18n_locale.sort_ord + i18n_cntry_mst.dis_ord_seq, VALUES 조인 3문장) + **Afghanistan→af(아프리칸스어) 오연결을 ps(파슈토어)로 교정**. ④ 상비 도구: `scripts/i18n-increment-translate.mjs` — ko.json 신규 키 원커맨드 증분 번역(그룹별 누락 감지·재실행 재개, langSwitcher 2키·piOAuth 5키 × 66언어 반영). 운영 승격 2회(65cde5b~0ddf2f8, 4dd756f~5362560). | asoká |
| v12.7 | 2026-07-07~08 | **글로벌 i18n 대확장 — 활성 locale 189개(전 세계 187개국) + 카페 테마명 번역키 전환 + UI 테마 12종** — ① **157개국 일괄 활성화·전량 번역**: 복사 그룹 110개국(기존 완역 언어 공유·API 비용 0) + 신규 46개 언어 Gemini 병렬 번역(ICU 변수 보존·한글 혼입 검증 게이트 — 최종 탈락 0), Gemini 유료 전환. 재사용 파이프라인 `scripts/i18n-bulk-*`(활성화·언어매핑 단일소스 `i18n-lang-map.mjs`·번역·언어마스터 시드 — 전 단계 멱등·중단 재개). ② **구조 버그 4종 근본수정**: 국기 이모지 베이스 오계산(0x1F1E0→0x1F1E6, il=🇨🇫 오국기·et/ps/sq 깨짐의 근원)·`i18n_cntry_mst.locale_cd` FK가 `i18n_lang_mst` 참조(사일런트 실패 → 언어마스터 시드로 187/187 연결)·Argentina 'ar'(아랍어) 충돌 → `ar-AR` 파생 활성화·**et(에티오피아) 에스토니아어 오번역 → 암하라어 재번역**(기존 에스토니아어는 신설 ee가 승계 — 국가코드≠언어코드 함정 해소). ③ **공유 언어 8개국 전용 locale 신설**(at·ca·ie·li·nz·sg·gb + AL=기존 sq, 통화 분리 EUR·CAD·CHF·NZD·SGD·GBP — au·mx 선례). ④ **번역키 전수 정비**: 환불 8키 22개 locale 전량 누락(영어 화면 한국어 노출 버그)·es/it 한글 혼입 3건·ICU 변수 오번역 4건(fr `{thème}`→`{theme}` 등 치환 실패 실버그)·죽은 키 `lbs.nearbySubtitle` 완전 제거(c7ce384에서 삭제 대신 빈값 잔재 → 신규 locale 99% 통계 원인). ⑤ **카페 테마명 번역키 전환**(`themes.<theme_cd>` 21종 × 66개 언어, 커밋 28f1e29): theme_nm/theme_nm_en 2단 구조 대체, `useThemeName` 훅(번역키 우선·폐기 테마 DB명 폴백), 적용 9곳(마켓플레이스 칩·카드/테마선택기/카페목록/주변탐색/배지 팝업/방 패널/카페 생성기/어드민 매출·대시보드) + 하드코딩 한국어 18건 동시 해소(검색 placeholder·이벤트 요금·배지 팝업 전체). ⑥ **UI 색상 테마 12종 시드**(sql/165·166 — 파스텔 다이어리·애스트로 팝·판다 밀크·홀로그램 글래스 등, 기존 CLAY/GLASS fx 재사용·toFlagEmoji 버그 코드 수정 포함). ⑦ **운영 반영 완료**: i18n DB 4종 pg_dump 복사(40.4만 행 — 69MB 단일 트랜잭션 운영 인스턴스 연결 종료 → 5만 행 청크 분할·재시도로 성공) + 테마 키 델타 7,332행 + 코드 2회 승격(fe73b68·28f1e29). sql/165~167. 정본: PRD_3_MUL_LAN v2.0·PRD_17 v2.1·TROUBLESHOOT 2026-07-08편. | asoká |
| v12.6 | 2026-07-03 | **운영 텔레그램 "발송O/콜백X" 사고 근본수정 2건 + 관리자 UI 소수정** — ① **webhook 자가치유**(커밋 b3563c0): 환경별 봇 분리(스테이징 `cafe_pi_not_bot`→loginpi / 운영 `cafe_pi_areal_bot`→cafepi) 후 운영 봇 수동 setWebhook 누락으로 인용답장·`/start` 콜백 유실 → `src/lib/telegram-webhook.ts` `ensureTelegramWebhook()`이 cron chat-noti(1분)마다 등록 URL을 `NEXT_PUBLIC_APP_URL` 기대값과 대조·자동 재등록(수동 절차 폐지). 진단/강제재등록 `/api/admin/telegram/webhook`(GET/POST). ⛔철칙=환경별 봇 분리 유지(공유 시 플립플롭). ② **딥링크 세션 오리진 교정**(Vercel env·커밋 b1128de 문서화): 운영 `NEXT_PUBLIC_PI_APP_DOMAIN` cafe7092.pinet.com→**cafepi.vercel.app**. localStorage `pi_token`은 오리진 격리라 딥링크가 세션이 사는 오리진과 다르면 "Pi Browser 열리는데 로그아웃" 증상 — **딥링크 도메인=세션 오리진** 원칙 확립(스테이징=pinet 도메인 현행 유지). 실기기 검증 완료(마스터). 배포 번들 인라인 값 실측 검증 기법 확립. ③ 관리자 하단 퀵메뉴-페이지네이션 겹침 **근본수정 3종**: ⓐ main 하단 패딩 `pb-32`(128px, safe-area 미반영·트리거 상단 130px+보다 낮음)→`calc(env(safe-area-inset-bottom)+9.5rem)` ⓑ **유령 클릭 제거** — auto-hide 후 opacity-0 트리거가 `pointer-events-auto`로 남아 안 보이는 채 페이지네이션 탭을 가로채던 버그(숨김 시 pointer-events-none, touchstart 히트테스트라 첫 탭은 아래로 통과) ⓒ `use-dynamic-limit` 퀵메뉴 상단경계 예약+12px 버퍼(chromeHeight 근사 오차 흡수). + `sq`·`ps` locale 정비(ps 통화 AFN·국가 af 매핑 보완, validate:locales 통과). 정본: PRD_13 v1.3 §8-2·§11-6, TROUBLESHOOT 2026-07-03편. | asoká |
| v12.5 | 2026-07-01 | **P2P 채팅 텔레그램 알림·봇 릴레이 + 직거래 문의방 정밀 정의 (Phase 27)** — ① **P2P 채팅 알림·릴레이**(당근 앱푸시 대체·PRD_13 §18): 하이브리드(앱 내 DM `msg_room` D + 텔레그램 미러 45초지연·미읽음게이트·인용답장 릴레이), sql/152(`noti_tp_cd` ORDER/CHAT/TXN_ST/FBCK 통합·`msg_tlgm_out` 인용답장매핑·`cur_relay_room_id`), **"판매자에게 문의" 진입 UI**(상품상세·주문관리 양방향·기존 `/api/chat/rooms` 재사용)·번역 21 locale. ② **직거래 문의방**: 당사자2명 전용·이름 '직거래 문의'·🤝 DIRECT 전용테마(sql/158·`use_yn='N'` 시스템전용)·12h `expr_dtm` 자동만기+🗑️수동만기(`expire` API)·상품별 분리(`msg_room.item_id` sql/160·같은판매자 다른상품=별도방·`room_desc` 상품명·판매자검증)·"내 카페" 최상단 별도분류(`theme_tp_cd='DIRECT'` sql/159·직거래>구독>일반)·선물(Bean/Pi) 금지(3계층: `/api/tips` 403·`canTip=false`·complete PI_TIP 변조방어). ③ **관리자 빠른메뉴**(`sys_quick_menu` sql/157·`/admin/quick-menu`). staging+운영 DB(sql/152·157~160)+코드 전량 배포(promote). 커밋 bb24fd4·0b2f896·7af4167·5330531·fdebea6·8d25eab·7f88632·3e9c722. | asoká |
| v12.4 | 2026-06-29~30 | **이중 요금제(BEAN/PI) 완성·운영 PI모드 전환 + 오픈기념 무료요금 OneKey + 매장별 Telegram 알림 + 메인넷 클린시작** — ① **이중 요금제 런타임 스위칭**(PRD_24): `fee_mode_config(BEAN\|PI)` 단일 토글·`/admin/fee-mode` 제어. PI 모드=진짜 Pi 직결제(window.Pi→pi_pymnt). 카페생성·이벤트방·스티커팩·카페선물(P2P)·구독을 모두 Bean 차감→Pi 직결제로 전환(sql/140~148), 마이크로요금(입장·번역·AI·배지·부스팅) PI모드 무료화, 후기 보상 매장주 보증금 선행 게이트 + PI 후기 실 A2U 송금(sql/144 멱등·cron `fbck-pi-payout`). 대시보드·통합분석 매출 Pi 통계 전환(sql/147 `_pi` 컬럼). 클라이언트 요금 표시 PI 반영(`FeatureFlagProvider feeMode`·`useMicroFeeLabel`). **운영 fee_mode=PI 전환 완료(2026-06-30)** — 운영=메인넷 PI모드 운영 중('메인넷 전 BEAN 유지' 정책 종료). ② **오픈기념 무료요금 OneKey**(PRD_26): `promo_fee_config.promo_active_yn` 단일 스위치로 9개 요금 전부 0 오버라이드(정상요금 보존), `promo_end_dtm` 도달 시 `fn_is_open_promo_active()` FALSE 자동 종료. `applyPromoGate` 7개 청구경로 통합(번역 포함)·`/api/admin/open-promo`. 그랜드오픈 Welcome 배너(종료시각 KST·D-day) + 번역 일일 무료한도(비구독자 하루 10건·사전 차단). **운영+staging 활성(2026-06-30~2026-07-31 KST 자동종료)**. ③ **매장별 Telegram 주문 알림**(sql/148 `mps_shop.tlgm`, 매장당 1:1) + Pi 딥링크 환경별(`pi://<host><path>`·staging≠운영·브리지 `/ko/open`, 운영 base=cafe7092.pinet.com). ④ **메인넷 데이터 클린시작 SQL**(운영 전용·testnet 테스트 데이터 단일 TRUNCATE·FK 대응 사용자테이블 6종 포함). ⑤ 운영 Storage 버킷 4종 수동 생성(pg_dump 미러 제외분). 기타: 상품 결제 approve 실패 가시화·STAGING 배너 채팅 헤더 겹침 수정·1인 1계정 강화(Pi 미연동 Google 차단·JWT 즉시 갱신). 커밋 86건(6/29~30). | asoka |
| v12.3 | 2026-06-28~29 | **운영DB 컷오버 완료 + 2단계 배포 파이프라인 완성 + 읽기전용 모드 + DB 정리** — ① **운영DB 컷오버 완료**(ajdwlcqoljkjamostutc·96테이블·59,280행): pg_dump 방식(git replay genesis 누락으로 불가), truncate→data-only 덤프→`session_replication_role=replica` 적재, 센티넬 검증 완료(cafepi→운영DB). SERVICE_ROLE_KEY 함정(service_role 아닌 publishable key 사용 시 sys_user RLS로 0행→세션 미인지) 해소. ② **운영DB 읽기전용 스위치 + 쓰기 가드**: `SUPABASE_READONLY_MODE` env + `src/lib/db-env.ts` 판정, 읽기전용 JWT 발급 스크립트(`scripts/issue-readonly-jwt.mjs`) + Pi/Google 로그인 세션 유지(쿠키/X-Pi-Token 보존). ③ **배포 컨트롤 2단계 분리**: `/admin/deploy` Stage/운영 별도 버튼 + Staging DB 스위치 관리자 화면 + 진행상태 라이브 폴링 UI. ④ **DB 정리**: 외부 테이블 12종 DROP(`sql/134`·cafepi 무관 잔재) + i18n 레거시 테이블 2종 + 데드 함수 제거. ⑤ **DB 연결 라이브 진단 API**(`/api/admin/db-health`·환경별 연결 상태 실시간 확인). 커밋 c62dd43·7c6a7be·808ee82·1f40095·200920f·4f3fc9e·1e17467. | asoka |
| v12.2 | 2026-06-27 | **Py 개명 완료 + 관리자 화면 개선** — ① **Pi→Py 공식 브랜드 전환**(커밋 fa19122): 표시 텍스트 전체 `PiCafé™→PyCafé™`·`PiShop™→PyShop™`·`PyTranslate™`·`PyVoice™`·`PyChat™` 적용(Pi 접두 상표 회피·C-A 공식답변 반영). 원형 유지 예외: DB 코드값(PICAFE/PISHOP/TRANSLATE)·변수/함수/컴포넌트 식별자·Pi 결제 memo(™ 특수문자 결제 호환 위험). 표시명 PyCafé≠코드값 PICAFE 불일치는 의도된 설계. ② **관리자 첫 화면을 `/admin/monitor`로 변경**(커밋 383c188): 로그인 후 모니터링 화면 직행. ③ **메인넷 출시 체크리스트 화면**(`/admin/checklist`·`mainnet_checklist` 테이블·`docs/PRD_23_MAINNET_OPEN.md`). ④ 문서: `docs/MAINNET_READINESS_CHECKLIST.md`·`docs/DEPLOY_STRATEGY.md` 갱신. CLAUDE.md Pi Browser UA 사전차단 금지 규칙·공식 브랜드 표기 규칙 명문화. | asoka |
| v12.1 | 2026-06-26 | **Pi Browser 인증 systemic 실패 복구 + Google 로그인 UI 안정화 + 계정 관리 개선** — ① **Pi Browser 인증 UA 사전차단 사고(8bf8752) 복구**(커밋 53e8340): `signIn()`의 `if (isInPiBrowser())` UA 가드가 실기기 UA 패턴 불일치로 모든 Pi 로그인을 systemic하게 차단하던 버그 제거. UA 기반 사전차단 금지 자칙(`if (!window.Pi)`만)을 CLAUDE.md 핵심 규칙으로 명문화 — 유일 신뢰 신호는 `authenticate()` 성공. ② **Google 로그인 일반 브라우저 UI 세션 버그 수정**: `GET /api/auth/pi`가 `pi_session` 쿠키만 보고 Google 세션 무시 → `getSessionUser()` 폴백 추가로 Google 로그인도 본문 인식. `usePiAuth`를 쓰는 모든 본문 게이트 일괄 해결. ③ **계정 연동 del_yn 토글 추가**(`/admin/links` PATCH): 관리자가 사용자 계정 활성/비활성화(본인 비활성 금지). 커밋 53e8340·054166c·64e5157. | asoka |
...
| v11.8 | 2026-06-23 | **금일 PyShop·Event·브랜드 종합 현행화 (오후 마무리)** — ① **Event 미션 개편**: M1+M2 통합(계정통합+프로필)·M2 'Bean Token 충전' 신설(`bean_charge` 훅)·10미션 완주 보상 1π→**5,000 Bean**(`fn_evt_grant_bean_reward` sql/095·mint+apply 멱등)·과거 충전자 백필(sql/098)·랭킹 페이지네이션/요원명 검색/보상 3단계(미션수행중·보상대기·보상완료). ② **Event#2(매장 온보딩)**: 보상 상태 3단계 통일·완수자 일괄지급 버튼·항목별 참여링크(M1 맵 GPS100m·M2 상품등록·M3 내 PyShop™ 탭·M4 알림). ③ **요원명 검색 서버 pg_trgm 전환**(Event#1·#2, sql/101 nick_nm GIN + 인젝션 방어). ④ **공식 브랜드 표기 확정**: PyCafé™·PyShop™·PyTranslate™ 전면 적용(코드값·Pi 결제 memo 제외). ⑤ **Bean 거래분포 대시보드 i18n** + 보상 출처별 분해. ⑥ **구독현황 버그 수정**(2개 중 1개만 표시 → `/subscriptions/list`). ⑦ **PyShop 로딩 지연 해결**(거리배지 GPS quick 모드, 최대20s→4s/캐시). ⑧ **카테고리 17대분류 표준**(v11.7). ⑨ **X-Frame-Options 긴급 제거**(Pi Browser 접속 차단 복구). | asoká |
| v11.7 | 2026-06-23 | **PyShop™ 상품 카테고리 표준 정립 (`docs/PRD_CATEGORY.md` 신설)** — 샘플성 6대분류(중고편중·식품/뷰티/유아/헬스 등 14도메인 누락·2단계) 폐기, 국내 E커머스 표준 **17대분류 3단계(대>중>소)** 체계 설계. 기존 `mps_ctgr` 재귀(parent_ctgr_id) 활용 — 스키마 변경 0. `sql/105`: 임시테이블(`_old_ctgr`)로 시드 직전 기존 ID 캡처 → 신규 시드(대분류17+중분류) → 기존 상품 미분류(NULL) 재지정 → 기존 카테고리만 논리삭제(물리 DELETE 금지). 소분류는 운영 CRUD/후속 시드 확장. PRD.md 기능현황 #32. ⚠️ `sql/105` Supabase 적용은 마스터. | asoká |
| v11.6 | 2026-06-23 | **Phase 20·21 후속 + DA 거버넌스 중간점검 완료** — ① **Phase 20 EVENT `getEventRanking` 병렬화**: 6개 순차 Supabase 쿼리 → 2단계 병렬(`Promise.all` 4쿼리 동시), 네트워크 왕복 ~40% 감소 (`src/lib/event.ts`). ② **Phase 21 Pi Browser 보안 조치**: CSP(Content-Security-Policy) 완전 제거 — WebView SDK 브릿지(`minepi.com/static/native-ui.js`) 차단 수정; X-Frame-Options 제거 — Pi Browser iframe 임베딩 차단 수정; Google Maps "could not load" 오류 수정(동일 CSP 원인). 보안 헤더는 HSTS·X-Content-Type만 유지 (`vercel.json` commit `01f1a58`). ③ **`docs/PRD_2_SECURITY.md` v1.0→v2.0**: KISA 23개 항목 전체 재분석(21개→23개), 판정 갱신(양호13→18·추가확인6→3·해당없음2). ④ **`docs/보안취약점점검결과표.pptx` 신규**: python-pptx 1.0.2로 자동 생성, 23항목 공식 점검결과표 11슬라이드 63KB. ⑤ **横단7 DA 거버넌스 중간점검**: da-governance-expert 에이전트 — sql/001~101 전수 감사(70+테이블), 전체 준수율 95%→100%. P1: `sql/102` i18n 3테이블(locale·message·country) 시스템 컬럼 4종+논리삭제 추가(Supabase 적용 완료). P2: `sql/103` bean_fee_plan SSGDM↔SSPDM 가격 역전 swap 정정(Supabase 적용 완료). `docs/da/reports/2026-06-23_DA중간점검보고서.md` 작성(21KB, 10섹션). `docs/ROADMAP.md` v11.6·`docs/PRD.md` v12.3·`docs/PRD_2_SECURITY.md` v2.0 동기화. | asoká |
...
| v10.6 | 2026-06-18 | **Phase 18 후속 개선 + 사용자 매뉴얼** — ① 주문 알림 **메뉴별 명칭·수량·금액 라인 표기**(라인 스냅샷+빌더). ② Telegram 연동 카드 **스토어 탭으로 이동**(판매자 전용)·앰버 하이라이트·문구 변경. ③ 프로필 **Google 계정 정보 섹션 제거**. ④ **지도 유입 결제 세션 복구**(결제 직전 silent 재인증, `pi-auth-provider`·`store-item-detail`·`client-cart`). ⑤ **매장별 주인 분리 발송 검증**(수신자=주문 seller_id, 매장 격리). ⑥ **`docs/Cafe.pi_쉬운_사용자매뉴얼.pptx` 신규**(컴맹 기준 7장 — 로그인·Bean·구독·PiCafÃ©â¢ 동시통역·중고장터·PyShopâ¢ O2O·차별점). PRD.md v11.9 동기화. | anakin |
...
| v1.8 | 2026-06-07 | Phase 7~9 PyCafé 로드맵 추가: TASK-050~074 (카페 MVP·수익화·생태계). 마일스톤 M14~M18 추가. PRD.md v4.0 통합 반영. | anakin |
...
| v2.9 | 2026-06-10 | Phase 12 PyTranslateâ¢™ MVP 구현 완료 — TASK-090~097. `sql/020_msg_trans.sql`(018·019 선점으로 번호 조정, Supabase 적용 완료), `chat-translate.ts`(Gemini 2.0 Flash REST + Claude Haiku fallback), `chat-translate-dedup.ts`(pending map + 번역 큐 + broadcast 내장), translate API 라우트, 메시지 POST `after()` 번역 큐 + GET locale pre-populate, `use-chat-room.ts` msg_trans 구독 + 수신 자동 번역 요청, 프로필 표시 언어 드롭다운(203 locale), `translated-message.tsx` 원문 토글. tsc·lint(0 errors)·build 통과. M21 달성. TASK-098(어드민 번역 통계)·099(품질 피드백)는 후속. | anakin |
| v4.0 | 2026-06-10 | Phase 13 PyShopâ¢(MPS) 로드맵 추가 — TASK-100~113 (`PRD_8_MPS.md` v1.1 수용). PiRC2 U2A 가상 에스크로·`stock_qty` 원자적 차감 불변 조건·`mps_` 6개 테이블(sql/021_mps.sql)·lib 헬퍼 3종·API 12종·화면 6종(SCR-01~06) Phase 1 MVP / TASK-108~111 Phase 2 확장 / TASK-112~113 Phase 3 PiRC3 마이그레이션. M22 마일스톤 추가. 현재 버전 헤더 갱신. | anakin |
| v5.2 | 2026-06-11 | Phase 14 PyVoice™ 음성통화 설계 추가 — `docs/PRD_9_VOICE_CHAT.md` v1.0 수용. WebRTC P2P 1:1 MVP, Supabase Realtime 시그널링 재사용(추가 인프라 0), 관리형 TURN으로 시작, 베타 무료. TASK-120~123(데이터모델·TURN발급·시그널링/통화API·WebRTC훅+UI) + S0~S3 Go/No-Go 로드맵. M23 마일스톤 추가. `voice-chat-architect` 에이전트 기준선 반영. | anakin |
| v5.1 | 2026-06-11 | Phase 9 PyCafé 생태계 완료 — TASK-070~074 전체 구현. `sql/022_chat_ecosystem.sql`(msg_theme_follow·msg_bet·msg_bet_optn·msg_bet_entry·msg_webhook + fn_chat_marketplace·fn_room_analytics·fn_room_mau RPC). 마켓플레이스(테마 필터+가중 랭킹+팔로우), Pi Bet(생성·U2A 참가·균등 분배 정산·BET_NOTI), Webhook·봇(API Key 인증·메시지 push·어드민 현황), 분석 대시보드(일별 통계+MAU+plotly), 커스텀 스티커(ownr_usr_id·mkt_yn·노출 규칙). **msg_msg CHECK AI_REPLY 누락 버그 수정**. M18 달성. tsc·lint(0 errors)·build 통과. | anakin |
