援먯껜: [C:\Users\anaki\AppData\Local\Temp\claude\C--Users-anaki-workspace-cafe-pi-claude\2f429599-f5f1-458b-931d-1dff8354b777\scratchpad\roadmap_old3.txt] ?꾩튂=-1 援먯껜: [C:\Users\anaki\AppData\Local\Temp\claude\C--Users-anaki-workspace-cafe-pi-claude\2f429599-f5f1-458b-931d-1dff8354b777\scratchpad\roadmap_old2.txt] ?꾩튂=-1 援먯껜: [C:\Users\anaki\AppData\Local\Temp\claude\C--Users-anaki-workspace-cafe-pi-claude\2f429599-f5f1-458b-931d-1dff8354b777\scratchpad\roadmap_old1.txt] ?꾩튂=7524 ﻿# Pi Network 기반 풀스택 앱 플랫폼 — 개발 로드맵
...
### 통계
- **총 Phase**: 24개 + 횡단7(DA) (0~18 + 횡단 개선·문서화 + Phase 20 성능 + Phase 21 보안 + 횡단7 DA 거버넌스 + Phase 22 분석 + Phase 23 모니터링 + **Phase 24 메인넷 전환·배포 인프라**)
- **완료**: 19개 구현 완료 (Phase 18 판매자 주문 알림 포함 — 2026-06-18) + 횡단7 DA 중간점검 ✅
- **진행 중**: 4개 (Phase 20 화면 성능 최적화 · Phase 21 보안 강화 · Phase 23 실시간 모니터링 · **Phase 24 메인넷 전환 & 2단계 배포 — 운영DB 컷오버 완료·읽기전용 스위치·배포 컨트롤 완성, 메인넷 등재 대기 2026-06-29**)
...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|---------|-------|
| v12.3 | 2026-06-28~29 | **?댁쁺DB 而룹삤踰??꾨즺 + 2?④퀎 諛고룷 ?명봽???꾩꽦 + ?쎄린?꾩슜 紐⑤뱶 + DB ?뺣━** ????**?댁쁺DB 而룹삤踰??꾨즺**(ajdwlcqoljkjamostutc쨌96?뚯씠釉붋?9,280??: pg_dump 諛⑹떇(git replay genesis ?꾨씫?쇰줈 遺덇?), truncate?뭗ata-only ?ㅽ봽?뭩ession_replication_role=replica ?곸옱, ?쇳떚??寃利??꾨즺(cafepi?믪슫?갆B). SERVICE_ROLE_KEY ?⑥젙(service_role ?꾨땶 publishable key ?ъ슜 ??sys_user RLS 0?됤넂?몄뀡 誘몄씤吏) ?댁냼. ??**?댁쁺DB ?쎄린?꾩슜 ?ㅼ쐞移?+ ?곌린 媛??*: `SUPABASE_READONLY_MODE` env + `src/lib/db-env.ts` ?먯젙, ?쎄린?꾩슜 JWT 諛쒓툒 ?꾩슜 ?ㅽ겕由쏀듃(`scripts/issue-readonly-jwt.mjs`) + Pi/Google 濡쒓렇???몄뀡 ?좎?(荑좏궎/X-Pi-Token 蹂댁〈). ??**諛고룷 而⑦듃濡?2??遺꾨━**: `/admin/deploy` Stage/?댁쁺 蹂꾨룄 踰꾪듉 + Staging DB ?ㅼ쐞移?愿由ъ옄 ?붾㈃ + 吏꾪뻾?곹깭 ?대쭅 UI. ??**DB ?뺣━**: ?몃? ?뚯씠釉?12醫?DROP(`sql/134`, cafepi 臾닿? ?덇굅?? + i18n ?덇굅???뚯씠釉?2醫?+ ?곕뱶 ?⑥닔 ?쒓굅. ??**DB ?곌껐 ?쇱씠釉?吏꾨떒** API(`/api/admin/db-health`): ?섍꼍蹂??곌껐 ?곹깭 ?ㅼ떆媛??뺤씤. 而ㅻ컠: c62dd43쨌7c6a7be쨌808ee82쨌1f40095쨌200920f쨌4f3fc9e쨌1e17467. Phase 24 ?꾨즺??媛깆떊 | asok찼 |
| v12.2 | 2026-06-27 | **Py 媛쒕챸 ?꾨즺 + 愿由ъ옄 ?붾㈃ 媛쒖꽑** ????**Pi?뭁y 怨듭떇 釉뚮옖???꾪솚**(而ㅻ컠 fa19122): ?쒖떆 ?띿뒪???꾩껜 `PiCaf챕?™넂PyCaf챕??, `PiShop?™넂PyShop??, `PyTranslate??, `PyVoice??, `PyChat?? ?곸슜(Pi ?묐몢 ?곹몴 ?뚰뵾쨌C-A 怨듭떇?듬? 諛섏쁺). ?붿젣???먰삎 ?좎?: DB 肄붾뱶媛?PICAFE/PISHOP/TRANSLATE)쨌蹂?샕룻븿?섎챸쨌Pi 寃곗젣 memo(???뱀닔臾몄옄 寃곗젣 ?명솚 ?꾪뿕). ?쒖떆紐?PyCaf챕?좎퐫?쒓컪 PICAFE 遺덉씪移섎뒗 ?섎룄???ㅺ퀎. CLAUDE.md 怨듭떇 釉뚮옖???쒓린 ?뱀뀡 異붽?. ??**愿由ъ옄 泥??붾㈃ ??/admin/monitor 蹂寃?*(而ㅻ컠 383c188): 濡쒓렇????紐⑤땲?곕쭅 ?붾㈃ 吏곹뻾. ??**硫붿씤??異쒖떆 泥댄겕由ъ뒪???붾㈃**(`/admin/checklist`, `mainnet_checklist` ?뚯씠釉? `docs/PRD_23_MAINNET_OPEN.md`). ??臾몄꽌: `docs/MAINNET_READINESS_CHECKLIST.md`쨌`docs/DEPLOY_STRATEGY.md` 媛깆떊. CLAUDE.md Pi Browser UA ?ъ쟾李⑤떒 湲덉? 洹쒖튃쨌怨듭떇 釉뚮옖???쒓린 洹쒖튃 紐낅Ц???꾨즺. | asok찼 |
| v12.1 | 2026-06-26 | **Pi Browser ?몄쬆 systemic ?ㅽ뙣 蹂듦뎄 + Google 濡쒓렇??UI ?덉젙??+ 怨꾩젙 愿由?媛쒖꽑** ????**Pi Browser ?몄쬆 UA ?ъ쟾李⑤떒 ?ш퀬(8bf8752) 蹂듦뎄**(而ㅻ컠 53e8340): `signIn()` ??`if (isInPiBrowser())` UA 媛?쒓? ?ㅺ린湲?UA ?⑦꽩 遺덉씪移섎줈 紐⑤뱺 Pi 濡쒓렇?몄쓣 systemic?섍쾶 李⑤떒?섎뜕 踰꾧렇 ?쒓굅. UA 湲곕컲 ?ъ쟾李⑤떒 湲덉? ?먯튃(`if (!window.Pi)`留? CLAUDE.md ?듭떖 洹쒖튃?쇰줈 紐낅Ц?? ??**Google 濡쒓렇???쇰컲 釉뚮씪?곗? UI ?몄뀡 踰꾧렇 ?섏젙**: `GET /api/auth/pi`媛 `pi_session` 荑좏궎留?蹂닿퀬 Google ?몄뀡 臾댁떆 ??`getSessionUser()` ?대갚 異붽?濡?Google 濡쒓렇?몃룄 ?몄떇. `usePiAuth`瑜??곕뒗 紐⑤뱺 蹂몃Ц 寃뚯씠???쇨큵 ?닿껐. ??**怨꾩젙 ?곕룞 del_yn ?좉? 異붽?**(`/admin/links` PATCH): 愿由ъ옄媛 ?ъ슜??怨꾩젙 ?쒖꽦/鍮꾪솢?깊솕 媛??蹂몄씤 鍮꾪솢??湲덉?). ??`src/lib/auth-check.ts`, `src/auth.ts`, `src/lib/users.ts`, `src/types/next-auth.d.ts` 媛쒖꽑. 而ㅻ컠: 53e8340쨌054166c쨌64e5157. | asok찼 || v12.0 | 2026-06-25 | **Phase 22 데이터 분석 & 시각화 기획 + 매장주 후기 동의·평가항목 시드 구현** — ① **4-탭 통합 분석 페이지 구상**(`docs/PRD_21_DATA_ANAL.md` v1.1 §12 신설): 6개 분석 도메인→4탭(매출·주문·접속/사용·퍼포먼스, `/admin/analytics` 허브). 북극성(활성 사용자) 배너 전 탭 고정·Pi/Bean 2층위 매출 분리·4-Zone 공통 레이아웃·Plotly 표준(`useThemeChartColors`). 기존 통계 인프라(stat_*_dly·차트 6종·API 7종) 확장. 신규 집계 제안 sql/122(코호트)·123(RFM) 즉시 / sql/124(세션 PV)·125(퍼널) 추적 선결. ② **매장주 후기·Bean 보상 동의(opt-in)**(`mps_shop.fbck_consent_yn` sql/117): 매장 등록/수정 동의 토글, 동의(Y) 매장 상품만 후기 버튼 노출(client-my-orders)+서버 거부(feedback POST 이중 방어). ③ **후기 평가항목 카테고리 전체 시드**(sql/118: mps_ctgr 93개 카테고리×5항목 멱등) + 이름기준 동적 시드(sql/116)·부모 카테고리 fallback(api/feedback/items). PRD.md 섹션 22 신설·기능현황 #33·섹션 재번호(22→26). 커밋 8a566d8·f1ad049. ⚠️ Supabase 적용 대기: sql/117·118(마스터). | asoka |
