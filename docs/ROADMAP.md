...
- ✅ `src/components/admin/admin-sidebar.tsx` — 4개 메뉴 네비게이션 (대시보드/사용자관리/결제내역/계정연동현황)
...
- [ ] **TASK-021**: 게시글 CRUD API
  - GET 목록 (`/api/board/[category]`) — 페이지네이션, 검색(제목+내용), 핀 우선 정렬
  - GET 상세 (`/api/board/[category]/[postId]`) — 조회수 increment
  - POST 작성 / PATCH 수정 / DELETE 논리삭제
- [ ] **TASK-022**: 댓글 + QNA 채택 API
  - POST/DELETE 댓글 (`/api/board/[category]/[postId]/comments`)
  - POST QNA 채택 (`/api/board/qna/[postId]/accept`)
- [ ] **TASK-023**: 첨부파일 API (Supabase Storage)
  - POST 업로드 (20MB/5개 제한), GET 다운로드, DELETE
- [ ] **TASK-024**: UI 구현
...
| M8: 통합 게시판 | Phase 4 | — | 4종 게시판 CRUD | 🔄 진행 중 |
