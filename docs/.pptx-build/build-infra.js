...
function pageNum(s, n) {
  ...
  s.addText('cafe.pi · Infrastructure', {
    x: 0.6,
    y: 6.98,
    w: 5,
    h: 0.35,
...
;(() => {
  ...
  s.addShape(pres.shapes.OVAL, {
    ...
    w: 6,
    h: 6,
    fill: { color: C.purple, transparency: 55 },
    line: { type: 'none' },
  })
  s.addShape(pres.shapes.OVAL, {
    x: 11.4,
  ...
  s.addText('CAFE.PI · INFRASTRUCTURE STRATEGY', {
    x: 0.8,
    y: 2.0,
    w: 11,
    h: 0.4,
  ...
  s.addText('DEV  →  STAGING  →  PRODUCTION', {
    x: 0.85,
    y: 4.7,
    w: 11,
    h: 0.5,
  ...
  s.addText(
    '단일 DB·스테이징 부재 현황을 3환경 완전 분리로 — 안전한 마이그레이션·배포 거버넌스',
    {
      x: 0.85,
      y: 5.5,
      w: 11,
...
;(() => {
  ...
  chevron(s, 8.55, 3.7)
  s.addText(
    '⛔  DB는 환경마다 완전히 분리 — 절대 공유 금지   |   하위 환경의 사고가 운영에 닿지 않게',
    {
      x: 0.6,
      y: 6.5,
      w: 12.1,
  ...
  s.addNotes(
    '테스트넷=Dev/Staging, 메인넷=Prod. 테스트넷 검증은 메인넷에 승계되지 않으며, 운영(메인넷)은 별도 환경을 새로 구축해 검증·오픈한다.',
  )
  pageNum(s, 3)
})()
...
;(() => {
  ...
  header(
    s,
    'Deep-dive · 스테이징 DB 스위칭',
    'Edge Config 무재배포 전환 + 운영DB 읽기 전용',
  )
  ...
  s.addText('⚡  Vercel Edge Config — 스위치 컨트롤러', {
    x: 3.45,
    y: 1.66,
    w: 6.4,
    h: 0.4,
  ...
  s.addText('stagingDb = dev ⟷ prod   ·   무재배포 · 전 리전 ~5ms 반영', {
    x: 3.45,
    y: 2.06,
    w: 6.4,
    h: 0.4,
  ...
  s.addText(
    '⛔  운영DB로의 쓰기 경로 없음   ·   운영 환경 DB는 스위칭 불가(고정)   ·   마이그레이션 DDL은 Staging DB에서만',
    {
      x: 0.6,
      y: 6.35,
      w: 12.13,
  ...
  s.addNotes(
    '스테이징은 평소 개발DB(RW)로 안전하게 테스트하고, 필요 시 Edge Config 플래그로 운영DB를 읽기 전용으로만 연결한다. 운영 쓰기·DDL 경로는 원천 차단.',
  )
  pageNum(s, 4)
})()
...
;(() => {
  ...
  s.addText('db diff → db push  /  Branching', {
    x: 4.6,
    y: 2.92,
    w: 4.04,
    h: 0.36,
  ...
  chip(
    ...
    '✅',
    'CDC가 진짜 필요한 곳',
    '분석 웨어하우스(ETL/Pipelines) · 라이브 UI(Realtime). dev-prod 미러링용 아님',
    C.green,
  )
  chip(
    6.88,
    '⛔',
    'CDC ≠ dev-prod 동기화',
    '운영 PII 연속 복제 금지 · Kafka CDC는 과하고 부적합',
    C.red,
  )
  ...
  s.addText(
    '스키마 = 마이그레이션(↑)   ·   데이터 = 마스킹 스냅샷(↓)   ·   Supabase Branching + CLI면 충분 (Kafka 불필요)',
    {
      x: 0.6,
      y: 6.4,
      w: 12.13,
  ...
  s.addNotes(
    '스키마는 CDC가 아니라 마이그레이션으로 개발→운영. 데이터는 연속 CDC가 아니라 마스킹 스냅샷으로 운영→개발(PII 원본 금지). CDC(ETL/Realtime)의 본래 용도는 분석·라이브이지 환경 미러링이 아니다.',
  )
  pageNum(s, 5)
})()
...
;(() => {
  ...
  lane(2.0, '코드 (git)', C.purple, [
    'feature/*\n(개발)',
    'staging\n(PR·리뷰·검증)',
    'master\n(운영 배포)',
  ])
  lane(3.55, 'DB (sql/NNN)', C.blue, [
    'Dev 적용\n검증',
    'Staging 적용\n검증',
    'Prod 적용\n(최종)',
  ])
  ...
  s.addText(
    [
      {
        text: '운영(Prod)에 먼저 적용 절대 금지 — 항상 Dev→Staging에서 검증 후 승격',
        options: {
          bullet: { code: '2022' },
          breakLine: true,
          color: C.white,
      ...
      {
        text: 'sql/NNN 번호·순서 유지 (DA 표준·da-ddl-guard) — 같은 마이그레이션을 각 환경에 순차 적용',
        options: {
          bullet: { code: '2022' },
          breakLine: true,
          color: C.white,
      ...
      {
        text: '롤백 = Vercel revert(코드) + 보상 마이그레이션(DB) · 물리 DELETE 금지(논리삭제)',
        options: { bullet: { code: '2022' }, breakLine: true, color: C.white },
      },
    ],
...
;(() => {
  ...
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    ...
    rectRadius: 0.1,
    fill: { color: C.white },
    line: { color: C.green, width: 1.25 },
    shadow: shadow(),
  })
  s.addText(
    '💡  Supabase 무료 플랜 = 활성 프로젝트 보통 2개 → 이렇게 배치하면 무료로 3환경',
    {
      x: 0.95,
      y: 2.05,
      w: 11.4,
  ...
  s.addText(
    [
      {
        text: '확장 옵션 (트래픽·팀 성장 시):  Supabase Pro($25/월) + Branching(Git연동 preview DB)로 스테이징 자동화',
        options: {
          bullet: { code: '2022' },
          breakLine: true,
          paraSpaceAfter: 9,
      ...
      {
        text: 'Vercel:  환경 스코프(Development/Preview/Production)는 이미 쓰는 Pro에 포함 — 추가 비용 0',
        options: {
          bullet: { code: '2022' },
          breakLine: true,
          paraSpaceAfter: 9,
      ...
      {
        text: '주의:  무료 티어는 프로젝트 수·용량 제한이 바뀔 수 있으니 도입 직전 현행 한도 확인',
...
;(() => {
  ...
  const steps = [
    [
      '1',
      '운영 동결',
      '현재 단일 DB를 ‘운영(Prod)’으로 확정 · 직접 실험 금지',
      C.purple,
    ],
    [
      '2',
      'Staging 신설',
      'Supabase Staging 프로젝트 + sql/NNN 재생으로 스키마 복제 + 합성 데이터',
      C.amber,
    ],
    ...
    [
      '4',
      '스코프 분리',
      'Vercel env 3분리 — 각자 자기 DB·Pi 설정 가리키게',
      C.green,
    ],
    [
      '5',
      '파이프라인',
      '마이그레이션 규율 확립: Dev→Staging→Prod 순차 적용',
      C.deep,
    ],
    [
      '6',
      '메인넷 컷오버',
      '(출시 시점) Prod = Mainnet 전환 — 새 도메인·API Key·지갑',
      C.goldDk,
    ],
  ]
  let y = 1.85
...
;(() => {
  ...
  s.addText('cafe.pi · 인프라 환경 전략 · 2026-06-26', {
    x: 0.8,
    y: 6.9,
    w: 11,
    h: 0.35,
