...
| 항목 | 상태 | 우선순위 | 비고 |
|---|---|---|---|
| **요청 계측 구현** | ✅ 완료 | 1순위 | route 래퍼 방식으로 구현(`src/lib/monitor-metric.ts` — Next.js middleware는 응답 status·소요시간을 볼 수 없어 미들웨어 대신 래퍼 채택). `api-guard.ts withGuard`에 배선 — 인증·결제 등 고위험 9개 엔드포인트 계측 중 |
| **metric_* 테이블 생성** | ✅ 완료 | 1순위 | `sql/125` — `sys_metric_req_perf`·`req_ip`·`auth`·`conn` 4종 (DA 승인) |
| **Vercel Analytics API 통합** | ❌ 미착수 | 2순위 | VERCEL_TOKEN 환경변수 + 권한 확인 필요 |
| **SSE 엔드포인트** | 🔁 폴링으로 대체 | — | 5초 폴링(`/api/admin/monitor`)으로 확정(§9.3). SSE는 서버리스 커넥션 유지 비용 문제로 후속 검토 |
| **시각화 컴포넌트** | ✅ 완료 | 2순위 | 경량 SVG 스파크라인 자체 구현(`monitor-dashboard.tsx` — 실시간 폴링에 Plotly보다 적합) |
| **RPC 함수 (fn_*)** | ⚠️ 부분 완료 | 1순위 | `fn_pi_payment_status`·`fn_monitor_active_orders` 구현(sql/125). `fn_api_response_stats`·`fn_suspicious_ips`는 후속 |
| **계측 보존 정리** | ✅ 완료 | 1순위 | `sql/170 fn_metric_purge`(7일) + cron `/api/cron/metric-purge`(매일 02:30) — 무한 증가 방지 |
...
### 9.3 설계 의사결정 (2026-07-08 확정)

| 항목 | 결정 | 근거 |
|---|---|---|
| **메트릭 저장 정책** | ✅ 전량 저장 (샘플링 안 함) — **2026-07-09 마스터 최종 확정** | 계측 범위가 고위험 9개 엔드포인트로 한정돼 볼륨이 낮음. 샘플링 10%는 "전 요청 계측" 전제의 추천이었고, 좁은 범위에선 p95·에러율(특히 ⭐Pi 결제 성공률 신호등) 표본을 훼손함. 총량 상한은 보존 7일이 담당. 운영 정책 3건(전량·7일·5초) 전부 마스터 확정 완료 |
| **메트릭 보존 기간** | ✅ 7일 | `sql/170 fn_metric_purge(7)` + cron `metric-purge`(매일 02:30 물리 정리, DA 예외 승인). 실시간 헬스 목적엔 7일이면 충분 — 장기 추세는 분석(PRD_21) 영역 |
| **실시간 갱신 주기** | ✅ 5초 폴링 | 1초는 서버리스 invocation 5배 비용 대비 관리자 UX 이득이 미미. 구현(`REFRESH_MS=5000`)과 일치. SSE 전환은 후속 검토 |
| **Supabase Pro 전환 시점** | ⏸ 미확정 (외부 조건) | 커넥션 limit 도달 시 — 사용량 추적 지속 |
...
| **4대 카드 정상 표시** | 모든 메트릭 값 로드 | ✅ (API 성능 카드는 계측 유입분 표시 — 무유입 시 "수집 대기 중") |
| **실시간 갱신** | 5초 폴링 업데이트 (§9.3 확정) | ✅ |
| **Pi 결제 성공률** | 🟢 / 🟡 / 🔴 신호등 작동 | ✅ (미완료>5건 또는 <95% 위험) |
| **경고 배너** | 임계치 초과 시 표시 | ✅ (미완료 결제 >5건 배너) |
| **데이터 일관성** | Vercel·Supabase·앱 메트릭 오차 < 5% | 🔍 검증 대기 (Vercel Analytics 미통합) |
...
| v1.1 | 2026-07-08 | 구현 현행화(§9.1 — 계측·테이블·대시보드 완료 반영), 설계 의사결정 3건 확정(§9.3 — 전량 저장·보존 7일·5초 폴링), 보존 정리 cron 신설(sql/170·metric-purge), MVP 체크 갱신(§10.1) | 아소카 |
