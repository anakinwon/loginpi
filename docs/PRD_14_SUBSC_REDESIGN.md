...
| 축 | 현행(파기) | 신규(본 설계) |
|---|---|---|
| 단위 | 사용자 등급 1개 (Explorer/Creator/Host) | **상품군별 독립 구독** (PiCafe·PiStore·자동번역), 동시 다중 |
...
|---|---|---|---|---|
| ☕ PiCafe 구독 | 단일 | 3,000 / 30π | 30,000 / 300π | 카페 생성·입장 건당요금 면제 |
| 🏪 PiStore 구독 | S (상품 30개↓) | 3,000 / 30π | 30,000 | 노출·생성 건당요금 면제 |
| 🏪 PiStore 구독 | M (상품 50개↓) | 4,000 / 40π | 40,000 | |
| 🏪 PiStore 구독 | L (상품 50개↑) | 5,000 / 50π | 50,000 | |
...
CREATE TABLE public.bean_subscr (
  ...
  prod_ctgr_cd  VARCHAR(16) NOT NULL,                  -- PICAFE / PISTORE / TRANSLATE (구독 대상 상품군)
  grade_cd      VARCHAR(10) NOT NULL DEFAULT 'GENERAL',-- PiStore: S/M/L, 그 외 단일
...
| 진입경로 | 프로필 구독현황 / PiStore 매장관리 / 카페·번역 기능 게이트 / 만료알림 |
...
**차별 UX:** (a) 등급카드 아닌 **상품카드**, (b) 각 카드에 **before→after 면제 증거**, (c) PiStore **상품 수 기반 등급 자동추천**(`mps_item` 카운트), (d) 확장: "이번 달 낸 건당요금 X→구독 시 Y" **ROI 배너**.
