...
**Cafe.pi 구독 시스템** — PiCafe(채팅 커뮤니티)와 PiStore(P2P 마켓플레이스)의 월간·연간 구독 요금제
...
> **이유**: 기존 인앱 "Pi Bean"(원두, 팁 단위)과의 자연스러운 연결. BEAN 토큰 발행 후 onchain Bean과 offchain Bean이 1:1로 연동될 예정.

### 2-2. 시세 가정 (내부 설계 기준용, 사용자 비노출)

**⚠️ 다음은 요금제 설계 근거일 뿐, 사용자에게 노출되지 않습니다.**

- **설계 기준**: 1 Pi ≈ 100,000 KRW (현황 가정치)
- **기준 월 가격**: 10,000 KRW = 0.1 Pi = 10 Bean (설계 단위)
...
### 2-3. Bean 표기 정책 (단위 정의)

| 항목 | 명칭 | 발행 상태 | 사용자 표기 | 시스템 참고 |
|---|---|---|---|---|
| **Bean** (BEAN 토큰) | 온체인 Bean | Phase 17 계획 (미발행) | 미래가: "발행 후 적용" | 토큰 발행 후 결제 수단 추가 |
| **Pi Bean** (팁 단위) | 오프체인 Bean (인앱) | 현재 운영 중 | "~ Pi Bean" 또는 "~ Bean" | `msg_tip`, `user_balance_bean` 등 |

**표기 규칙**:
- 구독 요금제: "월 1 Pi (**100 Bean**)" — 토큰 발행 후 계산 명확화
- 결제 화면: Pi·Bean 둘 다 표기하되, 실제 결제는 Pi로만 진행
- 토큰 발행 후: Bean 직접 결제 지원 (단, 환율 고정 1 Pi=100 Bean 유지)
...
**예시** (설계 가정: 1 Pi = 100,000 KRW):

| 요금제 | 월 기준가(KRW) | Pi 환산 | Bean 환산 |
|---|---|---|---|
| PREMIUM | 10,000 | 0.1 Pi | 10 Bean |
| BUSINESS | 50,000 | 0.5 Pi | 50 Bean |
...
| **티어** | PREMIUM |
| **월간 가격** | **1 Pi** / 100 Bean |
| **연간 가격** | **10 Pi** (2개월 무료) / 1,000 Bean |
| **갱신 주기** | 월 / 연 (자동갱신) |
| **취소 정책** | 만료일까지 이용 가능, 즉시 환불 불가(PiRC2 구조) |
| **설명** | 전문가용 플랜 — PREMIUM 테마 무제한, 그룹방 생성 무제한, AI 비서 월 10회 이용, Pi Bean 전송 가능 |
...
| **티어** | BUSINESS |
| **월간 가격** | **5 Pi** / 500 Bean |
| **연간 가격** | **50 Pi** (2개월 무료) / 5,000 Bean |
| **갱신 주기** | 월 / 연 (자동갱신) |
| **취소 정책** | 만료일까지 이용 가능, 즉시 환불 불가(PiRC2 구조) |
| **설명** | 운영자용 플랜 — 무제한 그룹방/이벤트방 생성, AI 비서 무제한, 분석 대시보드, Webhook/봇 API, 우선 지원 |
...
| **요금제명** | PiShop Seller |
| **티어** | PREMIUM |
| **대상 사용자** | 오프라인/온라인 매장 운영 판매자 |
| **월간 가격** | **0.5 Pi** / 50 Bean |
| **연간 가격** | **5 Pi** (2개월 무료) / 500 Bean |
| **자동갱신** | Y (만료 시 구독 해제 또는 재갱신 선택) |
| **취소 정책** | 만료일까지 이용, PiRC2 구조로 즉시 환불 불가 |
| **설명** | 판매자 전문가 플랜 — 매장 통계 대시보드, 상품 무제한 등록, 다중 매장(최대 3개) 관리, 우선 노출(검색 상위), 판매자 배지 |
...
### 7-2. PiStore 판매자 기능 매트릭스

| 기능 | 기본(FREE) | PiShop Seller |
|---|---|---|
| 매장 개설 | 1개 | 최대 3개 |
| 상품 등록 수 | 제한(50개) | 무제한 |
| 매장 통계 대시보드 | ❌ | ✅ |
...
| 판매자 배지 | ❌ | ✅ |
| 마케팅 도구 | ❌ | ✅ |
| 거래 수수료 | 0.01 Pi (네트워크) | 0.01 Pi (네트워크) |
| 판매자 보증금 | 선택(1 Pi) | 선택(1 Pi) |

---

## 8. 결제 연동 고려사항

### 8-1. Pi Payment (U2A 3단계)
...
2. **2단계 (대기)**: 서버 → Pi Browser U2A 결제 API 호출
   ...
   - `amount`: Pi 단위 (예: 1000000 stroops = 0.01 Pi)
...
**현재**: 1 Pi = 100,000 KRW (설계 가정)
**미래**: 1 Pi = 120,000 KRW (시세 변동 가정)
...
**Step 2**: 설계 기준 업데이트
- 본 PRD 2-2절 "시세 가정" 갱신
- 예: `1 Pi ≈ 120,000 KRW (2026-07-15 갱신)`

**Step 3**: 의사결정 필요 여부 판단
- **Case A** (가격 유지): Pi 시세 오르면 → 사용자 입장에선 더 저렴해짐 (추가 결정 불필요)
- **Case B** (가격 조정): 특정 원화 목표가(예: "항상 월 10,000원 수준")를 유지하려면 → Pi 금액 재산정 필요 → 아나킨 마스터님 결정 대기
...
| 버전 | 갱신일 | Pi 시세 | 비고 |
|---|---|---|---|
| v1.0 | 2026-06-18 | 1 Pi ≈ 100,000 KRW | 최초 설계 |
...
- **투자 권유성 배제**: "BEAN 토큰은 투자 상품이 아니며, 카페 내 사용권 및 보상 수단입니다" 명시
- **가격 표시 명확성**: 모든 가격은 Pi 기본 단위, 센트 이상 정밀도 미사용 (혼란 방지)
- **결제 투명성**: 결제 전 "월 1회 갱신 예정" 등 갱신 주기 명시

---

## 12. DB 시드 초안 (승인 대기)

### 12-1. msg_subscr_plan 신규 데이터 (PiStore 플랜)

**⚠️ 본 섹션은 초안이며, 실제 DB 적용은 아나킨 마스터님 승인 후 진행합니다.**

```sql
-- PiStore 판매자 구독 플랜 추가 (신설)
...
(
  ...
  'PiShop Seller 연간',
  'PISTORE 판매자 연간 구독 — 2개월 무료',
  'PISTORE',
  '5.0000',            -- 연간 5 Pi = 500 Bean
  12,                  -- 연간
  'Y',
  'N',
  'ADMIN',
...
| 항목 | 결정 |
|---|---|
| **PiCafe 구독** | 현행 가격 유지 (FREE/PREMIUM 1Pi/BUSINESS 5Pi) |
| **Bean 환산** | 1 Pi = 100 Bean (고정) |
| **연간 할인** | 월간 × 10 = 연간 (2개월 무료, 16.7% 할인) |
| **기간 표기** | 월간/연간 명시 (현행 구조 유지) |
| **PiStore 신설** | PiShop Seller 월 0.5Pi / 연 5Pi (승인 대기) |
| **사용자 노출** | Pi·Bean만 (원화 비노출) |
...
| 용어 | 정의 |
|---|---|
| **Pi** | Pi Network의 공식 암호자산 단위 |
| **Bean** (온체인) | Cafe.pi 생태계 토큰 (Phase 17 발행 예정) |
| **Pi Bean** (오프체인) | 현재 운영 중인 인앱 팁 단위 (앱 내부 통화) |
| **Stroops** | Pi의 최소 단위 (1 Pi = 10,000,000 stroops) |
| **U2A** | User-to-App 결제 (Pi Browser 내 결제 API) |
| **PiRC2** | Pi Network 반복 결제 스마트 컨트랙트 |
