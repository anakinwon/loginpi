...
> **채팅 상세 스펙**: `docs/PRD_4_CHAT.md` (v1.6)
...
Pi 계정 인증·결제, Google 소셜 로그인, 계정 연동, 관리자 시스템, 게시판, 다국어, **테마 기반 채팅 플랫폼 PiChat**, 그리고 **Pi Coin 전용 P2P 직거래 마켓플레이스 MyPiShop(MPS)**를 구현한다.
...
| 실시간 | Supabase Realtime (채팅 broadcast + 번역 완료 알림 — Phase 7~12) |
| 결제 | Pi Coin (U2A) |
| 다국어 | next-intl v4 (18개 언어 + AI 자동번역) |
| 채팅 | PiChat — 테마 기반 Pi Network 커뮤니티 채팅 + PiTranslate™ 글로벌 동시통역 (Phase 7~12) |
...
| 10 | PiChat MVP — 1:1·그룹 채팅 + 테마 선택 + Pi 결제 | ✅ 완료 | Phase 7 |
| 11 | 사용자 프로필 — 마이페이지 (개인정보·결제내역·구독현황) | ✅ 완료 | Phase 10 |
| 12 | 어드민 통계 대시보드 — DAU/WAU/MAU·테마별 매출 (react-plotly.js) | ✅ 완료 | Phase 11 |
| 13 | PiChat 수익화 — Pi Tip·스티커·AI 봇·이벤트방 | 🔜 준비중 | Phase 8 |
| 14 | PiChat 생태계 — 마켓플레이스·Webhook·대시보드 | 🔜 준비중 | Phase 9 |
...
| 5 | **PiTranslate™** | 어떤 언어로 채팅해도 선택 언어로 실시간 동시통역 — Gemini 2.0 Flash + Claude Haiku 하이브리드 (비용 ~76% 절감) |
...
**AI 채팅 비서** (`@ai` 멘션 → Anthropic Claude Haiku):
...
| TASK-096 | 채팅 UI 번역 토글 + 사용자 표시 언어 설정 (203개 locale) |
...
| 매출 유형 | 결제 경로 | 테마 |
|---|---|---|
| 채팅방 | `msg_room.pymnt_id` | `msg_room.theme_cd` |
