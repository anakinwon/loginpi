...
| 17 | PiVoice™ — WebRTC 실시간 1:1 음성 통화 (P2P·TURN·시그널링 재사용) | 🔜 준비중 | Phase 14 |
| 18 | LBS 위치기반서비스 — 동의 기반 위치 수집·주변 탐색·MPS 거리 표시 (직거래 성사율 향상) | 🔜 준비중 | Phase 15 |
...
> **sys_user 컬럼 추가**: `lbs_consent_yn CHAR(1) DEFAULT 'N'`, `lbs_consent_dtm TIMESTAMPTZ`, `lbs_consent_ver TEXT` — `sql/030_lbs.sql`
> **설계 결정**: 매장 위치(`loc_tp_cd='03'`)는 `mps_shop.lat/lng` 재활용 — 이중 저장 금지
