...
## 1.6. 🔐 환경 플래그 & 데이터 무결성 원칙 (2026-07-02 확정)

> 2026-07-02 Pi Browser 재동의·계정 무한 재생성 사고에서 도출된 확정 규격. 사고 경위·진단은 `docs/TROUBLESHOOT.md` [2026-07-02].

### SANDBOX_FLAG (`NEXT_PUBLIC_PI_SANDBOX`) — 환경별 **고정값**

Pi의 uid는 **(앱, Testnet/Mainnet 환경) 쌍마다 다른 scoped 값**이다. 이 플래그가 그 환경축을 결정하며, **로그인 uid·결제가 전부 여기에 종속**된다. `detectSandbox()`는 localhost를 `true`로 강제하고, 그 외에는 오직 `NEXT_PUBLIC_PI_SANDBOX==='true'`로 판정한다.

| 값 | Pi 환경 | 로그인 uid | 결제 |
|---|---|---|---|
| `true` | Pi Testnet(샌드박스) | 테스트넷 scoped | 테스트 Pi(가짜) |
| `false` / 미설정 | Pi Mainnet | 메인넷 scoped | 실제 Pi |
| (localhost) | 항상 Testnet(코드 강제) | — | — |

| 환경 | 값 | 비고 |
|---|---|---|
| 로컬(dev) | 자동 `true` | 설정 불필요 |
| **staging** | **`true`** | Testnet — 실제 돈 없이 검증 |
| **운영(cafepi)** | **`false`** | **Mainnet 확정(2026-07-02)** |

**철칙**: ① **환경 내 고정** — 바꾸면 전 사용자 pi_uid가 재발급되어 `upsertPiUser`(onConflict:pi_uid)가 전원 신규 계정 생성 → systemic 재동의·계정 재생성. ② **변경 시 재배포 필수** — `NEXT_PUBLIC_*`는 빌드 인라인이라 env만 바꾸면 미반영. ③ **메인넷 전환은 세트 작업** — 운영 DB 클린 초기화 + 계정 마이그레이션 계획과 동반. ④ **삭제로 해결 불가** — 플래그가 원인이면 계정을 지워도 재생성된다.

### 실 데이터 무결성 — 테스트 더미 절대 금지 ⛔

운영·staging 등 **실 환경에 테스트/더미 사용자·거래·자료를 생성하지 않는다.** 특히 Pi Network에서 **가짜 인간(다중·허위 계정)으로 대신 테스트하는 것은 KYC·1인1계정 정책 위반 → Pi 계정 영구제명** 위험이다(2026-07-02 확정). 계정이 제명되면 핵심가치(Pi Browser 로그인·결제)와 메인넷 등재가 통째로 붕괴한다. 검증은 **실사용자 행위로만**, 통계·장부 소급도 **소스가 실재하는 건만**(임의값·추정 삽입 금지) — `docs/TROUBLESHOOT.md` A 및 "돈·데이터 품질 무양보" 원칙의 연장.
