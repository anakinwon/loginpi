...
| 21 | 이벤트 미션 시스템 (Pi 요원 육성) — 10미션 게이미피케이션·자동감지·화이트리스트·sum 랭킹·선착순 10명 카카오 선물 | 📋 기획 완료 (PRD v1.2) | Phase 16 |
...
## 15. Phase 13 — PiShop (MPS) ✅ Phase 1+2 완료 (2026-06-13) · Phase 3 PiRC3 보류·Maps 예정
...
## 18. Phase 16 — 이벤트 미션 시스템 (Pi 요원 육성) 📋 (기획 완료 — 2026-06-14)

> 상세 요구사항: **`docs/PRD_11_EVENT.md` (v1.2)**
...
### 18.2 10가지 미션 (트리거 실증 기반)
| # | 미션 | 트리거 |
|---|---|---|
| M1 | 계정 통합 (Pi + Google) | `api/auth/link-complete` |
| M2 | 별명 + 카카오톡 ID 입력 | `api/profile` PATCH |
| M3 | PREMIUM 카페 생성 + 자동번역 | `api/chat/rooms` + translate |
| M4 | Pi Bet 생성 + 분배 | `api/chat/rooms/[id]/bets` |
| M5 | Bean(팁) 1인 이상 전송 | `api/tips` |
| M6 | 스티커 + 파일 + 음성채널 | voice/join + file + sticker |
| M7 | PiShop 판매자 거래 취소 | `api/store/orders/[id]/cancel` (판매자) |
| M8 | PiShop 구매자 거래 취소 | `api/store/orders/[id]/cancel` (구매자) |
| M9 | 판매자 보증금 1π + 위치동의 (선행) | `api/store/bond` + `api/location/consent` |
...
### 18.6 상태 및 확인 필요 항목
- 📋 **기획 완료** (PRD_11_EVENT.md v1.2) · 구현 대기
- ⚠️ **확인 필요**: 선착순 기준(전체 미션 완료 vs 1개 이상) 확정, M10 완료 판정 로직(M9 완료 시각 이후 취소·중복 기록 정책)
