...
> **사용자 매뉴얼(쉬움)**: `docs/Cafe.pi_쉬운_사용자매뉴얼.pptx` — 컴맹 기준 7장(로그인·Bean·구독·PiCafÃ©â¢ 동시통역·중고장터·PiShopâ¢ O2O·차별점)
> **구독 요금제**: `docs/PRD_14_SUBSC.md` (v1.0) — PiCafÃ©â¢(Explorer·Creator·Host) + PiShopâ¢(PiShopâ¢ Seller 신설안) 월간/연간, Bean 환산(1 Pi=100 Bean), 원화는 비공개 설계참고. ⚠️ 시드·plan_tp_cd 결정 후 적용
...
Pi 계정 인증·결제, Google 소셜 로그인, 계정 연동, 관리자 시스템, 게시판, 다국어, **테마 기반 카페 플랫폼 PiCafé**, 그리고 **Pi Coin 전용 P2P·O2O 마켓플레이스 PiShopâ¢(MPS)**를 구현한다.
...
| 카페 | PiCafé — 테마 기반 Pi Network 커뮤니티 카페 + PiTranslateâ¢™ 글로벌 동시통역 (Phase 7~12) |
...
- **실시간 글로벌 동시통역** (PiTranslateâ¢™): Gemini Flash + Claude Haiku 하이브리드, 채팅 번역 지연 < 1초
...
### 기능 차별화
- **P2P 직거래 에스크로**: Pi Coin 자동 정산, 분쟁 조정 (PiShopâ¢ MPS)
- **N:N 음성채널** (PiVoice™): WebRTC Full Mesh, 1~4명, 방장 제어, TURN 지원
...
| 13 | PiCafé 수익화 — Pi Bean(구 Tip)·스티커·AI 봇·이벤트방 | ✅ 완료 | Phase 8 |
| 14 | PiCafé 생태계 — 마켓플레이스·Pi Bet·Webhook·분석 대시보드 | ✅ 완료 | Phase 9 |
| 15 | PiTranslateâ¢™ — 글로벌 동시통역 (Gemini Flash + Claude Haiku 하이브리드) | ✅ 완료 | Phase 12 |
| 16 | PiShopâ¢(MPS) — Pi Coin P2P 직거래 마켓플레이스 (에스크로·재고·매장 관리·카테고리·거래내역) | ✅ Phase 1 MVP + Phase 2 확장 완료 (Phase 3 PiRC3·Maps 예정) | Phase 13 |
| 17 | PiVoice™ — WebRTC N:N 음성채널 (1~4명 Full Mesh·방장 마이크 제어·TURN) | ✅ 완료 (v2.0) | Phase 14 |
...
| 22 | PiShopâ¢(MPS) 후속 개선 — A2U 자동 환불(MPS_CANCEL_REFUND)·취소수수료 FR-10 ADMIN 게이트 버그 교정·상품 이미지 업로드(3장·1MB·썸네일)·상품 등록 시 위치 자동수집·주문관리 취소 버튼 역할 구분 | ✅ 완료 | Phase 13 후속 (2026-06-14) |
| 23 | 어드민 대시보드 고도화 — coin360 스타일 테마별 매출 트리맵·사용자 관리 통합·KST 집계 교정·매출 차트 색상 통일·결제내역 거래구분 통합+취소내역 포함 | ✅ 완료 | Phase 11 후속 (2026-06-14) |
| 24 | Pi Browser 안정화 4차 — CafePi 헤더 로고 교체·Pi Bet UI 아코디언+스포트라이트·다국어 선택기 기억·PiShopâ¢ 브랜드 통일·admin 게이트 open redirect 방어 | ✅ 완료 | 횡단 4차 (2026-06-14) |
...
| 26 | GTM 문서화 — 제품소개서(단기 4목표 13장: PiChat·PiShopâ¢·StarterKit·외주연계) + 공개·라이선스 정책(오픈코어 3계층: 미끼/상품/금고, 왕관보석 직영 비매 원칙) + 성능 리스크 레지스터(7종 병목 proactive 분석: 채팅·LBS·에스크로 등) + 운영 이슈 기록(Vercel Hobby cron 제약·GitHub Webhook 누락 대응) | ✅ 완료 | 문서화 (2026-06-16) |
| 27 | **PiShopâ¢ O2O 오프라인 매장 커머스** (§15.8) — 구글 카페 반자동 인증등록(half-인증: place_id 전체타이핑·전화 구글대조·GPS 100m·필수입력)·구글 Place 전체정보 보관(JSONB)·오프라인 주문 상태머신(주문중→준비중→상품대기중→10분 자동 판매완료)·주문방법 3종(매장/픽업/배달)·취소수수료(구매 0.9/판매 1.1, 역할 명시)·사장님 보이스 주문알림(차임+TTS×3)·지도 상품 썸네일 판매·카페 카테고리·상품 shop_id IDOR 차단 | ✅ 완료 | Phase 13 Phase 3 (2026-06-16) |
...
| 32 | **PiShop™ 상품 카테고리 표준** (`docs/PRD_CATEGORY.md` 수용) — 샘플성 6대분류(중고편중·식품/뷰티/유아/헬스 등 14도메인 누락) 폐기, 국내 E커머스 표준 **17대분류 3단계(대>중>소)** 체계 정립. 기존 `mps_ctgr` 재귀(parent_ctgr_id) 활용 — **스키마 변경 0**. 제거+시드 SQL(`sql/105`, 임시테이블로 기존만 안전 폐기·기존 상품 미분류 재지정·물리 DELETE 금지). 소분류는 운영 CRUD 확장 | 📝 설계 완료·SQL 적용 대기 | 카테고리 표준 (2026-06-23) |
...
| 5 | **PiTranslateâ¢™** | 어떤 언어로 카페해도 선택 언어로 실시간 동시통역 — Gemini 2.0 Flash + Claude Haiku 하이브리드 (비용 ~76% 절감) |
...
**PiTranslateâ¢™ 하이브리드 번역 아키텍처 (Phase 12)**:
```
...
**Phase 12: PiTranslateâ¢™ — 글로벌 동시통역**
...
## 14. Phase 12 — PiTranslateâ¢™ 글로벌 동시통역 ✅ (완료 — 2026-06-12, 어드민 번역 통계·품질 피드백 포함)
...
## 15. Phase 13 — PiShopâ¢ (MPS) ✅ Phase 1+2 완료 (2026-06-13) · Phase 3 O2O 카페 커머스 완료 (2026-06-16) · PiRC3 보류
...
> 온라인 커뮤니티(PiChat)에서 시작한 cafe.pi가 **오프라인 실물 카페 운영(O2O)** 으로 흐르는 교두보. 구글 지도의 실제 카페를 사장님이 인증 등록 → Pi로 메뉴 판매.
...
## 16. Phase 14 — PiVoice™ N:N 음성채널 ✅ (v2.0 구현 완료 — 2026-06-12)
...
| M7 | PiShopâ¢ 판매자 거래 취소 (에스크로서비스;PiRC3) | `api/store/orders/[id]/cancel` (판매자) |
| M8 | PiShopâ¢ 구매자 거래 취소 (에스크로서비스;PiRC3) | `api/store/orders/[id]/cancel` (구매자) |
...
10미션 완주 실 보상을 **Pi A2U 직접 송금에서 판매보증금(`mps_seller_bond`) 1π 직접 적립으로 전환**했다. A2U는 시드 미설정·송금 실패 시 PENDING을 반환해 실 지급 신뢰도가 낮은 반면, 보증금 적립은 블록체인 송금 없이 플랫폼 장부에 잔액을 가산하므로 송금 실패가 구조적으로 없고, 적립분이 PiShopâ¢ 판매 신뢰 자본으로 즉시 활용된다.
...
**목표**: 초기 PiShopâ¢ 판매자 100명 선착순 확보 + 1차 인센티브 배분
...
| `GEMINI_API_KEY` | 6, 12 | Gemini AI 번역 (Phase 6 다국어 + Phase 12 PiTranslateâ¢™ 주력 엔진) |
| `RESEND_API_KEY` | 6 | 결제 영수증 이메일 발송 |
| `ANTHROPIC_API_KEY` | 7 | Claude AI 카페 비서 (Phase 7 신규) · PiTranslateâ¢™ fallback (Phase 12) |
...
### 신규 테이블 (Phase 13 — PiShopâ¢ MPS, `mps_` 접두사)
...
### 신규 테이블 (Phase 14 — PiVoice™ N:N 음성채널)
...
| v7.0 | 2026-06-10 | Phase 11 완료 반영. Phase 12 PiTranslateâ¢™ 글로벌 동시통역 신규 추가 — 섹션 14 신설(`PRD_4_CHAT.md` v1.6 수용: Gemini 2.0 Flash 주력 + Claude Haiku fallback 하이브리드·비용 ~76% 절감·`msg_trans` 번역 캐시 테이블·in-memory dedup·broadcast 기반 실시간 전달·TASK-090~099). 기존 섹션 14→15, 15→16, 16→17, 17→18 재번호화. `GEMINI_API_KEY` Phase `6, 12`로 업데이트. Phase 12 파일 디렉토리 구조 추가. `msg_trans` DB 테이블 현황 추가. |
| v8.0 | 2026-06-10 | Phase 13 PiShopâ¢(MPS) Pi Coin P2P 직거래 마켓플레이스 통합 — 섹션 15 신설(`PRD_8_MPS.md` 수용: 양방향 거래완료 확인·9999 무제한 재고 센티널·PiRC2 가상 에스크로·`mps_` 테이블 6개·API 17개·마일스톤 3단계). 기존 섹션 15→16, 16→17, 17→18, 18→19 재번호화. MPS 디렉토리 구조(`store/`·`api/store/`)·lib 헬퍼 파일(`mps-item.ts`·`mps-order.ts`·`mps-shop.ts`) 추가. |
| v9.0 | 2026-06-12 | Phase 15 LBS 위치기반서비스 통합 — 섹션 16 신설(`PRD_10_GPS.md` v1.2 수용: 동의 게이트 Rule LBS-01~04·직거래 비즈니스 근거·`sql/030_lbs.sql`(sys_user_consent·usr_loc_hist·sys_user 컬럼 3개)·API 10개·Haversine 거리 계산·MPS 상품 거리 표시·TASK-130~139). PiVoice™ Phase 14 기능 현황 표 추가(#17). 섹션 16→17(환경변수), 17→18(디렉토리), 18→19(DB 테이블), 19→20(변경이력) 재번호화. GOOGLE_MAPS_API_KEY 환경변수 추가. DB 테이블 현황에 LBS 테이블 2종 추가. |
