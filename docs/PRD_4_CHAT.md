# PRD_CHAT.md — PyCafé: 테마 기반 Pi Network 카페 플랫폼
...
| 5 | **글로벌 동시통역 (PyTranslateâ¢™)** | 어떤 언어로 보내도 내가 선택한 언어로 즉시 통역 — 언어 장벽 제로 |

### 배경 및 목적

Pi Network는 글로벌 KYC 기반 사용자 베이스를 보유하고 있으나, Pi를 실질적으로 소비하는 서비스 생태계가 부족하다. PyCafé는 Pi의 마이크로 트랜잭션 특성과 커뮤니티 카페를 결합해 **Pi 소비 생태계의 핵심 허브**를 목표로 한다.
...
> Discord·Telegram·카카오톡이 무료인데, 왜 Pi를 지불하면서 PyCafé에서 카페를 만드는가?
...
Pi Network는 KYC(신원 인증)를 완료한 사람만 계정을 갖는다. PyCafé의 모든 참가자는 **실명 인증된 실제 인물**이다. 이 신뢰의 가치는 0.1 Pi를 훨씬 초과한다.
...
기존 카페 플랫폼에서 좋은 조언을 줘도 돌아오는 것은 "좋아요" 하나다. PyCafé에서는 **실제 Pi 보상**이 즉시 지급된다.
...
개인 강사, 전문가, 인플루언서에게 PyCafé는 **Pi 수익 창출 채널**이다.
...
이들에게 5 Pi/월 Business 요금은 **수익 대비 투자**다. PyCafé가 없으면 이 수익 채널 자체가 없다.
...
> **전제**: PyCafé는 Discord를 이기려는 것이 아니다.
> Discord는 **게임·익명 커뮤니티** 시장을 지배한다.
> PyCafé는 **라이프스타일·실명·Pi 경제** 시장을 새로 정의한다.
...
Discord와 PyCafé는 **대각선 반대 사분면**에 위치한다. 같은 시장이 아니다.

---

### 1대1 기능 비교

| 항목 | Discord | PyCafé | 승자 |
|---|---|---|---|
| **신원 인증** | 익명 (이메일만) | KYC 실명 인증 | PyCafé — 신뢰 |
| **입장 비용** | 무료 | Pi 소액 (선택적) | Discord — 접근성 |
| **커뮤니티 수** | 수천만 개 | 초기 단계 | Discord — 규모 |
| **서버 품질** | 99% 방치 | 소액 필터로 진지한 방 유지 | PyCafé — 품질 |
| **발견·탐색** | 태그 검색 (품질 보증 없음) | 테마 마켓플레이스 (관심사 정렬) | PyCafé — 정확도 |
| **카페 내 결제** | 없음 | Pi Tip·스티커·AI 단건 결제 | PyCafé — 유일 |
| **창작자 수익** | 서버 구독 (플랫폼 30% 수수료) | Pi Tip 직접 수취 (수수료 최소) | PyCafé — 창작자 우선 |
| **AI 봇** | 범용 봇 (Midjourney 등 외부) | 테마별 내장 AI (골프 코치·여행 번역) | PyCafé — 맥락 특화 |
| **이벤트 수익화** | Stage + Nitro (복잡) | Event Room 입장료 Pi 직접 수취 | PyCafé — 단순 |
| **보이스·비디오** | 풍부한 기능 | MVP 단계 미지원 | Discord — 성숙도 |
| **모바일 UX** | 앱 성숙 | Pi Browser 내 WebView 최적화 | Discord — 현재 |
| **Pi 연동** | 없음 | 네이티브 | PyCafé — 유일 |
...
PyCafé는 처음부터 KYC로 시작한다. 이 신뢰 레이어는 후발주자가 나중에 추가할 수 없다.
...
Discord는 이미 수천만 개의 서버가 있지만 **테마로 구조화되어 있지 않다**. PyCafé가 테마 그래프를 선점하면, 나중에 Discord가 테마 기능을 추가해도 커뮤니티 이동 비용이 너무 크다.

---

### 전략적 포지셔닝: "Discord를 버리지 않아도 된다"

PyCafé의 가장 현명한 전략은 **Discord와 병행 사용**을 권장하는 것이다.
...
PyCafé:     관심사 기반 실명 커뮤니티 + Pi 경제 활동 → 새로 추가
```

"Discord 대신 PyCafé"가 아니라 **"PyCafé는 Pi 사용자를 위한 추가 공간"**으로 포지셔닝한다. 이 전략은:
...
인기 Discord 서버의 운영자에게 PyCafé 이전을 제안한다:
- "Discord에서는 0원인데, PyCafé에서는 Tip으로 Pi를 벌 수 있습니다"
- Discord 서버 멤버를 PyCafé로 초대하는 전용 링크 생성 지원

#### 전술 3: 창작자 수익 우선 정책

Discord 서버 구독은 플랫폼이 30% 수수료를 가져간다.
PyCafé Pi Tip은 **수수료 0%로 창작자에게 직접 전달** (초기 3년 정책으로 공표).

이 한 가지만으로 Discord에서 수익을 내던 창작자들이 이동할 강력한 이유가 된다.

#### 전술 4: 테마 독점 콘텐츠 파트너십

- 골프: 국내 프로 골퍼의 PyCafé 전용 라이브 Q&A
- 여행: 유명 여행 유튜버의 PyCafé 전용 이벤트방
- 먹방: 인기 먹방 크리에이터의 Pi Tip 받는 라이브 카페

Discord에는 이런 라이브 경제 이벤트가 없다. **PyCafé만의 독점 콘텐츠**로 초기 사용자를 유입시킨다.

#### 전술 5: Pi Browser 네이티브 경험

Discord는 Pi Browser에서 불편하다 (웹 접근, Pi 결제 불가).
PyCafé는 **Pi Browser에 최적화**된 네이티브 UX로 Pi 사용자 경험을 압도한다.
...
> PyCafé는 **플랫폼조차 당신의 대화에 접근할 수 없다.**
...
PyCafé는 이 이분법을 깨는 **세 번째 포지션**을 가진다:
...
PyCafé에는 별도 계정이 없다. **Pi 지갑 = 당신의 ID**다.
...
PyCafé가 내일 서비스를 종료해도 당신의 Pi 지갑·Pi 자산은 그대로다. 플랫폼이 당신을 쫓아낼 수 없다.
...
누군가에게 Pi Tip을 보내면 PyCafé 서버를 거치지 않고 블록체인에서 직접 정산된다. PyCafé 운영자가 Tip을 가로막거나 수수료를 바꿀 수 없다.
...
**1:1·비밀 카페는 PyCafé 서버조차 내용을 읽을 수 없다.** 서버에는 암호화된 바이너리만 저장된다.

---

### 중앙화 서비스와의 프라이버시 비교

| 항목 | Discord | 카카오톡 | Telegram | PyCafé |
|---|---|---|---|---|
...
카카오톡·라인은 특정 국가에서 수사 협조를 했다. PyCafé의 E2E 암호화 1:1 카페는 **플랫폼 측 제공 데이터가 없어 법적 요청에 응할 수 없다.**

#### 시나리오 2: 비즈니스 기밀 카페

비밀 카페는 자동삭제 타이머 + E2E 암호화로, **PyCafé 서버에 회의 내용 흔적이 남지 않는다.**
...
방장이 자신의 테마 카페를 운영하는 한, PyCafé 운영자가 이유 없이 방을 강제 삭제할 수 없다. Pi 지갑에 연결된 카페 소유권은 블록체인이 보증한다.

---

### 탈중앙화 선언 (사용자 대상 메시지)

PyCafé가 사용자에게 공개적으로 약속하는 4가지:
...
PyCafé는 **Pi 마이크로 트랜잭션**이 수익 모델이므로 사용자 데이터를 팔 필요가 없다. 탈중앙화 프라이버시 정책이 비즈니스 모델과 충돌하지 않고 **오히려 강화**한다.

> **탈중앙화 = 마케팅 수사가 아닌 비즈니스 모델의 필연적 결과다.**

---

## 1-4. 글로벌 동시통역 (PyTranslateâ¢™) — "언어 없는 카페"

> **Discord도 없다. Telegram도 없다. WhatsApp도 없다.**
> 전 세계 어디서든, 어떤 언어로 보내도, 내가 선택한 언어로 실시간 통역되는 카페는 **PyCafé가 처음이다.**
...
**Discord에서 "한 카페에 6개 언어 사용자가 모여서 자유롭게 대화"하는 경험은 현재 불가능하다. PyCafé가 최초다.**
...
CREATE TABLE msg_msg (
  ...
  src_lang_cd VARCHAR(20),              -- 원본 언어 코드 (PyTranslateâ¢™ — Claude Haiku 감지)
  del_yn      CHAR(1)      DEFAULT 'N',
  regr_id     TEXT  NOT NULL DEFAULT 'ADMIN',
  reg_dtm     TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  modr_id     TEXT         NOT NULL DEFAULT 'ADMIN',
...
### Phase 10: 글로벌 동시통역 (PyTranslateâ¢™) ⭐ 글로벌 런칭 킬러 피처
...
| v1.4 | 2026-06-10 | 글로벌 동시통역 (PyTranslateâ¢™) 전면 추가 — 섹션 1-4 신설·KPI 번역 지표·DB `msg_trans` 테이블·번역 API·기술 아키텍처(Claude Haiku + dedup + broadcast)·Phase 10 로드맵 TASK-080~089 |
