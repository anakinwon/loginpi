...
> **작성일**: 2026-06-29 · **최종 현행화**: 2026-07-16 · 대상: master · **면책**: 적합 최종판정은 Pi 심사팀 권한. '확인필요'는 단정하지 않음.
> ✅ **2026-07-16 이행 완결**: 본 문서의 절제 요구사항은 전부 이행됐다 — 죽은 베팅 i18n 키 4,512행 삭제(sql/182)·절제 오버레이 25키 보강('발행'→'지급'·Bean Token→Bean 포인트)·테마 'Pi투자'→'크립토 라운지'·`_comment` 번들 직렬화 노출 차단. 운영 실측 금지어 0건. 등재 신청 완결(체크리스트 19/19)·승인 대기 — 정본 MAINNET_READINESS_CHECKLIST.
...
| ✅ 절제 이행 완료(2026-07-16) | ⑤Bean "토큰" 노출 표현 — listing 오버레이 35키(발행→지급·Bean Token→Bean 포인트) + 죽은 베팅 키 삭제 + 크립토 라운지, 운영 실측 0건 |
| 🟡 조건부(라벨 보강) | ②각국 통화 콤보 |
