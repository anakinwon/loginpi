...
<div class="grid">
  ...
  <section><h3>도구별 지연 중앙값 (초)</h3><p class="desc">호출 단위 p50 · p90·횟수·오류 병기 (평균은 꼬리에 끌려 착시)</p>${hbar(latencyBars, { valueFmt: v => `${v}s`, aria: '도구별 지연 중앙값' })}</section>
...
<h2 class="type">④ 산출 · 품질 · 위임<small>효율의 분모 — 요청 ${OC.requests}건 · 변경 파일 ${OC.files_changed} · 서브에이전트 ${agentRows.length}</small></h2>
<div class="grid">
  <section><h3>파일별 수정 횟수</h3><p class="desc">같은 파일을 여러 턴에 걸쳐 고치면 재작업 · 색은 재작업 여부</p>${hbar(OC.files.slice(0, 12).map(f => ({ label: f.file.split('/').pop(), value: f.calls, sub: `· ${f.turns}턴${f.rework ? ' · 재작업' : ''}`, color: f.rework ? 'var(--s2)' : 'var(--s1)', tip: `${f.file}: ${f.calls}회 수정 · ${f.turns}턴` })), { labelW: 190, aria: '파일별 수정 횟수' })}</section>
  <section><h3>서브에이전트 비용</h3><p class="desc">에이전트별 USD · 실행 시간 · 출력 병기 (메인 세션 비용과 별도)</p>${hbar(agentRows.sort((a, b) => (b.cost_usd || 0) - (a.cost_usd || 0)).slice(0, 12).map(a => ({ label: a.name, value: +(a.cost_usd || 0).toFixed(2), sub: `· ${a.wall} · ${a.output} tok`, color: 'var(--s4)', tip: `${a.name} (${a.type}, ${a.model}) 턴 ${a.turn}: ${a.cost} · ${a.wall} · 도구 ${a.tools} · 오류 ${a.errors}` })), { labelW: 150, valueFmt: v => `$${v}`, aria: '서브에이전트별 비용' })}</section>
  <section class="wide"><h3>품질 표</h3><p class="desc">정의는 ${esc(stem)}.metrics.json (지표 레지스트리)</p>${htmlTable([{ k: '사용자 교정률', v: pct(OC.correction_rate, 1), d: `${OC.corrections}/${OC.requests} 요청 — 10분 내 같은 요청 반복·교정 어두·재전송` }, { k: '파일 재작업률', v: pct(OC.rework_rate, 1), d: `${OC.files_reworked}/${OC.files_changed} 파일이 2턴 이상에서 수정` }, { k: '무검증 쓰기 턴', v: `${OC.unverified_write_turns}/${OC.write_turns}`, d: '쓰기 후 테스트·실행 확인 스텝 없음' }, { k: '오류 회복 배수', v: OC.recovery_multiplier != null ? `${OC.recovery_multiplier}×` : '-', d: `오류 ${total.errors}건 → 추가 호출 ${OC.error_extra_calls}회 · ${ms2(OC.error_extra_ms)}` }, { k: '중단 · 재전송 · 흡수', v: `${OC.interrupted} · ${OC.superseded} · ${total.absorbed}`, d: '중단=품질 사고, 흡수=턴 도중 요청(정상)' }], [['지표', o => esc(o.k)], ['값', o => esc(o.v), 'num'], ['설명', o => esc(o.d), 'req']])}</section>
</div>
