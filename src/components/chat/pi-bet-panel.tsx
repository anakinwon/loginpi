        ...
        <div className="p-4">
          ...
          {showForm && (
            <div className="mb-4 space-y-2.5 rounded-2xl border-2 border-dashed p-4">
              ...
              <input
                ...
                className="focus:ring-primary/50 w-full rounded-xl border bg-transparent px-3 py-2 text-sm focus:ring-2 focus:outline-none"
                maxLength={200}
              />
              <input
                value={optionsText}
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  max="100"
                  step="0.01"
                  className="focus:ring-primary/50 w-24 rounded-xl border bg-transparent px-3 py-2 text-sm focus:ring-2 focus:outline-none"
                />
                <button
                  onClick={createBet}
                  disabled={creating}
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {bets.map((bet) => {
                const totalEntries = bet.options.reduce(
                  (s, o) => s + o.entry_cnt,
                  0,
                )
                const iWon =
                  bet.my_entry?.win_yn === 'Y' && bet.bet_st_cd === 'SETTLED'
                  ...
                  >
                    ...
                    {iWon && (
                      <div
                        ...
                        style={{
                          background:
                            'linear-gradient(90deg, #fbbf24 0%, #f59e0b 25%, #fde68a 50%, #f59e0b 75%, #fbbf24 100%)',
                    ...
                    >
                      <div className="overflow-hidden">
                        <div className="px-4 pt-2.5 pb-3">
                          {/* 메타 정보 */}
                          <div className="text-muted-foreground mb-2.5 flex flex-wrap items-center gap-x-3 gap-y-1 pl-1 text-xs">
                            <span>
                              참가비{' '}
                              ...
                              <>
                                ...
                                <span className="text-primary font-medium">
                                  내 선택:{' '}
                                  {
                                    bet.options.find(
                                      (o) =>
                                        o.optn_no === bet.my_entry?.optn_no,
                                    )?.optn_nm
                                  }
                                </span>
                              </>
                          ...
                          {/* 선택지 목록 */}
                          <div className="border-muted ml-2 space-y-1 border-l-2 border-dashed pl-3">
                            {bet.options.map((opt, idx) => {
                              const palette =
                                OPTION_PALETTE[idx % OPTION_PALETTE.length]
                              const isWin = bet.win_optn_no === opt.optn_no
                              const ratio =
                                totalEntries > 0
                                  ? (opt.entry_cnt / totalEntries) * 100
                                  : 0
                              const iMyChoice =
                                bet.my_entry?.optn_no === opt.optn_no
                                ...
                                <div
                                  ...
                                  style={
                                    isWin
                                      ? {
                                          animation:
                                            'bet-spotlight 1.6s ease-in-out infinite',
                                        }
                                      : undefined
                                  }
                                >
                                  <div className="mb-1.5 flex items-center justify-between gap-2">
                                    <div className="flex min-w-0 items-center gap-1.5">
                                      {isWin ? (
                                        ...
                                        <span
                                          className={`h-2 w-2 shrink-0 rounded-full bg-gradient-to-br ${palette.bar}`}
                                        />
                                      )}
                                      <span
                                      ...
                                      {iMyChoice && !isWin && (
                                        <span className="bg-primary/15 text-primary shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-bold">
                                          내 선택
                                        </span>
                                      )}
                                    </div>
                                    <div className="flex shrink-0 items-center gap-2">
                                      ...
                                      {bet.bet_st_cd === 'OPEN' &&
                                        bet.is_creator && (
                                          <button
                                            onClick={() =>
                                              settleBet(bet, opt.optn_no)
                                            }
                                            className="text-muted-foreground hover:bg-muted rounded-lg border px-2.5 py-1 text-[11px]"
                                          >
                                            정산
                                          </button>
                                        )}
