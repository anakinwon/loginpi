...
export function GroupRoomCreator() {
  ...
  useEffect(() => {
    ...
    piFetch('/api/subscriptions/check')
      ...
      .then((d: { canUsePremiumTheme?: boolean; canCreateRoomFree?: boolean; canCreateEventRoom?: boolean } | null) => {
  ...
  }, [selectedTheme, roomNm, roomDesc, isPublic, maxMbr, entryFee, eventEndDtm, router])
              ...
              <div className='mb-3 grid grid-cols-2 gap-2'>
                <button
                  ...
                  className={`rounded-xl border px-3 py-2 text-sm font-medium transition-colors ${
                    roomType === 'G' ? 'border-primary bg-primary/10 text-primary' : 'hover:bg-muted'
                  }`}
                >
                  💬 그룹방
                </button>
                <button
                  ...
                  className={`rounded-xl border px-3 py-2 text-sm font-medium transition-colors ${
                    roomType === 'E'
                      ...
                      : canCreateEventRoom ? 'hover:bg-muted' : 'opacity-60 hover:bg-muted'
                  }`}
                >
            ...
            <div className='space-y-4'>
              ...
              {roomType === 'G' ? (
                ...
                <>
                  ...
                  <div>
                    <p className='mb-2 text-sm font-medium'>입장료 (참가자 결제)</p>
                    <div className='grid grid-cols-4 gap-2'>
                      {([0, 0.1, 0.5, 1] as EntryFee[]).map(fee => (
                        <button
                          ...
                          className={`rounded-xl border py-2 text-sm font-medium transition-colors ${
                            entryFee === fee ? 'border-primary bg-primary/10 text-primary' : 'hover:bg-muted'
                          }`}
                        >
                          {fee === 0 ? '무료' : `π${fee}`}
                        </button>
                  ...
                  <div>
                    <label className='mb-1 block text-sm font-medium'>이벤트 종료 시각 *</label>
                    <input
                      type='datetime-local'
                      value={eventEndDtm}
                      min={localDtmNow()}
                    ...
                    <p className='mt-1 text-xs text-muted-foreground'>
                      종료 시각이 지나면 참가자(GUEST) 멤버십이 자동 만료됩니다
                    </p>
                  </div>
                </>
              )}
              ...
              <div className='flex gap-2'>
                ...
                {roomType === 'E' ? (
                  <button
                    ...
                    disabled={isBusy || !eventEndDtm || new Date(eventEndDtm) <= new Date()}
                    className='flex-1 rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:opacity-40'
                  >
