...
export function BadgeAwardPopup({ badge, onUpgraded, onClose }: BadgeAwardPopupProps) {
    ...
    window.Pi.createPayment(
      {
        ...
        metadata: { type: 'FEATURE_ADDON', feature_cd: 'BADGE_UPGRADE', theme_cd: badge.theme_cd },
      },
      {
        ...
        onReadyForServerCompletion: async (paymentId, txid) => {
          ...
          setPaying(false)
          if (res.ok) {
            toast.success(`${badge.theme_emoji} ${badge.theme_nm} 배지가 강화되었습니다!`)
            onUpgraded()
          } else {
    ...
    >
      ...
      >
        ...
        <div className='mt-4 space-y-2.5'>
          ...
          >
            {paying ? '결제 진행 중…' : (
              <><span className='font-serif italic'>π</span> {BADGE_UPGRADE_PI} 배지 강화</>
            )}
          </button>
          <button
            onClick={onClose}
