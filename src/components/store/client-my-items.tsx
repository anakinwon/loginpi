...
export function ClientMyItems({ serverAuthed = false }: { serverAuthed?: boolean }) {
  ...
  async function remove(itemId: string) {
    ...
    const res = await piFetch(`/api/store/items/${itemId}`, { method: 'DELETE' })
    if (res.ok) {
  ...
  const filtered = tab === 'ALL' ? items : items.filter(i => i.item_st_cd === tab)
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex flex-wrap items-center gap-2'>
        {ST_TABS.map(s => (
          ...
          >
            ...
            <span className='ml-1'>
              ({s === 'ALL' ? items.length : items.filter(i => i.item_st_cd === s).length})
            </span>
          </button>
        ))}
      ...
      {loading ? (
        ...
        <p className='text-muted-foreground py-16 text-center text-sm'>{t('noItems')}</p>
      ) : (
        <div className='space-y-2'>
          {filtered.map(item => (
            <div key={item.item_id} className='flex items-center gap-3 rounded-lg border p-3'>
              ...
              <div className='min-w-0 flex-1'>
                <Link href={`/store/${item.item_id}`} className='truncate text-sm font-medium hover:underline'>
                  {item.item_nm}
                </Link>
                <p className='text-muted-foreground text-xs'>
                  {Number(item.price_pi)} π · {t(`itemSt.${item.item_st_cd}`)}
                  {item.reg_qty !== 9999 && ` · ${t('stockLeft', { count: item.stock_qty })}`}
                </p>
              </div>
              <div className='flex shrink-0 gap-1.5'>
                ...
                {item.item_st_cd === 'OPEN' && (
                  <Button size='sm' variant='outline' onClick={() => changeStatus(item.item_id, 'CLOSED')}>
                    {t('actionClose')}
                  </Button>
                )}
                <Button size='sm' variant='outline' onClick={() => remove(item.item_id)}>
                  {t('actionDelete')}
                </Button>
              </div>
            </div>
