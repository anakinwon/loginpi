...
async function ghCompare(
  base: string,
  head: string,
): Promise<{ ahead: number; behind: number; commits: CommitInfo[]; error?: string }> {
  const token = process.env.GITHUB_DEPLOY_TOKEN
  if (!token) return { ahead: 0, behind: 0, commits: [], error: 'GITHUB_DEPLOY_TOKEN 미설정' }
  try {
    const res = await fetch(`${GH}/repos/${repo()}/compare/${base}...${head}`, {
      headers: ghHeaders(token),
      cache: 'no-store',
    })
    if (!res.ok) return { ahead: 0, behind: 0, commits: [], error: `GitHub ${res.status}` }
    ...
    return { ahead: 0, behind: 0, commits: [], error: e instanceof Error ? e.message : 'unknown' }
  }
}
...
export async function getDeployOverview(): Promise<DeployOverview> {
  const configured = {
    ...
    stagingStatus: !!(process.env.VERCEL_API_TOKEN && process.env.VERCEL_STAGING_PROJECT_ID),
    prodStatus: !!(process.env.VERCEL_API_TOKEN && process.env.VERCEL_PROD_PROJECT_ID),
  }
...
export async function promoteToProduction(): Promise<{ ok: boolean; sha?: string; error?: string }> {
  const token = process.env.GITHUB_DEPLOY_TOKEN
  if (!token) return { ok: false, error: 'GITHUB_DEPLOY_TOKEN 미설정' }
  const [master, cmp] = await Promise.all([ghCommit('master'), ghCompare('production', 'master')])
  if (cmp.error) return { ok: false, error: cmp.error }
  if (cmp.behind > 0)
    return { ok: false, error: `production이 master보다 ${cmp.behind}커밋 앞섬(갈라짐) — fast-forward 불가` }
  if (cmp.ahead === 0) return { ok: false, error: '승격할 커밋 없음(이미 최신)' }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { ok: false, error: `승격 실패 GitHub ${res.status} ${txt.slice(0, 160)}` }
    }
...
export async function setStagingDbTarget(target: DbTarget): Promise<{ ok: boolean; error?: string }> {
  const st = getDbSwitchState()
  if (st.tier !== 'staging') return { ok: false, error: '스테이징 환경에서만 전환 가능(운영 WAS 불변)' }
  if (!st.apiConfigured) return { ok: false, error: 'Vercel API 토큰/프로젝트ID/Deploy Hook 미설정' }
  ...
  try {
    const res = await fetch(`${VERCEL}/v10/projects/${projectId}/env${teamQs('upsert=true')}`, {
      method: 'POST',
      headers: {
