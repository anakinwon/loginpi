...
export async function POST(req: NextRequest) {
  ...
  if (!fbck_id) return NextResponse.json({ error: 'fbck_id가 필요합니다' }, { status: 400 })
  if (hide_yn !== 'Y' && hide_yn !== 'N') {
    return NextResponse.json({ error: 'hide_yn은 Y 또는 N이어야 합니다' }, { status: 400 })
  }
  if (hide_yn === 'Y' && !hide_reason_txt?.trim()) {
    return NextResponse.json({ error: '숨김 처리 시 사유를 입력해 주세요' }, { status: 400 })
  }
  ...
  return NextResponse.json({
    ok: true,
    message: hide_yn === 'Y' ? '후기를 숨겼습니다.' : '후기를 다시 공개했습니다.',
  })
}
