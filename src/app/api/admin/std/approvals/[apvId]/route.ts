...
export async function PATCH(
  ...
  const body = (await req.json()) as { action: 'approve' | 'reject'; reject_reason?: string }
  ...
  if (fetchErr || !apv) return NextResponse.json({ error: '요청을 찾을 수 없습니다' }, { status: 404 })
  ...
  const { error: updErr } = await db
    ...
    .update({
      ...
      decided_at:    now,
      reject_reason: body.action === 'reject' ? body.reject_reason!.trim() : null,
      modr_id:       requester.id,
      mod_dtm:       now,
    })
    .eq('apv_id', apvId)

  if (updErr) return NextResponse.json({ error: '상태 업데이트 실패' }, { status: 500 })
