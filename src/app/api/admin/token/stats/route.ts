...
export async function GET() {
  ...
  // 병렬 집계: 발행·유통·4종 거버넌스 지갑·일별 트렌드
  const [chargeRes, circulatingRes, govWalletsRes, dailyRes] = await Promise.all([
    // 총 발행량 = CHARGE 거래 합계
    db.from('bean_txn').select('bean_amt').eq('txn_tp_cd', 'CHARGE').eq('del_yn', 'N'),
    // 유통량 = USER 지갑 합계
    db.from('bean_token_wallet').select('bean_amt').eq('wallet_type', 'USER').eq('del_yn', 'N'),
  ...
  const totalIssued = (chargeRes.data ?? []).reduce((s, r) => s + Number(r.bean_amt), 0)
  const circulating = (circulatingRes.data ?? []).reduce((s, r) => s + Number(r.bean_amt), 0)

  type GovRow = { wallet_type: string; bean_amt: number }
  const govMap = Object.fromEntries(
    (govWalletsRes.data ?? []).map((r: GovRow) => [r.wallet_type, Number(r.bean_amt)]),
  )
  ...
  return NextResponse.json({
    kpi: {
      ...
      collection_rate_percent:
        totalIssued > 0 ? Math.round((totalCollected / totalIssued) * 10000) / 100 : 0,
      ...
      // 배분 비율 (발행량 대비)
      platform_pct:   totalIssued > 0 ? Math.round((platformBalance  / totalIssued) * 10000) / 100 : 0,
      foundation_pct: totalIssued > 0 ? Math.round((foundationBalance / totalIssued) * 10000) / 100 : 0,
      reward_pool_pct: totalIssued > 0 ? Math.round((rewardPoolBalance / totalIssued) * 10000) / 100 : 0,
      // 항등식 검증
