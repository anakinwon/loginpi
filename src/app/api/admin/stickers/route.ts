...
export async function GET() {
  ...
  const [{ data: packs, error }, { data: stkrRows }, { data: ownrRows }, { data: themes }] =
    await Promise.all([
      db.from('msg_stkr_pack')
        .select(`
      ...
      db.from('msg_theme').select('theme_cd, theme_nm, theme_emoji').eq('del_yn', 'N').order('sort_ord'),
    ])

  if (error) {
    return NextResponse.json({ error: '스티커팩 목록 조회 실패' }, { status: 500 })
  }

  // 팩별 스티커 수·보유자 수 집계 (N+1 대신 IN 일괄 조회 후 Map 매핑)
  const stkrCnt = new Map<string, number>()
  for (const r of stkrRows ?? []) stkrCnt.set(r.pack_id, (stkrCnt.get(r.pack_id) ?? 0) + 1)
  const ownrCnt = new Map<string, number>()
  for (const r of ownrRows ?? []) ownrCnt.set(r.pack_id, (ownrCnt.get(r.pack_id) ?? 0) + 1)
...
export async function POST(req: NextRequest) {
  ...
  if (!Number.isFinite(price) || price < 0 || price > 1000) {
    return NextResponse.json({ error: '가격은 0~1000 Pi여야 합니다' }, { status: 400 })
  }
  ...
  if (theme_cd) {
    const { data: theme } = await db
      .from('msg_theme').select('theme_cd').eq('theme_cd', theme_cd).eq('del_yn', 'N').maybeSingle()
    if (!theme) return NextResponse.json({ error: '존재하지 않는 테마입니다' }, { status: 400 })
  }
  ...
  if (is_dflt_yn === 'Y' && theme_cd) {
    await db.from('msg_theme_stkr').upsert(
