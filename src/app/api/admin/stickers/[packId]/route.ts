...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const [{ data: pack }, { data: stickers }] = await Promise.all([
    db.from('msg_stkr_pack')
      .select('pack_id, pack_nm, pack_desc, theme_cd, price_pi, is_dflt_yn, use_yn, ownr_usr_id, mkt_yn, reg_dtm')
      .eq('pack_id', packId)
      .eq('del_yn', 'N')
      .maybeSingle(),
    db.from('msg_stkr')
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const { pack_nm, pack_desc, theme_cd, price_pi, use_yn, is_dflt_yn } = body as {
    pack_nm?: string
    pack_desc?: string
  ...
  const update: Record<string, unknown> = { modr_id: 'ADMIN', mod_dtm: new Date().toISOString() }

  if (pack_nm !== undefined) {
    const name = pack_nm.trim()
    if (!name || name.length > 100) {
      return NextResponse.json({ error: '팩 이름은 1~100자여야 합니다' }, { status: 400 })
    }
    update.pack_nm = name
  }
  ...
  if (is_dflt_yn !== undefined) update.is_dflt_yn = is_dflt_yn === 'Y' ? 'Y' : 'N'
  ...
  if (error) return NextResponse.json({ error: '팩 수정 실패' }, { status: 500 })
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  const { error } = await getSupabaseAdmin()
    .from('msg_stkr_pack')
    .update({ del_yn: 'Y', del_dtm: new Date().toISOString(), modr_id: 'ADMIN' })
    .eq('pack_id', packId)
    .eq('del_yn', 'N')

  if (error) return NextResponse.json({ error: '팩 삭제 실패' }, { status: 500 })
  return NextResponse.json({ ok: true })
}
