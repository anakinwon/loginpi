...
async function ownShop(shopId: string, userId: string, admin: boolean) {
  ...
  if (!shop) return { error: 404 as const }
  if (shop.seller_id !== userId && !admin) return { error: 403 as const }
  return { shop }
}
...
export async function GET(_req: Request, { params }: Params) {
  ...
  if ('error' in r)
    return NextResponse.json({ error: 'forbidden' }, { status: r.error })
