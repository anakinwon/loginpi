...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  const { data: bets, error } = await db
    .from('msg_bet')
    .select('bet_id, room_id, crtr_usr_id, bet_titl, bet_amt_pi, bet_st_cd, close_dtm, win_optn_no, settle_dtm, reg_dtm')
    .eq('room_id', roomId)
    .eq('del_yn', 'N')
    .order('reg_dtm', { ascending: false })
    .limit(20)

  if (error) return NextResponse.json({ error: '베팅 목록 조회 실패' }, { status: 500 })

  const betIds = (bets ?? []).map((b: { bet_id: string }) => b.bet_id)
  let options: { bet_id: string; optn_no: number; optn_nm: string }[] = []
  let entries: { bet_id: string; usr_id: string; optn_no: number; bet_amt_pi: number; win_yn: string; payout_pi: number }[] = []
  ...
  const result = (bets ?? []).map((b: { bet_id: string; crtr_usr_id: string }) => {
    ...
    return {
      ...
      total_pool_pi: betEntries.reduce((sum, e) => sum + Number(e.bet_amt_pi), 0),
      my_entry: myEntry
        ? { optn_no: myEntry.optn_no, win_yn: myEntry.win_yn, payout_pi: myEntry.payout_pi }
        : null,
    }
  })

  return NextResponse.json({ bets: result, is_room_owner: mbr.mbr_role_cd === 'OWNER' })
}
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const optionNames = (options ?? []).map(o => String(o).trim()).filter(Boolean)

  if (!title || title.length > 200) {
    return NextResponse.json({ error: '베팅 주제를 입력해주세요 (200자 이내)' }, { status: 400 })
  }
  if (!Number.isFinite(amount) || amount <= 0 || amount > 100) {
    return NextResponse.json({ error: '베팅 금액은 0 초과 100 Pi 이하여야 합니다' }, { status: 400 })
  }
  if (optionNames.length < 2 || optionNames.length > 10) {
    return NextResponse.json({ error: '선택지는 2~10개여야 합니다' }, { status: 400 })
  }
  if (close_dtm && Number.isNaN(Date.parse(close_dtm))) {
    return NextResponse.json({ error: '유효하지 않은 마감 시각' }, { status: 400 })
  }
  ...
  if (error || !bet) return NextResponse.json({ error: '베팅 생성 실패' }, { status: 500 })
  ...
  if (optError) {
    ...
    await db.from('msg_bet').update({ del_yn: 'Y', del_dtm: new Date().toISOString() }).eq('bet_id', bet.bet_id)
