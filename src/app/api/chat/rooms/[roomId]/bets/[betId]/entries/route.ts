...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  if (!Number.isInteger(optnNo) || optnNo < 1) {
    return NextResponse.json({ error: '유효하지 않은 옵션 번호' }, { status: 400 })
  }
  ...
  if (existing) return NextResponse.json({ error: '이미 참가한 베팅입니다' }, { status: 409 })

  const betRow = bet as { bet_titl: string; bet_amt_pi: number; bet_st_cd: string; close_dtm: string | null; crtr_usr_id: string }
  if (betRow.bet_st_cd !== 'OPEN') {
    return NextResponse.json({ error: '참가가 마감된 베팅입니다' }, { status: 409 })
  }
  if (betRow.close_dtm && new Date(betRow.close_dtm) <= new Date()) {
    return NextResponse.json({ error: '베팅 마감 시각이 지났습니다' }, { status: 409 })
  }
  if (betRow.crtr_usr_id === user.id) {
    return NextResponse.json({ error: '베팅 생성자는 참가할 수 없습니다' }, { status: 403 })
  }
