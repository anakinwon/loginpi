...
const EndCallSchema = z.object({
  end_rsn_cd: z.enum(['USER_ENDED', 'TIMEOUT', 'REJECTED', 'FAILED']).optional(),
...
export async function POST(req: Request, { params }: Params) {
  ...
  const { end_rsn_cd = 'USER_ENDED', relay_yn, rtt_ms, packet_loss_pct, jitter_ms } = parsed.data
  ...
  const finalStatus =
    call.call_st_cd === 'RINGING'
      ? end_rsn_cd === 'REJECTED' ? 'DECLINED' : 'MISSED'
      : 'ENDED'
  ...
  if (call.call_st_cd === 'CONNECTED' && (rtt_ms !== undefined || packet_loss_pct !== undefined || jitter_ms !== undefined)) {
    await getSupabaseAdmin()
