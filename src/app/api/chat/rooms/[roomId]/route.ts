...
export async function GET(_request: Request, { params }: Params) {
  ...
  const { data: theme } = await getSupabaseAdmin()
    ...
    .single()
  const themeEmoji =
    (theme as { theme_emoji?: string } | null)?.theme_emoji ?? '💬'

  const { data: members } = await getSupabaseAdmin()
    .from('msg_room_mbr')
    .select(
      'room_mbr_id, usr_id, mbr_role_cd, lst_read_msg_id, expire_dtm, reg_dtm',
    )
    .eq('room_id', roomId)
    .eq('del_yn', 'N')
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  if (room_desc !== undefined) {
    const desc =
      room_desc === null ? null : String(room_desc).trim().slice(0, 500)
  ...
  if (nextPublic === 'Y') {
    ...
    if (join_pwd !== undefined || is_public_yn === 'Y')
