  ...
  return (
    <div className='space-y-4'>
      ...
      {isActive && expireLabel && (
        <p className='text-sm text-muted-foreground'>만료일: {expireLabel}</p>
      )}

      {isActive && subscr?.auto_renew_yn === 'Y' && (
        <p className='text-xs text-muted-foreground'>자동 갱신 활성화됨</p>
      )}
      ...
      {isActive && subscr?.auto_renew_yn === 'N' && (
        <p className='text-xs text-muted-foreground'>자동 갱신이 취소되어 있습니다.</p>
      )}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className='mb-5 space-y-2.5 rounded-xl bg-muted/50 p-4 text-xs text-muted-foreground'>
          <PolicyItem>
            취소 즉시 환불되지 않습니다.{' '}
            {expireLabel ? (
              <>
                <strong className='text-foreground'>{expireLabel}</strong>까지 정상 이용할 수 있습니다.
              </>
            ) : (
          ...
          <PolicyItem>
            취소는 <strong className='text-foreground'>자동 갱신을 중단</strong>하는 방식입니다.
            현재 구독 기간 종료 후 더 이상 결제되지 않습니다.
          </PolicyItem>
          <PolicyItem>부분 환불 및 잔여 기간 환급은 제공되지 않습니다.</PolicyItem>
          <PolicyItem>결제 실패 시에도 자동 갱신이 자동으로 중단됩니다.</PolicyItem>
        </div>
...
function PolicyItem({ children }: { children: React.ReactNode }) {
  return (
    <div className='flex gap-2'>
      <span className='mt-0.5 shrink-0 text-muted-foreground/70'>•</span>
      <span>{children}</span>
    </div>
  )
}
