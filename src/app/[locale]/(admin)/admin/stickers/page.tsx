...
export default function StickersPage() {
  ...
  async function createPack() {
    if (!newNm.trim()) { toast.error(t('createValidation')); return }
    setCreating(true)
    try {
      const res = await fetch('/api/admin/stickers', {
        method: 'POST',
      ...
      setNewNm(''); setNewDesc(''); setNewTheme(''); setNewPrice(0); setNewDflt(false)
      await reload()
    } catch (err) {
  ...
  async function openDetail(packId: string) {
    if (detailId === packId) { setDetailId(null); return }
    setDetailId(packId)
    setDetailLoading(true)
    try {
      const res = await fetch(`/api/admin/stickers/${packId}`)
      if (!res.ok) { setDetailId(null); return }
  ...
  async function removePack(p: PackRow) {
    ...
    const res = await fetch(`/api/admin/stickers/${p.pack_id}`, { method: 'DELETE' })
    if (!res.ok) { toast.error(t('deleteFail')); return }
  ...
  async function uploadStickers(packId: string, files: FileList | null) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/stickers/${packId}/items`, { method: 'POST', body: fd })
      if (!res.ok) {
        const d = (await res.json()) as { error?: string }
        throw new Error(d.error ?? t('uploadFail'))
      }
      const d = (await res.json()) as { stickers: StickerRow[]; uploaded: number }
      toast.success(t('uploadSuccess', { count: d.uploaded }))
      setStickers((prev) => [...prev, ...d.stickers])
      setPacks((prev) => prev.map((p) =>
        p.pack_id === packId ? { ...p, stkr_cnt: p.stkr_cnt + d.uploaded } : p
      ))
    } catch (err) {
  ...
  async function removeSticker(packId: string, stkrId: string) {
    ...
    const res = await fetch(`/api/admin/stickers/${packId}/items/${stkrId}`, { method: 'DELETE' })
    if (!res.ok) { toast.error(t('stkrDeleteFail')); return }
    setStickers((prev) => prev.filter((s) => s.stkr_id !== stkrId))
    setPacks((prev) => prev.map((p) =>
      p.pack_id === packId ? { ...p, stkr_cnt: Math.max(0, p.stkr_cnt - 1) } : p
    ))
  }
    ...
    <div className='space-y-4'>
      <div>
        ...
        <p className='text-muted-foreground mt-1 text-sm'>
          {t('desc', { count: packs.length, official: officialCount, custom: customCount, active: activeCount })}
        </p>
      </div>
      ...
      <div className='rounded-lg border p-4 space-y-3'>
        ...
        <div className='flex flex-wrap gap-2 items-end'>
          <div className='flex flex-col gap-1'>
            <label className='text-xs text-muted-foreground'>{t('packNm')}</label>
            <input
              className='rounded-md border px-3 py-1.5 text-sm w-52 bg-background'
              value={newNm}
              onChange={(e) => setNewNm(e.target.value)}
            />
          </div>
          <div className='flex flex-col gap-1'>
            <label className='text-xs text-muted-foreground'>{t('packDesc')}</label>
            <input
              className='rounded-md border px-3 py-1.5 text-sm w-64 bg-background'
              value={newDesc}
          ...
          <div className='flex flex-col gap-1'>
            ...
            <input
              ...
              max={1000}
              step={0.1}
              className='rounded-md border px-3 py-1.5 text-sm w-24 bg-background'
              value={newPrice}
              onChange={(e) => setNewPrice(Number(e.target.value))}
            />
          </div>
          <label className='flex items-center gap-1.5 pb-2 text-xs text-muted-foreground'>
            <input type='checkbox' checked={newDflt} onChange={(e) => setNewDflt(e.target.checked)} />
            {t('isDflt')}
          </label>
      ...
      <div className='flex flex-wrap gap-2'>
        {(['all', 'official', 'custom', 'stopped'] as const).map((f) => (
          ...
          >
            {t(`filter.${f}`)}
            {f === 'official' && <span className='ml-1'>({officialCount})</span>}
        ...
        <div className='overflow-hidden overflow-x-auto rounded-lg border'>
          <table className='w-full text-sm'>
            <thead className='border-b bg-muted/50'>
              <tr>
                <th className='px-4 py-2 text-left font-medium'>{t('col.pack')}</th>
                <th className='px-4 py-2 text-left font-medium'>{t('col.theme')}</th>
                <th className='px-4 py-2 text-left font-medium'>{t('col.type')}</th>
                <th className='px-4 py-2 text-right font-medium'>{t('col.price')}</th>
                <th className='px-4 py-2 text-right font-medium'>{t('col.stkrCnt')}</th>
                <th className='px-4 py-2 text-right font-medium'>{t('col.ownrCnt')}</th>
                <th className='px-4 py-2 text-left font-medium'>{t('col.status')}</th>
                <th className='px-4 py-2 text-left font-medium'>{t('col.regDtm')}</th>
            ...
            <tbody className='divide-y'>
              {displayed.map((p) => (
                <PackTableRow
                  ...
                  editState={{ editNm, editTheme, editPrice, editUseYn, editDflt, setEditNm, setEditTheme, setEditPrice, setEditUseYn, setEditDflt }}
                  stickers={stickers}
...
function PackTableRow({
  pack: p, themes, expanded, editing, editState, stickers, detailLoading, uploading,
  fileInputRef, t, tc, onDetail, onEdit, onSave, onCancelEdit, onDelete, onUpload, onRemoveSticker,
}: {
  pack: PackRow
  themes: ThemeOption[]
  expanded: boolean
    ...
    <>
      <tr className='transition-colors hover:bg-muted/30'>
        <td className='px-4 py-3'>
          {editing ? (
            <input
              className='rounded border px-2 py-1 text-sm w-44 bg-background'
              value={es.editNm}
            ...
            <>
              ...
              {p.pack_desc && (
                <p className='max-w-56 truncate text-xs text-muted-foreground'>{p.pack_desc}</p>
              )}
            </>
          )}
        </td>
        ...
        <td className='px-4 py-3 text-right'>
          {editing ? (
            <input
              ...
              max={1000}
              step={0.1}
              className='w-20 rounded border px-2 py-1 text-xs text-right bg-background'
              value={es.editPrice}
          ...
          {editing ? (
            <div className='flex flex-col gap-1 text-xs'>
              <label className='flex items-center gap-1'>
                <input type='checkbox' checked={es.editUseYn} onChange={(e) => es.setEditUseYn(e.target.checked)} />
                {t('statusOn')}
              </label>
              <label className='flex items-center gap-1'>
                <input type='checkbox' checked={es.editDflt} onChange={(e) => es.setEditDflt(e.target.checked)} />
                {t('isDflt')}
              </label>
            </div>
          ) : (
        ...
        <td className='px-4 py-3'>
          {editing ? (
            <div className='flex gap-1'>
              <Button size='sm' className='h-6 px-2 text-xs' onClick={onSave}>{tc('save')}</Button>
              <Button size='sm' variant='ghost' className='h-6 px-2 text-xs' onClick={onCancelEdit}>{tc('cancel')}</Button>
            </div>
          ) : (
            <div className='flex gap-1'>
              <Button size='sm' variant='outline' className='h-6 px-2 text-xs' onClick={onDetail}>
              ...
              <div className='space-y-3'>
                ...
                {stickers.length === 0 ? (
                  ...
                  <div className='grid grid-cols-4 gap-3 sm:grid-cols-6 md:grid-cols-8'>
                    {stickers.map((s) => (
                      <div key={s.stkr_id} className='group relative rounded-lg border bg-background p-2'>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={s.stkr_url} alt={s.stkr_nm} className='aspect-square w-full object-contain' />
                        <p className='mt-1 truncate text-center text-[10px] text-muted-foreground'>{s.stkr_nm}</p>
                        <button
