...
const PROD_ORDER: Product[] = ['PICAFE', 'PISHOP', 'TRANSLATE']
...
export default function SubscrPricingPage() {
  ...
  // product → { M: row, Y: row }
  const grouped: Partial<Record<Product, { M?: FeePlanRow; Y?: FeePlanRow }>> = {}
  rows.forEach((r) => {
    const p = prodKey(r)
    if (!grouped[p]) grouped[p] = {}
    grouped[p]![r.bill_cycle_cd] = r
  })
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            {PROD_ORDER.map((prod) => {
              ...
              return (
                <div
                  key={prod}
                  className={`rounded-2xl border-2 bg-card p-5 shadow-sm ${meta.color}`}
                >
                  {/* 카드 헤더 */}
                  ...
                  <div className="space-y-3">
                    {[
                      { row: mRow, cycleLabel: '월간', cycleNote: '' },
                      { row: yRow, cycleLabel: '연간', cycleNote: '(2개월 무료)' },
                    ].map(({ row, cycleLabel, cycleNote }) => {
                      if (!row) return null
