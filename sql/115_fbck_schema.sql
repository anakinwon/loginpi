...
CREATE TABLE IF NOT EXISTS public.fbck_mst (
  ...
  -- ── 거래 유형: 카페 후기 XOR 상품 후기 (중복방지는 아래 CREATE UNIQUE INDEX로 처리)
...
-- ── 부분 유니크 인덱스: 카페별 사용자 1회만 (활성 후기)
CREATE UNIQUE INDEX IF NOT EXISTS uq_fbck_mst_usr_shop
  ON public.fbck_mst(usr_id, shop_id)
  WHERE shop_id IS NOT NULL AND del_yn='N';

-- ── 부분 유니크 인덱스: 주문별 사용자 1회만 (활성 후기)
CREATE UNIQUE INDEX IF NOT EXISTS uq_fbck_mst_usr_order
  ON public.fbck_mst(usr_id, order_id)
...
CREATE TABLE IF NOT EXISTS public.fbck_img (
  ...
  modr_id        TEXT            NOT NULL DEFAULT 'ADMIN',
  mod_dtm        TIMESTAMPTZ     NOT NULL DEFAULT CURRENT_TIMESTAMP
  -- ── 중복방지 부분 유니크 인덱스는 아래 CREATE UNIQUE INDEX로 처리
);
...
-- ── 부분 유니크 인덱스: 후기당 이미지 순서 중복 불가 (활성 이미지)
CREATE UNIQUE INDEX IF NOT EXISTS uq_fbck_img_ord
  ON public.fbck_img(fbck_id, img_ord)
  WHERE del_yn='N';
