...
-- ============================================================================
-- 4. evt_exclude 컬럼명 표준화: exclude_reason_tx → reason (DA 표준)
-- ============================================================================
ALTER TABLE evt_exclude
RENAME COLUMN exclude_reason_tx TO reason;

-- ============================================================================
-- 5. evt_gift_log 스키마 정정: 불필요한 컬럼 정리 후 선물 발송 상태 단순화
-- ============================================================================
-- 기존: gift_rank_no, kakao_id(중복) → 삭제하고 깔끔하게
-- 새 구조: user_id, gift_nm (선물명), sent_yn (Y/N), sent_dtm

-- 주의: 기존 데이터가 있다면 마이그레이션 신중히. 현재는 신규이므로 스키마만 정정.
-- 만약 기존 데이터가 있다면 COMMENT 또는 별도 마이그레이션 필요.
