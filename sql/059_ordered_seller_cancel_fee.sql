-- 오프라인 주문중(ORDERED) 판매자 취소수수료 적용 — 057에서 누락된 ORDERED 추가.
-- 판매자 취소: 구매자 1.0 + 보상 0.1 = 1.1π 환불 (보상은 판매자 보증금에서 충당)
-- 구매자 취소: 구매자 1.0 - 수수료 0.1 = 0.9π 환불
-- ⚠️ 보상/수수료는 판매자 보증금(>=0.1π) 활성 시에만 적용 (무보증금이면 전액 환불)
...
BEGIN
  ...
  -- 판매자 수수료: 직거래(ESCROW/TRADING/SELLER_DONE) + 오프라인 ORDERED에서 판매자 취소
